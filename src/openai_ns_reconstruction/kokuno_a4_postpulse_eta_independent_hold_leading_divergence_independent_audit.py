"""Independent divergence audit for exact A1 #1148 eta-independent hold.

Scientific derivatives consume only the save/reloaded public Cartesian
``velocity(x,y,z,t)`` surface.  A4 reconstructs Cartesian derivatives with its
own centered FD2 operator at the preregistered absolute steps.  Candidate
analytic/source-coordinate derivatives are deliberately excluded from the
scientific divergence path.
...
from .kokuno_pa16_current_cartesian_postpulse_eta_independent_hold import (
    KokunoPA16CurrentCartesianPostPulseEtaIndependentHold,
)

TASK = "K4-VAL-119"
SCHEMA = "kokuno-a4-postpulse-hold-leading-divergence-audit-v1"
UPSTREAM_PR = 1148
UPSTREAM_HEAD = "76aba89041e8103ac464fda9ed75f7a18ef596b9"
PARENT_PR = 1140
PARENT_HEAD = "c5442c11165d7f0976889b826bf58dce38a3d3dd"
SEED = 9173911
SPATIAL_STEPS = (0.02, 0.01, 0.005)
TIMES = (0.31, 0.47, 0.63, 0.71)
HOLD_RHO_BANDS = ((0.08, 0.24), (0.38, 0.58), (0.72, 0.88))
...
_TRUTH_BOUNDARY = {
    "exact_a1_1148_postpulse_hold_leading_consumed": True,
    ...
    "matching_postpulse_hold_oscillatory_composite_materialized": False,
...
def default_field() -> KokunoPA16CurrentCartesianPostPulseEtaIndependentHold:
...
def _region(
    ...
    log_X = field.log_X_flatten_end + field.hold_length * rho
    ...
    dlogX = field.hold_length * (rho_hi - rho_lo)
...
def make_heldout_probes(field: Any) -> HeldoutProbeSet:
    ...
    for tid, time in enumerate(TIMES):
        for zid, (lo, hi) in enumerate(HOLD_RHO_BANDS):
            blocks.append(
                _region(
                    field,
                    rng,
                    time,
                    tid,
                    f"eta_independent_hold_zone_{zid}",
                    lo,
                    hi,
                )
            )
    ...
    late_log_X = field.log_X_flatten_end + field.hold_length * late_rho
...
def _subset_divergence(
    ...
    jac = independent_fd2_jacobian(field, np.asarray(points)[mask], np.asarray(times)[mask], step)
...
def _metrics(field: Any, probes: HeldoutProbeSet, step: float) -> dict[str, Any]:
    ...
    if len(valid_indices):
        ...
        norm_weighted = _stable_weighted_metrics(normed, probes.log_weights[valid_indices])
    ...
    for label in sorted(set(map(str, probes.region.tolist()))):
        chosen = [j for j, idx in enumerate(valid_indices) if str(probes.region[idx]) == label]
        item = {
            "probe_count": int(np.count_nonzero(probes.region == label)),
            "representable_count": len(chosen),
        }
        if chosen:
            vals = div[np.asarray(chosen, int)]
            idxs = valid_indices[np.asarray(chosen, int)]
    ...
    by_time = []
    for tid, time in enumerate(TIMES):
        chosen = [j for j, idx in enumerate(valid_indices) if int(probes.time_index[idx]) == tid]
        item = {
            "time": time,
    ...
    seam_div, seam_rep = _subset_divergence(field, probes.seam_points, probes.seam_times, step)
    axis_div, axis_rep = _subset_divergence(field, probes.axis_points, probes.axis_times, step)
    late_div, late_rep = _subset_divergence(field, probes.late_points, probes.late_times, step)
    return {
        ...
        "hold_entry_seam_fd_representability": seam_rep,
        "axis_fd_representability": axis_rep,
        "late_hold_fd_representability": late_rep,
        "hold_entry_seam_sampled_max": float(np.max(np.abs(seam_div))) if seam_div.size else None,
        "axis_axis_near_sampled_max": float(np.max(np.abs(axis_div))) if axis_div.size else None,
        "late_hold_sampled_max": float(np.max(np.abs(late_div))) if late_div.size else None,
...
def _configuration_mutation_rejected(saved: Mapping[str, Any]) -> bool:
    ...
    raw["bound_scope"]["hold_length_role"] = "residual_tuned"
    try:
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.from_configuration(raw)
    except (ValueError, RuntimeError):
        return True
    return False


def _hold_length_mutation_rejected(saved: Mapping[str, Any]) -> bool:
    raw = copy.deepcopy(dict(saved))
    raw["bound_scope"]["hold_length"] = float(raw["bound_scope"]["hold_length"]) * 1.001
    try:
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.from_configuration(raw)
    except (ValueError, RuntimeError):
        return True
...
def _post_stage_fails_closed(field: Any) -> bool:
    try:
        field.similarity_profile_values_logX(field.log_X_hold_end + 0.02, 0.0)
    except (ValueError, RuntimeError):
        return True
    return False
...
def _all_fd_representable(metrics: Mapping[str, Any]) -> bool:
    return all(
        ...
        for key in (
            ...
            "hold_entry_seam_fd_representability",
            "axis_fd_representability",
            "late_hold_fd_representability",
        )
    )
...
def materialize_receipt() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        field = KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.load_configuration(path)
        ...
        role_mutation_rejected = _configuration_mutation_rejected(saved)
        length_mutation_rejected = _hold_length_mutation_rejected(saved)
    ...
    gates = {
        ...
        "hold_entry_seam_pass": _numeric_leq(
            fine["hold_entry_seam_sampled_max"], DIVERGENCE_GATE
        ),
        "axis_near_pass": _numeric_leq(
            fine["axis_axis_near_sampled_max"], DIVERGENCE_GATE
        ),
        "late_hold_pass": _numeric_leq(fine["late_hold_sampled_max"], DIVERGENCE_GATE),
        ...
        "hold_role_mutation_rejected": role_mutation_rejected,
        "hold_length_mutation_rejected": length_mutation_rejected,
    ...
    return {
        ...
        "times": list(TIMES),
        "hold_rho_bands": [list(v) for v in HOLD_RHO_BANDS],
        "late_rho": list(LATE_RHO),
        "hold_length": field.hold_length,
        ...
        "probe_counts": {
            ...
            "late_hold": len(probes.late_points),
        },
        ...
        "limitations": [
            ...
            "A2 has no matching self-contained eta-independent-hold composite on exact A1 #1148",
...
def enforce_preregistered_gates(report: Mapping[str, Any]) -> None:
    ...
    required = (
        ...
        "hold_role_mutation_rejected",
        "hold_length_mutation_rejected",
