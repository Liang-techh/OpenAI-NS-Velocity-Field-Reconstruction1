"""Independent A4 audit of the current RF40 power-law radial force.

Consumes JSON-round-tripped public receipts from Agent-3 PR #1045 and public
shifted axial-stress receipts from Agent-3 PR #1037.  The independent reference
...
TASK = "K4-VAL-107"
SCHEMA = "kokuno-a4-current-rf40-power-law-radial-force-independent-audit-v1"
...
PARENT_FORCE_SCHEMA = "kokuno-a3-current-rf40-power-law-nonlinear-radial-force-v1"

STRESS_AGENT3_PR = 1037
STRESS_AGENT3_HEAD = "9a5cdcdf78b7862d5ebe171efb9ca56d53664612"
STRESS_AGENT3_SOURCE_BLOB = "44dc6d61bc965cef08b119aa5df3026175abd5de"
STRESS_SCHEMA = "kokuno-a3-current-rf40-power-law-nonlinear-radial-stress-v1"
STRESS_PARENT_AGENT3_1028_HEAD = "4b7a4400dcf1ee0be6bb07f23c2f8af6f893de88"
STRESS_PARENT_AGENT3_1028_SOURCE_BLOB = "4ec98a6adbb7afd16d6595cc24072fe18e62fa87"

AGENT2_1010_HEAD = "e36d9da4b7f037e998f5b1658f8c0ea291a76b80"
AGENT1_1005_HEAD = "2c76ebdc41d6c566f43a1305034ba2ff9dce410b"

FROZEN_SEED = 9173791
FROZEN_TIME = 0.39
FROZEN_Z_CENTERS = (-0.143, -0.067, 0.049)
...
def frozen_radial_grid() -> np.ndarray:
    """Deterministic positive-radius off-grid ladder."""
...
def _validate_force_receipt(
    ...
    *,
    ...
    if receipt.get("schema") != PARENT_FORCE_SCHEMA:
        raise ValueError("unexpected A3 #1045 force receipt schema")
    if receipt.get("parent_agent3_1037_source_blob") != STRESS_AGENT3_SOURCE_BLOB:
        raise ValueError("A3 #1037 source identity drifted")
...
def _validate_stress_receipt(
    ...
    *,
    ...
    if receipt.get("schema") != STRESS_SCHEMA:
        raise ValueError("unexpected A3 #1037 stress receipt schema")
    if receipt.get("parent_agent3_1028_source_blob") != STRESS_PARENT_AGENT3_1028_SOURCE_BLOB:
        raise ValueError("A3 #1028 source identity drifted")
    provenance = receipt.get("provenance")
    if not isinstance(provenance, Mapping):
        raise ValueError("stress receipt provenance is missing")
    expected = {
        "parent_agent3_head": STRESS_PARENT_AGENT3_1028_HEAD,
        "agent2_composite_head": AGENT2_1010_HEAD,
        "agent1_leading_head": AGENT1_1005_HEAD,
    }
    for key, value in expected.items():
...
def _fd4(
    indexed: Mapping[float, Mapping[str, np.ndarray]],
    piece: str,
    h: float,
) -> np.ndarray:
    return (
...
def enforce_scientific_gates(report: Mapping[str, Any]) -> None:
    ...
    if report.get("parent_agent3_head") != PARENT_AGENT3_HEAD:
        raise RuntimeError("A3 #1045 identity drifted")
    if report.get("audit_pass") is not True:
        raise RuntimeError(
            "independent RF40 power-law radial-force audit failed: "
