...
This is the narrow Agent-3 seam after ``kokuno_typed_mean_radial_stress``.
The public API accepts only raw same-cycle field providers plus physical radial
geometry and derivative settings.  It recomputes

    R = u_t + (u . grad)u + grad(p) - nu Delta(u) - f,

projects the actual theta/z angular means, reconstructs their compact radial
stresses, and then applies the already-frozen repository-autonomous finite-head
factor

    Delta C_aut = -w_aut * requestedStress_actual.

The factor is replayed from Agent-3 #514 through the exact helper already frozen
in #586.  It is deliberately *not* Kokuno's nonconstructive theorem-machine
``missingWeight`` and is not retuned here.  This module therefore closes an
engineering execution seam; it does not claim a real full-candidate debt until
Agent 1 supplies the missing same-cycle leading/cross, matched-pressure and
restricted-forcing handoff.
"""
...
from .kokuno_autonomous_oscillatory_mean_debt import (
    EXPECTED_AUTONOMOUS_MISSING_WEIGHT,
    _replay_autonomous_factor,
)
...
TASK = "KOKUNO-A3-TYPED-MEAN-FINITE-HEAD-DEBT-053"
SCHEMA = "kokuno-a3-typed-mean-finite-head-debt-v1"
PARENT_AGENT3_PR = 677
PARENT_AGENT3_HEAD = "56024553b981833a283f98648c8599011d6f38ab"
...
def materialize_typed_mean_finite_head_debt(
    ...
    *,
    ...
    No residual, mean source, radial stress or finite-head factor may be supplied
    by the caller.  The actual stress is recomputed by #677 and the finite-head
    factor is replayed from the frozen #514/#586 engineering realization.
    """
...
def truth_boundary() -> dict[str, object]:
    ...
    return {
        ...
        "autonomous_factor_replay_provider": "kokuno_autonomous_oscillatory_mean_debt:_replay_autonomous_factor",
...
def deterministic_receipt() -> dict[str, object]:
    ...
    identity = CycleIdentity("analytic-typed-mean-debt-v1", 0, "t0")
    center = 0.50
    halfwidth = 0.30
    ...
    return {
        ...
        "provenance": {
            ...
            "repository_role": "typed actual mean stress -> frozen autonomous finite-head debt",
            "autonomous_factor_origin_pr": AUTONOMOUS_FACTOR_ORIGIN_PR,
            "autonomous_factor_replay_pr": AUTONOMOUS_FACTOR_REPLAY_PR,
        },
        "analytic_regression": {
