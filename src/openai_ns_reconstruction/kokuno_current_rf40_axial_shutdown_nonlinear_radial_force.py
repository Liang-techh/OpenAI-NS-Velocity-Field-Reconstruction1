"""Materialize RF40 axial-shutdown radial force from current compact axial stress.

The corrected 2026-09-09 Kokuno symmetric compact stress representation has

    (div T)_r = partial_z sigma_1,

where ``sigma_1`` is the axial/e=1 compact stress.  Agent-3 PR #1015 already
materializes the current mixed/quadratic/aggregate compact stresses on the
identity-bound leading+oscillatory lineage through RF40 axial shutdown ``X_2``.
This module performs only that next source-required divergence step by
recomputing the exact same stress at neighboring physical-z slices.
...
from .kokuno_current_rf40_axial_shutdown_nonlinear_radial_stress import (
...
TASK = "KOKUNO-A3-CURRENT-RF40-AXIAL-SHUTDOWN-NONLINEAR-RADIAL-FORCE-111"
SCHEMA = "kokuno-a3-current-rf40-axial-shutdown-nonlinear-radial-force-v1"
...
HISTORICAL_CURRENT_RADIAL_FORCE_AGENT3_PR = 1006
HISTORICAL_CURRENT_RADIAL_FORCE_AGENT3_HEAD = (
    "2c10faab7c229b801cb868355e0a1de0a729445c"
)
...
def _authenticate_parent_source() -> str:
    ...
    if source is None:
        raise RuntimeError("RF40 axial-shutdown nonlinear radial-stress source is unavailable")
    blob = _git_blob_sha1(Path(source))
    if blob != PARENT_AGENT3_SOURCE_BLOB:
        raise RuntimeError("A3 #1015 RF40 axial-shutdown radial-stress source blob drifted")
    return blob


def _axial_stress_pieces(
    witness: CurrentRF40AxialShutdownNonlinearRadialStressWitness,
) -> dict[str, np.ndarray]:
    if not isinstance(witness, CurrentRF40AxialShutdownNonlinearRadialStressWitness):
        raise TypeError(
            "witness must be CurrentRF40AxialShutdownNonlinearRadialStressWitness"
        )
...
class CurrentRF40AxialShutdownNonlinearRadialForceWitness:
    """Current through-X2 compact stress plus source-required radial force."""

    geometry: StrictInnerTransportRadialGeometry
    center_stress: CurrentRF40AxialShutdownNonlinearRadialStressWitness
    z_steps: tuple[float, ...]
    ...
    def to_receipt(self) -> dict[str, object]:
        return {
            ...
            "task": TASK,
            "backend_kind": (
                "exact-current-a3-pr1015-axial-shutdown-axial-stress-to-radial-force"
            ),
            "geometry": {
            ...
            "parent_agent3_1015_source_blob": self.parent_source_blob,
            "provenance": {
...
def materialize_current_rf40_axial_shutdown_nonlinear_radial_force(
    backend: ExactCurrentRF40AxialShutdownNonlinearBackend,
    geometry: StrictInnerTransportRadialGeometry,
) -> CurrentRF40AxialShutdownNonlinearRadialForceWitness:
    """Recompute #1015 stresses at neighboring z and form ``partial_z sigma_1``."""
    ...
    center = materialize_current_rf40_axial_shutdown_nonlinear_radial_stress(
        backend, geometry
    )

    def sampler(z_value: float) -> Mapping[str, np.ndarray]:
        shifted = replace(geometry, axial_z=float(z_value))
        witness = materialize_current_rf40_axial_shutdown_nonlinear_radial_stress(
            backend, shifted
        )
    ...
    return CurrentRF40AxialShutdownNonlinearRadialForceWitness(
        geometry=geometry,
...
def truth_boundary() -> dict[str, object]:
    signature = inspect.signature(
        materialize_current_rf40_axial_shutdown_nonlinear_radial_force
    )
    forbidden = {
    ...
    return {
        ...
        "current_RF40_axial_shutdown_axial_e1_stress_consumed": True,
        "current_RF40_axial_shutdown_radial_force_from_dz_sigma1_materialized": True,
        ...
        "scoped_current_RF40_axial_shutdown_radial_force_authorized_as_correction_target": False,
