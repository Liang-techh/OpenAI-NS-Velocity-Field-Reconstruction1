"""Route exact current-I2 nonlinear m=0 means through compact radial stress.

This Kokuno Agent-3 increment consumes the exact current-I2 nonlinear mean
attribution from PR #1073,

    M_bar = <(u_lead . grad)u_osc + (u_osc . grad)u_lead>_theta,
    Q_bar = <(u_osc . grad)u_osc>_theta,
    N_bar = M_bar + Q_bar,

for the identity-bound Agent-2 PR #1071 / Agent-1 PR #1061 candidate.  It
...
PARENT_AGENT3_SOURCE_BLOB = "a9288dc3fe5e3f5e37b33c10762d42b93dfca529"
...
def _authenticate_parent_source() -> str:
    source = inspect.getsourcefile(materialize_current_i2_nonlinear_mean_attribution)
    if source is None:
        raise RuntimeError("current-I2 nonlinear mean source is unavailable")
    blob = _git_blob_sha1(Path(source))
    if blob != PARENT_AGENT3_SOURCE_BLOB:
        raise RuntimeError("A3 #1073 current-I2 nonlinear mean source blob drifted")
    return blob


@dataclass(frozen=True)
class CurrentI2NonlinearRadialStressWitness:
    """Exact current-I2 mean attribution plus inherited compact radial stresses."""
    ...
    def to_receipt(self) -> dict[str, object]:
        ...
        return {
            "schema": SCHEMA,
            "task": TASK,
            "backend_kind": "exact-current-a3-pr1073-mean-to-inherited-compact-radial-stress",
            "geometry": base["geometry"],
            "current_i2_mean_witness": base["mean_witness"],
            ...
            "parent_agent3_1073_source_blob": self.parent_source_blob,
            "provenance": {
...
def _materialize_current_mean_witness(
    ...
    *,
    ...
    """Apply only the inherited compact radial operator to an exact #1073 mean."""
    ...
    if parent_source_blob != PARENT_AGENT3_SOURCE_BLOB:
        raise ValueError("parent source blob does not match exact A3 #1073")

    inherited = _materialize_from_mean_witness(
        witness,
        geometry,
        backend_kind="current-a3-pr1073-mean-reusing-a3-pr920-a3-pr875-radial-operator",
    )
    if inherited.mean_witness is not witness:
        raise RuntimeError("inherited radial routing detached the current #1073 mean witness")
    return CurrentI2NonlinearRadialStressWitness(
        inherited=inherited,
        parent_source_blob=parent_source_blob,
    )


def materialize_current_i2_nonlinear_radial_stress(
    backend: ExactCurrentI2NonlinearBackend,
    geometry: StrictInnerTransportRadialGeometry,
) -> CurrentI2NonlinearRadialStressWitness:
    """Recompute exact #1073 means and route them through compact radial stress."""
    ...
    witness = materialize_current_i2_nonlinear_mean_attribution(backend, radii, z, t)
    if witness.backend_kind != "exact-current-a2-pr1071-through-i2-nonlinear-attribution":
        raise RuntimeError("current-I2 nonlinear mean backend identity drifted")
...
def truth_boundary() -> dict[str, object]:
    signature = inspect.signature(materialize_current_i2_nonlinear_radial_stress)
    forbidden = {
    ...
    return {
        ...
        "current_I2_mixed_nonlinear_mean_consumed": True,
        "current_I2_quadratic_oscillatory_mean_consumed": True,
        "current_I2_aggregate_nonlinear_mean_consumed": True,
        "current_I2_nonlinear_radial_inverse_performed": True,
        "current_I2_quadratic_radial_stress_materialized": True,
        "current_I2_mixed_radial_stress_materialized": True,
        "current_I2_aggregate_radial_stress_materialized": True,
        ...
        "scoped_current_I2_radial_stress_authorized_as_complete_ns_defect": False,
        "scoped_current_I2_radial_stress_authorized_as_correction_target": False,
