"""Independent A4 audit of the exact current-I2 correction-side radial force.

Consumes JSON-round-tripped public receipts from Agent-3 PR #1090 and public
shifted axial-stress receipts from Agent-3 PR #1081. The independent reference
never calls the production centered-FD2 differentiation helper. It reconstructs
...
This is scoped correction-side operator-consistency evidence only. It is not a
...
PARENT_AGENT3_SOURCE_BLOB = "9259ecdf4aa3912fb521acdf7e62060272395e38"
PARENT_FORCE_SCHEMA = "kokuno-a3-current-i2-nonlinear-radial-force-v1"

STRESS_AGENT3_PR = 1081
STRESS_AGENT3_HEAD = "cb9ee25d0c466d42ec7ca30ee89cc10517d4b5c4"
STRESS_AGENT3_SOURCE_BLOB = "876cdd2cbe4060b9d249cfc4960c26da0d577ca9"
STRESS_SCHEMA = "kokuno-a3-current-i2-nonlinear-radial-stress-v1"
STRESS_PARENT_AGENT3_1073_HEAD = "a798056d7ddabe0bf4020082861807bc5ac1efbd"
STRESS_PARENT_AGENT3_1073_SOURCE_BLOB = "a9288dc3fe5e3f5e37b33c10762d42b93dfca529"

AGENT2_1071_HEAD = "48d69e37e78e7f7f0e4e9936f28ff7974719288d"
AGENT1_1061_HEAD = "9c2abbc69ba1ef8c8a9e8d1699a466a75d0293c3"

FROZEN_SEED = 9173841
FROZEN_TIME = 0.39
FROZEN_Z_CENTERS = (-0.149, -0.061, 0.053)
...
def _validate_force_receipt(
    ...
    *,
    ...
    if receipt.get("schema") != PARENT_FORCE_SCHEMA:
        raise ValueError("unexpected A3 #1090 force receipt schema")
    if receipt.get("parent_agent3_1081_source_blob") != STRESS_AGENT3_SOURCE_BLOB:
        raise ValueError("A3 #1081 source identity drifted")
    ...
    expected = {
        ...
        "agent2_composite_head": AGENT2_1071_HEAD,
        "agent1_leading_head": AGENT1_1061_HEAD,
    }
...
def _validate_stress_receipt(
    ...
    *,
    ...
    if receipt.get("schema") != STRESS_SCHEMA:
        raise ValueError("unexpected A3 #1081 stress receipt schema")
    if receipt.get("parent_agent3_1073_source_blob") != STRESS_PARENT_AGENT3_1073_SOURCE_BLOB:
        raise ValueError("A3 #1073 source identity drifted")
    provenance = receipt.get("provenance")
    if not isinstance(provenance, Mapping):
        raise ValueError("stress receipt provenance is missing")
    expected = {
        "parent_agent3_head": STRESS_PARENT_AGENT3_1073_HEAD,
        "agent2_composite_head": AGENT2_1071_HEAD,
        "agent1_leading_head": AGENT1_1061_HEAD,
    }
    for key, value in expected.items():
...
def _audit_case(
    ...
    for piece in ("quadratic", "mixed", "aggregate"):
        ...
        cm = _relative_rms(independent_levels[piece][0.02], independent_levels[piece][0.01])
        mf = _relative_rms(independent_levels[piece][0.01], independent_levels[piece][0.005])
        pieces_out[piece] = {
            "levels": levels_out,
...
def audit_serialized_cases(cases: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    """Audit preregistered held-out z centers from public JSON receipts only."""
...
def enforce_scientific_gates(report: Mapping[str, Any]) -> None:
    ...
    if report.get("parent_agent3_head") != PARENT_AGENT3_HEAD:
        raise RuntimeError("A3 #1090 identity drifted")
    if report.get("audit_pass") is not True:
        raise RuntimeError(
            "independent current-I2 radial-force audit failed: "
...
def truth_boundary() -> dict[str, Any]:
    ...
    return {
        ...
        "heldout_z_centers": list(FROZEN_Z_CENTERS),
        "z_step_ladder": list(FROZEN_Z_STEPS),
        "newer_i3_i4_identity_consumed": False,
        "complete_ns_defect": False,
        "authorized_correction_target": False,
        "cartesian_correction_velocity_materialized": False,
        "after_correction_ns_residual_assessed": False,
