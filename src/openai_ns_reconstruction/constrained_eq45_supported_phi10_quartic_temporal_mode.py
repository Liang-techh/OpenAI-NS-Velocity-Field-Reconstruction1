"""Callable derivative-balanced quartic Phi(1,0) supported Eq45 candidate.

Agent-7 PR #166 screened one additional temporal nullspace degree on the existing
support-connected cubic Phi(1,0) schedule.  The screened displacement preserves
the already-selected early snapshot and the exact static-supported returns at
`t=.50/.625/.75`, while reducing return-frame coefficient-slope and late-half
velocity collateral.  The nullspace coefficient is selected target-free from the
profile schedule only; no PDE residual or public image enters that selection.

This module materializes that already-screened schedule as a bounded, callable,
serializable velocity candidate.  Every scalar-time snapshot delegates to
#166's ``quartic_phi10_snapshot`` and therefore reuses the existing Eq. (4.5)
backbone and physical-support transform rather than duplicating them.

This remains a visualization/research candidate only.  It does not identify the
hidden OpenAI field, validate Navier--Stokes, establish paper exactness, or prove
blow-up.
"""
...
from .constrained_eq45_supported_phi10_quartic_derivative_capacity import (
    ...
    quartic_phi10_delta,
    quartic_phi10_snapshot,
)


SCHEMA = "eq45_supported_phi10_quartic_derivative_balanced_temporal_candidate_v1"
_MODE = (1, 0)
_SCHEDULE = (
    "cubic_phi10_delta(tau,early_delta)+a*(tau+1)*tau*(tau-0.5)*(tau-1)"
)
_NULLSPACE_RULE = "least_squares_minimize_return_node_coefficient_slope"
...
_CLASSIFICATION = {
    ...
    "quartic_temporal_shape": "autonomous_design",
    "balanced_nullspace_rule": "autonomous_design",
...
class Eq45SupportedPhi10QuarticDerivativeBalancedTemporalCandidate:
    """Support-connected Eq45 field with #166's balanced quartic Phi(1,0) shape."""
    ...
    def __post_init__(self) -> None:
        ...
        # The capacity helper validates the complete quartic coefficient range,
        # not just this midpoint snapshot, before returning a supported child.
        quartic_phi10_snapshot(
            self.base,
            self.time_midpoint,
            early_delta=float(self.early_delta),
            nullspace_coefficient=self.nullspace_coefficient,
        )
    ...
    @property
    def nullspace_coefficient(self) -> float:
        """Return the deterministic #166 target-free balancing coefficient."""
        return float(balanced_nullspace_coefficient(early_delta=float(self.early_delta)))
    ...
    def coefficient_at_tau(self, tau):
        ...
        return self.midpoint_coefficient + quartic_phi10_delta(
            tau,
            early_delta=float(self.early_delta),
            nullspace_coefficient=self.nullspace_coefficient,
        )
    ...
    def velocity(self, points, time) -> np.ndarray:
        """Evaluate Cartesian ``[...,3] = [u,v,w]`` from the quartic temporal field."""
        ...
        if not np.all(np.isfinite(out)):
            raise RuntimeError("quartic temporal Eq45 velocity became nonfinite")
        return out
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "temporal_mode": {
                ...
                "early_delta": float(self.early_delta),
                "nullspace_coefficient": self.nullspace_coefficient,
                "nullspace_rule": _NULLSPACE_RULE,
                "schedule": _SCHEDULE,
    ...
    def from_dict(
        ...
        if not isinstance(data, Mapping):
            raise ValueError("quartic temporal candidate payload must be an object")
        ...
        if temporal.get("nullspace_rule") != _NULLSPACE_RULE:
            raise ValueError("balanced nullspace rule was modified")
        if temporal.get("tau_definition") != _TAU_DEFINITION:
            raise ValueError("temporal tau definition was modified")
        if "early_delta" not in temporal or "nullspace_coefficient" not in temporal:
            raise ValueError("temporal_mode is missing a required coefficient")

        early_delta = float(temporal["early_delta"])
        expected = float(balanced_nullspace_coefficient(early_delta=early_delta))
        supplied = float(temporal["nullspace_coefficient"])
        if not np.isfinite(supplied) or supplied != expected:
            raise ValueError("serialized nullspace coefficient does not match the balanced rule")

        return cls(base=base, early_delta=early_delta)
