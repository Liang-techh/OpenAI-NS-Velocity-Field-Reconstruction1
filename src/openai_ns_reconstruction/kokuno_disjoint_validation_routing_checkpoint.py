"""Agent-5 routing checkpoint after disjoint damping validation was made executable.

Integration only: bind immutable Agent-1/2/3/4 receipts, preserve source and
validation truth boundaries, and route the shortest path to a global candidate.
No missing mathematical lane is reconstructed here and no final candidate is
instantiated.
"""
...
def _pairs(times: list[float], z_values: list[float]) -> set[tuple[float, float]]:
    return {(float(t), float(z)) for z in z_values for t in times}
    ...
    pipeline = [
        {
            ...
            "ready": False,
            "status": "Xi_boundary_and_band_schedule_executable_missing_incoming_five_moment_Tsh_and_actual_positive_order_multiband_modes",
        },
        {
            "stage": "leading_candidate",
            "ready": False,
            "status": "selected_Xi_boundary_ready_missing_incoming_discrepancy_Tsh_inner_join_I3_I4_global_pressure",
        },
        {
            "stage": "oscillatory_augmentation",
            "ready": False,
            "status": "source_torus_and_band_covering_ready_missing_actual_positive_order_background_multiband_modes_and_public_by_beta_total_velocity",
        },
        {
            "stage": "mean_radial_corrections",
            "ready": False,
            "status": "damping_calibration_holdout_split_passes_local_transfer_but_one_band_rank_two_is_0_of_25",
        },
        {
            "stage": "finite_correction_cycle",
            "ready": False,
            "status": "blocked_until_actual_multiband_family_rank_budget_spacetime_radial_quadratic_guards_pass",
        },
        {"stage": "candidate_artifact", "ready": False, "status": "schema_reserved_not_instantiated"},
        {
            "stage": "independent_validation",
            "ready": False,
            "status": "local_A4_source_torus_audit_exists_A3_holdout_transfer_is_not_the_formal_A4_full_domain_PDE_gate",
        },
        {"stage": "report_and_export", "ready": False, "status": "blocked_until_frozen_candidate_artifact_exists"},
    ]
    routing = {
        "new_fact": (
            "Agent-3 #431 separates the shared damping calibration grid from a disjoint validation grid and transfers the frozen lambda without retuning. The held-out local stress transfer passes, but the upstream one-band inverse remains rank-deficient, so no correction or finite cycle is promoted."
        ),
        "interpretation": (
            "The prior shared-lambda gain is no longer being treated as if calibration cells were held-out cells. This closes a validation-semantics seam only. The shortest global blocker remains Agent-2's missing actual multi-band physical velocity family with a genuinely independent covariance direction; Agent-1's global leading closure proceeds in parallel."
        ),
        "shortest_next_closure": [
            ...
            "Agent 3: consume exactly that family, require genuine rank-two coverage plus unchanged unit/budget/spacetime/radial/quadratic guards, then repeat the disjoint damping-transfer protocol on the proposed actual correction before materialization and finite-cycle rerun.",
            "Agent 1: propagate the executable Xi boundary through the missing incoming five-moment discrepancy and numerical T_sh certificate, complete inner-to-outer join, I3/I4, and global matched pressure under fixed/restricted forcing.",
            "Agent 4: independently audit the first actual multi-band physical family and, only after a frozen full composite exists, run the formal full-domain normalized NS gate with same-protocol ST006 comparison.",
            "Agent 5: instantiate/save/load/export the deterministic candidate only after leading, oscillatory and correction stages coexist in one executable ancestry.",
        ],
        "do_not_do": [
            "do not call the A3 local stress-transfer residual an NS residual",
            ...
            "do not call autonomous or caller-supplied hidden source choices recovered Kokuno data",
            "do not run/promote the finite correction cycle before all frozen guards pass",
...
def validate_checkpoint(payload: dict[str, Any]) -> None:
    ...
    if payload["upstream"]["agent4"]["formal_full_domain_pde_gate_assessed"]:
        raise ValueError("Agent-4 local source audit cannot be promoted to formal PDE validation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output",
        default="artifacts/kokuno_agent5/disjoint_validation_routing_checkpoint_v26/checkpoint.json",
    )
