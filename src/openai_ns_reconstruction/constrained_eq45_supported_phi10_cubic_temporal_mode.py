"""Callable cubic-localized temporal Phi(1,0) supported Eq45 candidate.

Agent-7 PR #158 screened one additional temporal degree on the existing
support-connected Eq45 Phi(1,0) mode.  The cubic displacement is

    delta Phi10(tau) = early_delta * tau * (tau - 0.5) * (tau - 1) / (-3),

with tau=-1,0,0.5,+1 mapping to t=.25,.50,.625,.75 for the current delivery
interval.  The default early_delta=-1.4 preserves the already-useful early
endpoint while returning exactly to the static supported field at the midpoint,
intermediate late-half checkpoint, and late endpoint.

This module materializes that already-screened shape as a bounded, callable,
serializable velocity candidate.  Every scalar-time snapshot delegates to
#158's ``cubic_phi10_snapshot`` and therefore reuses the existing Eq. (4.5)
backbone and physical-support transform rather than duplicating them.

This remains a visualization-candidate representation only.  It does not fit a
...
SCHEMA = "eq45_supported_phi10_cubic_localized_temporal_candidate_v1"
_MODE = (1, 0)
_SCHEDULE = "early_delta*tau*(tau-0.5)*(tau-1)/(-3)"
...
_CLASSIFICATION = {
    ...
    "cubic_temporal_shape": "autonomous_design",
...
class Eq45SupportedPhi10CubicLocalizedTemporalCandidate:
    """Support-connected Eq45 field with the screened cubic Phi(1,0) shape."""
    ...
    def __post_init__(self) -> None:
        ...
        # The cubic derivative has exactly two interior roots.  Endpoints plus
        # those roots cover the full coefficient range on tau in [-1,1].
        root_lo = (3.0 - np.sqrt(3.0)) / 6.0
        root_hi = (3.0 + np.sqrt(3.0)) / 6.0
        check_tau = np.array([-1.0, root_lo, root_hi, 1.0], dtype=float)
        ...
        if np.min(coefficients) < -limit or np.max(coefficients) > limit:
            raise ValueError(
                "cubic Phi(1,0) schedule exceeds the existing profile coefficient bound "
    ...
    def coefficient_at_tau(self, tau):
        ...
        return self.midpoint_coefficient + cubic_phi10_delta(
    ...
    def velocity(self, points, time) -> np.ndarray:
        """Evaluate Cartesian ``[...,3] = [u,v,w]`` from the cubic temporal field."""
        ...
        if not np.all(np.isfinite(out)):
            raise RuntimeError("cubic-localized temporal Eq45 velocity became nonfinite")
        return out
    ...
    def from_dict(
        ...
        if not isinstance(data, Mapping):
            raise ValueError("cubic-localized temporal candidate payload must be an object")
