"""Current-lineage nonlinear mean attribution through the RF40 power-law endpoint.

This Kokuno Agent-3 increment consumes the exact Agent-2 PR #1010 field

    u_current_to_X4 = u_lead,current-through-X4 + u_osc,frozen-complete-curl

through the RF40 constant-lambda power-law endpoint X_4, together with the
already-frozen Agent-2 PR #960 oscillatory Jacobian.  It deliberately reuses
the existing Agent-3 nonlinear decomposition and rotating cylindrical m=0
projector rather than reproducing Agent-2 curl/Jacobian machinery or Agent-1
RF40 construction.

This remains a scoped nonlinear-transport attribution.  It does not perform a
...
TASK = "KOKUNO-A3-CURRENT-RF40-POWER-LAW-NONLINEAR-MEAN-112"
SCHEMA = "kokuno-a3-current-rf40-power-law-nonlinear-mean-v1"
...
AGENT2_COMPOSITE_CLASS = "CurrentRF40PowerLawLeadingOscillatoryField"
AGENT2_COMPOSITE_SOURCE_BLOB = "4f1dc0566c041b9de20f18b4ae9c49d9fd93c58d"

AGENT2_COMPOSITE_PARENT_PR = 1004
AGENT2_COMPOSITE_PARENT_HEAD = "e9697cdfe2249e1b8bc2f9241bd16d456c60418f"
AGENT2_COMPOSITE_PARENT_SOURCE_BLOB = "def2998256b2339115c2400c75b6db6ab58dfe95"
...
def _validate_power_law_configuration(
    configuration: Mapping[str, Any],
    *,
    runtime_semantic_sha256: str,
    runtime_oscillatory_sha256: str,
) -> None:
    """Validate exact A2 #1010 identity/truth without recomputing A2 work."""
    if not isinstance(configuration, Mapping):
        raise ValueError("A2 #1010 configuration must be a mapping")

    parent = configuration.get("parent_agent2")
    if not isinstance(parent, Mapping):
        raise ValueError("A2 #1010 parent identity is missing")
    if parent.get("pr") != AGENT2_COMPOSITE_PARENT_PR:
        raise ValueError("A2 #1010 parent PR identity drifted")
    if parent.get("head") != AGENT2_COMPOSITE_PARENT_HEAD:
        raise ValueError("A2 #1010 parent head identity drifted")
    if parent.get("source_blob_sha1") != AGENT2_COMPOSITE_PARENT_SOURCE_BLOB:
        raise ValueError("A2 #1010 parent source identity drifted")

    a1 = configuration.get("agent1_leading")
    if not isinstance(a1, Mapping):
        raise ValueError("A2 #1010 Agent-1 leading identity is missing")
    if a1.get("pr") != AGENT1_LEADING_PR or a1.get("head") != AGENT1_LEADING_HEAD:
        raise ValueError("A2 #1010 Agent-1 leading lineage drifted")
    if a1.get("source_blob_sha1") != AGENT1_LEADING_SOURCE_BLOB:
        raise ValueError("A2 #1010 Agent-1 leading source identity drifted")
    if not _valid_sha256(a1.get("semantic_sha256")):
        raise ValueError("A2 #1010 Agent-1 semantic identity is malformed")
    if not _valid_sha256(a1.get("configuration_sha256")):
        raise ValueError("A2 #1010 Agent-1 configuration identity is malformed")

    oscillatory = configuration.get("oscillatory_runtime")
    if not isinstance(oscillatory, Mapping):
        raise ValueError("A2 #1010 oscillatory runtime identity is missing")
    payload_sha = oscillatory.get("payload_sha256")
    if not _valid_sha256(payload_sha):
        raise ValueError("A2 #1010 oscillatory runtime SHA is malformed")
    if payload_sha != runtime_oscillatory_sha256:
        raise ValueError("A2 #1010 oscillatory runtime SHA disagrees with runtime")

    if not _valid_sha256(runtime_semantic_sha256):
        raise ValueError("A2 #1010 composite semantic identity is malformed")

    if configuration.get("source_provenance") != _expected_source_provenance():
        raise ValueError("A2 #1010 corrected-source provenance drifted")

    truth = configuration.get("truth_boundary")
    if not isinstance(truth, Mapping):
        raise ValueError("A2 #1010 truth boundary is missing")
    required_true = (
        "current_leading_plus_oscillatory_velocity_through_RF40_power_law_materialized",
        "RF40_power_law_leading_plus_oscillatory_materialized",
        "full_concrete_oscillatory_runtime_digest_bound",
        "identity_preserving_RF40_power_law_composite_save_load_available",
    ...
    for key in required_true:
        if truth.get(key) is not True:
            raise ValueError(f"A2 #1010 truth boundary drifted at {key}")
...
class ExactCurrentRF40PowerLawNonlinearBackend:
    """Checksum-bound A2 #1010/#960 bridge into the existing A3 mean projector."""
    ...
    def bind(
        ...
        if field_type.__module__ != AGENT2_COMPOSITE_MODULE:
            raise ValueError("unexpected A2 #1010 composite module identity")
        if field_type.__name__ != AGENT2_COMPOSITE_CLASS:
            raise ValueError("unexpected A2 #1010 composite class identity")
        if not callable(getattr(composite_field, "velocity", None)):
            raise TypeError("A2 #1010 composite must expose velocity(x,y,z,t)")
        if not callable(getattr(composite_field, "configuration", None)):
            raise TypeError("A2 #1010 composite must expose configuration()")

        leading = getattr(composite_field, "leading_backend", None)
        if leading is None or not callable(getattr(leading, "velocity", None)):
            raise TypeError("A2 #1010 composite must retain its authenticated leading backend")

        field_source = inspect.getsourcefile(field_type)
        if field_source is None:
            raise ValueError("A2 #1010 composite source is unavailable")
        field_blob = _git_blob_sha1(field_source)
        if field_blob != AGENT2_COMPOSITE_SOURCE_BLOB:
            raise ValueError("A2 #1010 composite source blob drifted")
...
def materialize_current_rf40_power_law_nonlinear_mean_attribution(
    backend: ExactCurrentRF40PowerLawNonlinearBackend,
    radius: Any,
    z: Any,
    t: Any,
) -> OscillatoryNonlinearMeanAttributionWitness:
    """Project exact through-X4 nonlinear pieces; no surrogate defect is accepted."""
    ...
    if backend.composite_source_blob != AGENT2_COMPOSITE_SOURCE_BLOB:
        raise ValueError("A2 #1010 composite provenance drifted")
    ...
    return _materialize_from_provider(
        ...
        t,
        backend=backend,
        backend_kind="exact-current-a2-pr1010-through-rf40-power-law-nonlinear-attribution",
    )
