...
def run():
    x=np.random.default_rng(9172610).uniform(-2,2,(2048,3));rows=[];info={}
    for name in ('bipolar_joint_baseline96','bipolar_joint_odd3'):
    ...
    report=dict(seed=9172610,points=2048,candidates=info,rows=rows,pde_validated=False,scope='Fresh uniform box sample. Theta residual is pressure-independent for axisymmetric pressure. Full vector uses p=0 and is not minimized over pressure; do not call it a pressure-independent obstruction.')
    p=Path('artifacts/bipolar_joint_comparison/report.json');p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(report,indent=2)+'\n')
