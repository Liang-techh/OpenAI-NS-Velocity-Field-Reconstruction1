...
def _held_out_points(correction: KokunoCompleteCurlCorrection) -> tuple[np.ndarray, dict[str, slice]]:
    """Return deterministic points not used by the correction construction.

    Strata deliberately include ordinary interior points, support-collar points,
    axis-near points, and near-corner points.  The correction is Cartesian, but
    the axis-near stratum is retained because the eventual composite candidate
    is expected to be evaluated against an axisymmetric leading field.
    """
    ...
    corner = (
        rng.uniform(0.75, 0.9, (64, 3))
        * widths
        * rng.choice((-1.0, 1.0), (64, 3))
    )
...
def _curl_from_public_potential(
...
def _divergence_from_public_velocity(
    correction: KokunoCompleteCurlCorrection,
    points: np.ndarray,
    time: float,
    step: float,
    *,
    inject_divergence: float = 0.0,
) -> np.ndarray:
...
def run_independent_preflight(
    correction: KokunoCompleteCurlCorrection | None = None,
) -> dict[str, object]:
    """Run the held-out fourth-order structural preflight.

    Passing this function means only that the Agent-2 oscillatory correction is
    independently resolved as the curl of its public potential and numerically
    divergence-free on the declared samples.  It cannot set ``pde_validated``.
    """
    ...
    for time in TIMES:
        ...
        for step in STEPS:
            ...
            mutated = _divergence_from_public_velocity(
                correction,
                points,
                time,
                step,
                inject_divergence=MUTATION_DIVERGENCE,
            )
            ...
            row = {
                "time": float(time),
                "step": float(step),
                "curl_error_max": curl_norms["max"],
                "curl_error_rms": curl_norms["rms"],
                "divergence_max": div_norms["max"],
                "divergence_rms": div_norms["rms"],
                "mutation_divergence_max": mutation_norms["max"],
                "mutation_divergence_rms": mutation_norms["rms"],
            }
            rows.append(row)
            by_step.append(row)

        for index in range(2):
            by_step[index]["curl_rms_order_to_next"] = _order(
                by_step[index]["curl_error_rms"], by_step[index + 1]["curl_error_rms"]
            )
            by_step[index]["divergence_rms_order_to_next"] = _order(
                by_step[index]["divergence_rms"], by_step[index + 1]["divergence_rms"]
            )

        finest = by_step[-1]
        fine_curl = _curl_from_public_potential(correction, points, time, STEPS[-1]) - _velocity(
            correction, points, time
        )
        fine_divergence = _divergence_from_public_velocity(
            correction, points, time, STEPS[-1]
        )
        fine_regions[str(time)] = {
            name: {
                "curl_error": _norms(fine_curl[region]),
                "divergence": _norms(fine_divergence[region]),
            }
    ...
    outside_support_max = max(
        float(np.max(np.abs(_velocity(correction, outside_points, time)))) for time in TIMES
    )
    ...
    return {
        ...
        "registered_full_ns_gate": {
            ...
            "assessed": False,
            "reason": (
                "No complete Kokuno leading+oscillatory(+correction) 3-D velocity with "
                "pressure and fixed/restricted forcing contract is available in this ancestry."
            ),
        },
        "pde_validated": False,
        "truth_boundary": {
