"""Independent A4 audit of A1 #1196's post-swirl ``l=-1`` hold.
...
TASK = "K4-VAL-124"
SCHEMA = "kokuno-a4-lminus1-hold-independent-audit-v1"
UPSTREAM_PR = 1196
UPSTREAM_HEAD = "e0cf691884fd371c477de5ae55a18cdabbd72e81"
SEED = 9173951
TIMES = (0.31, 0.47, 0.63, 0.71)
RELATIVE_FD_LADDER = (2.0e-6, 1.0e-6, 5.0e-7)
CANONICAL_ABSOLUTE_FD_LADDER = (0.02, 0.01, 0.005)
INTERIOR_RHO_BANDS = ((0.06, 0.18), (0.39, 0.58), (0.78, 0.92))
SEAM_RHO = (0.005, 0.015)
LATE_RHO = (0.96, 0.985, 0.997)
...
_TRUTH_BOUNDARY = {
    "lminus1_hold_divergence_independently_audited": True,
    "canonical_absolute_fd_transfer_ready": False,
    "leading_only_ns_residual_assessed": False,
    "leading_plus_oscillatory_ns_residual_assessed": False,
    "after_correction_ns_residual_assessed": False,
    "minus1_to_minush_transition_materialized": False,
...
def default_candidate() -> KokunoPA16CurrentCartesianPostSwirlLMinus1Hold:
    return KokunoPA16CurrentCartesianPostSwirlLMinus1Hold()
...
def _point(
    candidate: Any, rho: float, theta: float, t: float, role: str
) -> dict[str, Any]:
    if not (0.0 < rho < 1.0):
        raise ValueError("held-out hold coordinate must be strict-interior")
    q = _q_at_z0(candidate, t)
    s = float(rho) * float(candidate.hold_length)
    lx = float(candidate.log_X_hold_start + s)
...
def _heldout_points(candidate: Any) -> list[dict[str, Any]]:
    ...
    for t in TIMES:
        for bi, (lo, hi) in enumerate(INTERIOR_RHO_BANDS, 1):
            for k in range(2):
                out.append(
                    _point(
                        candidate,
        ...
        for k, rho in enumerate(SEAM_RHO):
            out.append(
                _point(
                    candidate,
                    rho,
                    ...
                    t,
                    f"hold_entry_seam_{k}",
                )
            )
        for k, rho in enumerate(LATE_RHO):
            out.append(
                _point(
                    candidate,
                    rho,
...
def _family(role: str) -> str:
    if role.startswith("hold_entry"):
        return "seam"
    if role.startswith("late_hold"):
        return "late"
...
def _seam(candidate: Any) -> dict[str, Any]:
    ...
    for t in TIMES:
        q = _q_at_z0(candidate, t)
        r = math.exp(0.5 * (math.log(2.0) + math.log(q) + candidate.log_X_hold_start))
...
def _stage_axis(candidate: Any) -> dict[str, Any]:
    ...
    before = after = False
    try:
        candidate.profile_logX(candidate.log_X_hold_start - 1.0e-3, 0.0)
    except ValueError:
        before = True
    try:
        candidate.profile_logX(candidate.log_X_hold_end + 1.0e-3, 0.0)
    except ValueError:
        after = True
    return {
        "exact_axis_transverse_velocity_zero": bool(all(axis)),
        "before_hold_profile_fails_closed": before,
        "after_hold_profile_fails_closed": after,
    }
...
def _assess(report: Mapping[str, Any]) -> list[str]:
    ...
    if not report["hold_entry_seam_replay"]["all_exact"]:
        fail.append("hold_entry_seam")
    fw = report["axis_and_stage_firewall"]
    if not fw["exact_axis_transverse_velocity_zero"]:
        fail.append("axis_regular")
    if not fw["before_hold_profile_fails_closed"] or not fw["after_hold_profile_fails_closed"]:
...
def audit_loaded_lminus1_hold(
    loaded: Any, pre_serialization_reference: Any
) -> dict[str, Any]:
    """Run frozen K4-VAL-124; no caller scientific tuning knobs."""
    if not isinstance(loaded, KokunoPA16CurrentCartesianPostSwirlLMinus1Hold):
        raise TypeError("loaded candidate must be exact #1196 l=-1 hold type")
    if not isinstance(
        pre_serialization_reference, KokunoPA16CurrentCartesianPostSwirlLMinus1Hold
    ):
        raise TypeError("reference must be exact #1196 l=-1 hold type")
    ...
    late_speeds = [
        speed for speed, p in zip(speeds, points) if str(p["role"]).startswith("late_hold")
    ]
    report: dict[str, Any] = {
        "schema": SCHEMA,
        "task": TASK,
        ...
        "seed": SEED,
        "times": list(TIMES),
        "hold_length": float(loaded.hold_length),
        "h_current": float(loaded.h_value),
        "interior_rho_bands": [list(x) for x in INTERIOR_RHO_BANDS],
        "seam_rho": list(SEAM_RHO),
    ...
    report["scoped_lminus1_hold_divergence_gate_passed"] = len(failures) == 0
    ...
    report["final_project_blockers"] = [
        "minus1_to_minush_transition_missing",
...
def enforce_preregistered_gates(report: Mapping[str, Any]) -> None:
    failures = _assess(report)
    if failures:
        raise AssertionError("K4-VAL-124 scoped gate failure: " + ", ".join(failures))
    ...
    if report.get("final_project_admission_ready") is not False:
        raise AssertionError("scoped l=-1 hold audit cannot promote final admission")


def public_api_has_no_scientific_tuning_knobs() -> bool:
    params = set(inspect.signature(audit_loaded_lminus1_hold).parameters)
    forbidden = {
        "step",
        "h",
        "threshold",
