...
def run(output='artifacts/constrained/poloidal_joint',anchor_core=False):
    old=LocalPressureCandidate.load('artifacts/constrained/local_pressure/candidate.json')
    zero=PoloidalCandidate(old.base,old.pressure_coefficients,anchor_core=anchor_core)
    ...
    probe=np.linspace(-.03,.03,27)
    direct=training_residual(PoloidalCandidate(old.base,old.pressure_coefficients,tuple(probe),anchor_core),old.force,x,t,.01)
    ...
    def raw(v):return evaluate(v[:27])+G@(v[27:]-p0)
    def fun(v):
        nonlocal calls
        calls+=1;R=raw(v);norm=np.linalg.norm(R,axis=1)
        e=energy(v[:27]);violation=np.r_[np.maximum(.1005-e,0),np.maximum(e-9.999,0)]
    ...
    def jac(v):
        R=raw(v);J=np.concatenate((evaluate.jacobian(v[:27]),G),axis=-1)
        ...
        e=energy(v[:27]);de=np.column_stack((energy.jacobian(v[:27]),np.zeros((9,27))))
        penalty=np.vstack((-de*(e<.1005)[:,None],de*(e>9.999)[:,None]))
        return np.vstack((J.reshape(-1,54)/np.sqrt(len(x)),100*penalty))
    fit=least_squares(fun,np.r_[np.zeros(27),p0],jac=jac,bounds=(-np.ones(54),np.ones(54)),max_nfev=80,ftol=1e-8)
    if 'v' not in best:raise RuntimeError('no energy-feasible iterate')
    v=best['v'];candidate=PoloidalCandidate(old.base,tuple(v[27:]),tuple(v[:27]),anchor_core)
    out=Path(output);out.mkdir(parents=True,exist_ok=True);candidate.save(out/'candidate.json')
    report={'anchor_core':anchor_core,'status':'joint_fit_not_validated','force':asdict(old.force),'calls':calls,'max_nfev':80,'termination':fit.message,
            'initial_artifact':'artifacts/constrained/local_pressure/candidate.json','seed':20260916,'grid':[48,48,9],
            'training_points':len(x),'loss':'mean fourth power full residual','training_loss':best['loss'],'history':history,
            'coefficient_bounds':[-1,1],'velocity_coefficients':27,'pressure_coefficients':27,'cache_direct_discrepancy':discrepancy}
