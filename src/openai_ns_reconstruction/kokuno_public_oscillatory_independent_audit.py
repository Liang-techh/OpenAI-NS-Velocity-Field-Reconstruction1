"""Independent Agent-4 black-box audit of the public Kokuno oscillatory provider.

The primary oracle consumes only the public ``velocity_osc(x,y,z,t)`` interface
introduced by Agent 2.  Spatial derivatives are reconstructed with an
independent Cartesian FD4 operator on held-out off-grid points; no complete-curl,
phase-frame, mass, training-loss, pressure, or forcing helper is used as an
oracle.  A separate public-parameter finite difference checks whether the two
signed covariance target directions remain independent after the full public
physical pipeline.

This is not a full Navier--Stokes validation because a global matched leading
velocity/pressure plus materialized correction is still unavailable.  In
particular, no pressure or forcing is fitted here.  The formal project gates
remain frozen at normalized momentum max/L2 <= 1e-3 and divergence max/L2 <=
1e-5; this module reports only the oscillatory-component preflight needed before
that composite gate can be run.
"""
...
from .kokuno_public_candidate_velocity import (
...
# Frozen before exact-head Actions.  These are local oscillatory preflight
# thresholds except for the explicit project support compatibility check.
GUARDS = {
...
def _vector_rms(values: np.ndarray) -> float:
    values = np.asarray(values)
    return float(np.sqrt(np.mean(np.sum(np.abs(values) ** 2, axis=-1))))
...
def _fd4_jacobian(
    ...
    """Return du_i/dx_j using a Cartesian centered fourth-order stencil."""
    ...
    for axis in range(3):
        ...
        deriv = (-f_p2 + 8.0 * f_p1 - 8.0 * f_m1 + f_m2) / (12.0 * step)
        jac[:, :, axis] = deriv
    return jac
...
def _divergence_metrics(jac: np.ndarray) -> dict[str, float]:
    ...
    scale_rms = max(_rms(point_grad), 1.0e-300)
    scale_max = max(float(np.max(point_grad, initial=0.0)), 1.0e-300)
    return {
        "absolute_rms": _rms(div),
        "absolute_max": float(np.max(np.abs(div), initial=0.0)),
        "relative_rms": _rms(div) / scale_rms,
        "relative_max": float(np.max(np.abs(div), initial=0.0)) / scale_max,
        "gradient_rms": scale_rms,
...
def _cylindrical_covariance(
    ...
    *,
    ...
    vel = np.asarray(field.velocity_osc(x, y, np.full_like(theta, z), np.full_like(theta, time)), dtype=float)
...
def _covariance_jacobian(resolution: int, delta: float = 1.0e-4) -> np.ndarray:
    ...
    for key in ("normal_target_ratio", "cross_target_ratio"):
        ...
        c_plus = _cylindrical_covariance(
            KokunoPublicCandidateOscillatoryVelocity(**plus),
            radius=0.73,
            z=0.17,
            time=0.43,
        ...
        c_minus = _cylindrical_covariance(
            KokunoPublicCandidateOscillatoryVelocity(**minus),
            radius=0.73,
            z=0.17,
            time=0.43,
            resolution=resolution,
...
def _support_metrics() -> dict[str, float]:
    # Axis-near and radial exterior checks use only the public default provider.
    ...
    # Project support is registered as |z|<2.  Keep R strictly inside the
    # public annulus so any nonzero value is an axial-support leak, not a radial
    # edge ambiguity.
...
def run_audit() -> dict[str, Any]:
    ...
    for step in FD4_STEPS:
        ...
        row = {"step": float(step), **_divergence_metrics(jac)}
        div_rows.append(row)
        jac_finest = jac
    ...
    # Calibrate the independent divergence detector with an external mutation
    # u_x -> u_x + lambda*x.  This does not modify or reuse candidate internals.
    lam = max(float(div_rows[-1]["gradient_rms"]), 1.0) * 0.25
    div_finest = np.trace(jac_finest, axis1=1, axis2=2)
    mutated_div = div_finest + lam
    ...
    # Public-parameter perturbations are deliberately outside the generator's
    # hidden state: they instantiate the same documented public class and show
    # that h remains an effective, non-collapsed candidate coordinate.
    perturb_points = points[:12]
    perturb_times = times[:12]
    low = KokunoPublicCandidateOscillatoryVelocity(h=0.0045).velocity_osc(
        perturb_points[:, 0], perturb_points[:, 1], perturb_points[:, 2], perturb_times
    )
    high = KokunoPublicCandidateOscillatoryVelocity(h=0.0055).velocity_osc(
        perturb_points[:, 0], perturb_points[:, 1], perturb_points[:, 2], perturb_times
    )
    ref = base_vel[:12]
    parameter_changes = {
    ...
    mutation_passed = bool(
        mutation_relative_rms >= GUARDS["minimum_divergence_mutation_relative_rms"]
    )
    ...
    return {
        ...
        "nontriviality": {
            ...
            "passed": nontriviality_passed,
        },
        "parameter_perturbation": {
            **parameter_changes,
            "passed": parameter_perturbation_passed,
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    # A scientific rejection is a valid validator result; software/runtime
    # failures still raise above and fail CI.
    return 0
