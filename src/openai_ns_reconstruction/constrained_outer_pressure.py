...
def run():
    c=AngularMomentumCandidate.load('artifacts/constrained/outer_shape/candidate.json')
    ...
    fit=lsq_linear(A,-r.ravel(),bounds=(-100,100),tol=1e-10,max_iter=200)
    if not fit.success:raise RuntimeError(fit.message)
    c.pressure_coefficients=tuple(fit.x)
    out=Path('artifacts/constrained/outer_pressure');out.mkdir(exist_ok=True);c.save(out/'candidate.json')
    after=training_residual(c,c.force,x,t,.01,h)
    report={'status':'pressure_fit_only_not_validated','force':{'a':c.force.a,'c':c.force.c},
        'seed':20260916,'basis':'B(r²/4)B(z²/4)(r²/4)^i(z²/4)^j[2(t-.25)]^k; i,j=0..2,k=0..1',
