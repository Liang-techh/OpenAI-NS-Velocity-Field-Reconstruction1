...
def run(multistart=False, odd_extension=False, training_order=None, output=None, momentum=False, energy_cap=None):
    ...
    if odd_extension:
        old=b; modes_new=[(i,j) for i in range(old.radial_degree+1) for j in range(4)]
        phi=np.zeros(len(modes_new));swirl=np.zeros(len(modes_new))
        for i,mode in enumerate(old.mode_indices):
            k=modes_new.index(mode);phi[k]=old.phi_coefficients[i];swirl[k]=old.swirl_coefficients[i]
        b=replace(old,eta_degree=3,phi_coefficients=tuple(phi),swirl_coefficients=tuple(swirl))
    modes=[('phi',i) for i,(radial,j) in enumerate(b.mode_indices) if j%2==1 and (not odd_extension or j==1 or (radial,j)==(0,3))]+[('swirl',i) for i,(_,j) in enumerate(b.mode_indices) if j%2==0]
    ...
    trials=[]
    theta_loss=lambda a:0.
    replay_error=None
    if momentum:
        from .constrained_bipolar_theta_cache import build_theta_objective
        theta_loss,replay_error=build_theta_objective(field,len(modes),times)
    if multistart:
...
if __name__=='__main__':
    import sys
    run(multistart='--multistart' in sys.argv,odd_extension='--odd-extension' in sys.argv)
