"""Independent staged Cartesian divergence audit for A2 #992 through RF40 first turn.

The scientific path save/reloads the identity-bound A2 composite and then calls only
public ``velocity(x,y,z,t)``. For staged attribution it reconstructs exact A1 #986
from the leading configuration stored in the saved artifact and again calls only
public Cartesian velocity. No production Jacobian/divergence, source-coordinate
radial derivative, primitive/schedule derivative, pressure, forcing, training tensor,
or candidate-side residual routine is used by the independent numerical operator.

This audit is deliberately scoped. It covers a nontrivial inner shell, fresh points
strictly in ``X_R < X < X_1=e X_R``, both sides of the ``X_R`` seam, and Cartesian
axis/near-axis regression probes. It does not assess the full Navier--Stokes momentum
residual and cannot promote canonical whole-domain or PDE-valid state.
"""
...
from . import kokuno_current_rf40_first_turn_leading_oscillatory_identity as _upstream

SCHEMA = "kokuno-a4-current-rf40-first-turn-leading-oscillatory-divergence-audit-v1"
UPSTREAM_PR = 992
UPSTREAM_HEAD = "2a2fad307a674ec1eb5a2bc426c0d01c3a0e7377"
UPSTREAM_SOURCE_BLOB = "c126dace161e5dae1745185c79152736817994b8"
AGENT1_PR = 986
AGENT1_HEAD = "23c00b98526e187ff04d432745300a084ec859f2"
AGENT1_SOURCE_BLOB = "bf71948cbdb827d791b86ab8f00496bd1b231961"
PARENT_A4_PR = 989
PARENT_A4_HEAD = "84b8aab2f5a56dbfc8ef02b6053d19bea910a959"
...
POST_XR_ZONES = (
    ("post_xr_early", 0.08, 0.28),
    ("post_xr_middle", 0.38, 0.62),
    ("post_xr_late", 0.72, 0.92),
)
POST_XR_POINTS_PER_ZONE_TIME = 6
XR_SEAM_OFFSETS = (-0.06, -0.02, 0.02, 0.06)
...
TRUTH_BOUNDARY = {
    "current_leading_plus_oscillatory_velocity_through_RF40_first_turn_consumed": True,
    "candidate_save_reload_required": True,
    "independent_cartesian_fd_operator_used": True,
    "post_XR_RF40_first_turn_scoped_divergence_assessed": True,
    ...
    "RF40_axial_shutdown_composite_materialized": False,
...
def make_probes(field: Any) -> ProbeSet:
    ...
    for time_id, time in enumerate(TIMES):
        for zone_id, (_, s0, s1) in enumerate(POST_XR_ZONES):
            n = POST_XR_POINTS_PER_ZONE_TIME
            s = s0 + (s1 - s0) * _latin_hypercube(rng, n)
            X = float(field.X_R) * np.exp(s)
            if not np.all((X > float(field.X_R)) & (X < float(field.X_1))):
                raise RuntimeError("post-XR sampler escaped the first RF40 turn")
            ...
            # Sampling is uniform in s=log(X/X_R), eta and theta, hence dX=X ds.
            pw.append(
                (s1 - s0)
                * (eta1 - eta0)
                * 2.0
            ...
            pt.append(tv)
            pg.append(np.full(n, time_id * len(POST_XR_ZONES) + zone_id, dtype=int))

    seam, seam_t = [], []
    for time_id, time in enumerate(TIMES):
        for j, offset in enumerate(XR_SEAM_OFFSETS):
            X = float(field.X_R) * math.exp(offset)
    ...
    return ProbeSet(
        ...
        ProbeStratum(np.concatenate(pp), np.concatenate(pt), np.concatenate(pw), np.concatenate(pg)),
...
def _stats(J: np.ndarray, weights: np.ndarray | None = None) -> dict[str, Any]:
    ...
    if weights is not None:
        ...
        if not (volume > 0.0 and np.all(w > 0.0)):
...
def metrics_for_step(total: Any, leading: Any, probes: ProbeSet, step: float) -> dict[str, Any]:
    ...
    post, jp = _staged(total, leading, probes.post_xr.points, probes.post_xr.times, step, probes.post_xr.weights)
    ...
    wp = {stage: int(post[stage]["worst_index"]) for stage in ("total", "leading", "oscillatory_increment")}
    for group in (inner, post, seam, axis):
        for stage in ("total", "leading", "oscillatory_increment"):
            group[stage].pop("worst_index", None)

    labels = [f"t={time:.2f}/{zone[0]}" for time in TIMES for zone in POST_XR_ZONES]
    post_by_time_zone = {
        stage: _grouped_stats(J, probes.post_xr.weights, probes.post_xr.group_index, labels)
        for stage, J in zip(("total", "leading", "oscillatory_increment"), jp)
    }
    ...
    return {
        "step": float(step),
        "inner": inner,
        "post_xr_first_turn": post,
        "xr_seam": seam,
        "axis_axis_near": axis,
        "post_xr_by_time_zone": post_by_time_zone,
        "worst_inner_witnesses": {
            stage: witness(probes.inner.points, probes.inner.times, wi[stage], inner[stage])
            for stage in wi
        },
        "worst_post_xr_witnesses": {
            stage: witness(probes.post_xr.points, probes.post_xr.times, wp[stage], post[stage])
            for stage in wp
        },
    }
...
def _metric_keys():
    for region in ("inner", "post_xr_first_turn"):
        for stage in ("total", "leading", "oscillatory_increment"):
            for name in ("sampled_max", "weighted_rms"):
                yield region, stage, name
    for region in ("xr_seam", "axis_axis_near"):
        for stage in ("total", "leading", "oscillatory_increment"):
...
def _post_x1_fail_closed(field: Any) -> bool:
    p = _source_xyz(field, [float(field.X_1) * 1.001], [0.2], [0.7], [0.5])[0]
    try:
...
def _osc_mutation_rejected(saved: Mapping[str, Any]) -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            _upstream.CurrentRF40FirstTurnLeadingOscillatoryField.load_candidate(path)
        except ValueError:
            return True
    return False
...
def materialize_receipt() -> dict[str, Any]:
    ...
    if source_path is None:
        raise RuntimeError("cannot locate A2 #992 source")
    source_blob = _git_blob_sha1(source_path)

    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "candidate.json"
        saved = original.save_candidate(path)
        field = _upstream.CurrentRF40FirstTurnLeadingOscillatoryField.load_candidate(path)
        leading = _load_leading(field)
        probes = make_probes(field)

        replay_p = np.concatenate((probes.inner.points[:8], probes.post_xr.points[:8]))
        replay_t = np.concatenate((probes.inner.times[:8], probes.post_xr.times[:8]))
        replay = float(
        ...
        rp = np.arange(len(probes.post_xr.points) - 1, -1, -1)
        reverse = ProbeSet(
            ProbeStratum(
                probes.inner.points[ri], probes.inner.times[ri], probes.inner.weights[ri], probes.inner.group_index[ri]
            ),
            ProbeStratum(
                probes.post_xr.points[rp], probes.post_xr.times[rp], probes.post_xr.weights[rp], probes.post_xr.group_index[rp]
            ),
        ...
        order_ok = (
            ...
            and abs(
                rev["post_xr_first_turn"]["total"]["weighted_rms"]
                - fine["post_xr_first_turn"]["total"]["weighted_rms"]
            ) <= ORDER_ATOL
            and abs(
        ...
        mutation_detected = max(
            ...
            mut["post_xr_first_turn"]["total"]["sampled_max"],
        ...
        checks = {
            ...
            "post_X1_fail_closed": _post_x1_fail_closed(field),
        }
        ...
        receipt = {
            ...
            "protocol": {
                ...
                "post_xr_zones": [list(v) for v in POST_XR_ZONES],
                "post_xr_points_per_zone_time": POST_XR_POINTS_PER_ZONE_TIME,
                "xr_seam_offsets": list(XR_SEAM_OFFSETS),
                "inner_probe_count": len(probes.inner.points),
                "post_xr_probe_count": len(probes.post_xr.points),
                ...
                "operator": (
                    ...
                    "public A1 #986 leading velocity only"
                ),
                ...
                "scope_note": (
                    "scoped inner plus strictly post-XR first-turn physical-volume estimates; "
            ...
            "mutation": {
                ...
                "finest_post_xr_total_sampled_max": mut["post_xr_first_turn"]["total"]["sampled_max"],
            },
            "checks": checks,
...
def enforce_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if not derived:
        ...
        raise RuntimeError(
            "RF40-first-turn composite divergence audit failed: "
            f"inner_total_max={fine['inner']['total']['sampled_max']:.6e}, "
            f"inner_total_rms={fine['inner']['total']['weighted_rms']:.6e}, "
            f"post_xr_total_max={fine['post_xr_first_turn']['total']['sampled_max']:.6e}, "
            f"post_xr_total_rms={fine['post_xr_first_turn']['total']['weighted_rms']:.6e}, "
            f"post_xr_lead_max={fine['post_xr_first_turn']['leading']['sampled_max']:.6e}, "
            f"seam_total={fine['xr_seam']['total']['sampled_max']:.6e}, "
...
def _main() -> None:
    ...
    if args.verify_receipt is not None:
        ...
        print("A4 RF40-first-turn leading+oscillatory divergence receipt passes frozen gates")
        return
