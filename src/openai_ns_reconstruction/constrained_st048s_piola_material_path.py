"""Replay frozen material paths on Agent-7's ST048-S Piola diagnostic crossing.

This module is a visualization-side governance experiment only.  It reconstructs
frozen ST048-S from PR #390, applies the single divergence-preserving axial
Piola coordinate degree screened by Agent 7 in PR #407 at beta=.075, restores
E(.25)=1 with the same positive common normalization rule, and measures the
result under the exact CR-A9-040/042/043/044 material-path contract.

Nothing here promotes the warped field to a production candidate or transfers
its parent's pressure/forcing/PDE receipt.
"""
...
TASK_ID = "CR-A9-046"
SCHEMA = "st048s_piola_material_path_v1"
...
AGENT7_SOURCE_PR = 407
AGENT7_SOURCE_HEAD = "2d7cb2fe05696c6303e09ad20b91c61aad5f977e"
AGENT7_TASK_ID = "CR003-ST048S-PIOLA-WARP-SCREEN-061"
PIOLA_BETA = 0.075
AGENT7_REPORTED_SCALE_APPROX = 0.9992827694
PHYSICAL_Z_CUT = 2.0
...
def _warp_z_and_jacobian(z, beta: float = PIOLA_BETA):
    beta = float(beta)
...
class PiolaWarpVelocity:
    """Contravariant Piola pullback followed by a positive common scale."""

    def __init__(self, parent: VelocityCallable, *, beta: float, scale: float):
        ...
        self.scale = scale
        _warp_z_and_jacobian(np.array([0.0]), self.beta)
...
def normalized_piola_velocity(parent: VelocityCallable) -> tuple[PiolaWarpVelocity, dict]:
    raw = PiolaWarpVelocity(parent, beta=PIOLA_BETA, scale=1.0)
    ...
    warped = PiolaWarpVelocity(parent, beta=PIOLA_BETA, scale=scale)
    ...
    return warped, {
        ...
        "agent7_reported_scale_approx": AGENT7_REPORTED_SCALE_APPROX,
        "scale_minus_agent7_approx": scale - AGENT7_REPORTED_SCALE_APPROX,
    }
...
def _comparison(summary: dict, reference: dict) -> dict:
    return {
        ...
        "inward_path_count_delta": int(summary["inward_path_count"] - reference.get("inward_path_count", summary["inward_path_count"])),
...
def build_report(candidate_root: str | Path) -> dict:
    ...
    parent_measurement = measure_material_paths(parent)
    warped, normalization = normalized_piola_velocity(parent)
    warped_measurement = measure_material_paths(warped)
    report = {
        "schema": SCHEMA,
        "task_id": TASK_ID,
        ...
        "agent7_piola_source": {
            ...
            "migration_scope": "single beta=.075 Piola z-coordinate map and positive E(.25)=1 common normalization",
        ...
        "measurements": {
            "reconstructed_parent_st048s": parent_measurement,
            "piola_beta_0075": warped_measurement,
        },
        "comparisons": {
            "warped_vs_same_run_parent": _comparison(warped_measurement, {
                "mean_radius_change": parent_measurement["mean_radius_change"],
                "mean_absolute_turns": parent_measurement["mean_absolute_turns"],
                "maximum_absolute_turns": parent_measurement["maximum_absolute_turns"],
                "mean_pair_separation_change": parent_measurement["mean_pair_axial_separation_change"],
                "mean_pair_separation_ratio": parent_measurement["mean_pair_axial_separation_ratio"],
                "inward_path_count": parent_measurement["inward_path_count"],
                "pair_growth_count": parent_measurement["pair_axial_separation_growth_count"],
                "pair_shrink_count": parent_measurement["pair_axial_separation_shrink_count"],
            }),
            "warped_vs_frozen_st048s_receipt": _comparison(warped_measurement, ST048S_REFERENCE),
            "warped_vs_st006": _comparison(warped_measurement, ST006_REFERENCE),
        },
        "references": {
            "st048s": {"source_head": ST048S_REFERENCE_HEAD, "source_report_sha256": ST048S_REFERENCE_REPORT_SHA256, "measurement": ST048S_REFERENCE},
            "st006": {"source_head": ST006_REFERENCE_HEAD, "source_report_sha256": ST006_REFERENCE_REPORT_SHA256, "measurement": ST006_REFERENCE},
        },
        "truth_boundary": dict(TRUTH_BOUNDARY),
    }
    # Independent reconstruction should reproduce the frozen parent receipt to display precision.
    report["parent_reference_replay"] = {
        "mean_radius_change_abs_error": abs(parent_measurement["mean_radius_change"] - ST048S_REFERENCE["mean_radius_change"]),
        "mean_absolute_turns_abs_error": abs(parent_measurement["mean_absolute_turns"] - ST048S_REFERENCE["mean_absolute_turns"]),
        "maximum_absolute_turns_abs_error": abs(parent_measurement["maximum_absolute_turns"] - ST048S_REFERENCE["maximum_absolute_turns"]),
        "mean_pair_separation_change_abs_error": abs(parent_measurement["mean_pair_axial_separation_change"] - ST048S_REFERENCE["mean_pair_separation_change"]),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = report["measurements"]["piola_beta_0075"]
    print(json.dumps({
        "report_sha256": report["report_sha256"],
        "normalization_scale": report["normalization"]["normalization_scale"],
