"""Current-I1 nonlinear mean attribution for the exact A1/A2 candidate identity.

This Kokuno Agent-3 increment consumes Agent-2 PR #1062

    u_current_I1 = u_lead,A1#1051 + u_osc,frozen-complete-curl
...
then reuses the repository cylindrical m=0 projector.  It does not reproduce
Agent-2 curl/Jacobian machinery, perform a radial inverse, materialize pressure
or forcing, define a complete Navier--Stokes defect, construct a correction
velocity, or run a finite correction cycle.
"""
...
PARENT_AGENT3_PR = 1054
PARENT_AGENT3_HEAD = "8f2f485e2000955bab47fc08f478b512d3b49e5d"

AGENT2_COMPOSITE_PR = 1062
AGENT2_COMPOSITE_HEAD = "109527f520abb29bbe10372b0517eda44bcad0b6"
AGENT2_COMPOSITE_MODULE = "openai_ns_reconstruction.kokuno_current_i1_leading_oscillatory_identity"
AGENT2_COMPOSITE_CLASS = "CurrentI1LeadingOscillatoryField"
AGENT2_COMPOSITE_SOURCE_BLOB = "3a8db3e12f57050fd6579b36b9e20ce0e6b95af7"

AGENT2_COMPOSITE_PARENT_PR = 1052
AGENT2_COMPOSITE_PARENT_HEAD = "d58cee2bf38bdca8da13a796215a079a569ad7d7"
AGENT2_COMPOSITE_PARENT_MODULE = "openai_ns_reconstruction.kokuno_source_zero_data_amplitude_sensitivity"
AGENT2_COMPOSITE_PARENT_SOURCE_BLOB = "eba3c00703bd5763b12e6f48a8c34eb918dec82e"
...
AGENT1_LEADING_PR = 1051
AGENT1_LEADING_HEAD = "ea59dc305b4265fcb0bb0f948c2a45f4477d1ae5"
AGENT1_LEADING_MODULE = "openai_ns_reconstruction.kokuno_pa16_current_cartesian_i1_frozen_gate"
AGENT1_LEADING_CLASS = "KokunoPA16CurrentCartesianI1FrozenGate"
AGENT1_LEADING_SOURCE_BLOB = "fe14dab8dccdbca828d722bdfe2f75f287aa9ea8"
...
def _validate_current_i1_configuration(
    configuration: Mapping[str, Any],
    *,
    runtime_semantic_sha256: str,
    runtime_oscillatory_sha256: str,
) -> None:
    """Fail closed unless the runtime is the exact A2 #1062 / A1 #1051 identity."""
    if not isinstance(configuration, Mapping):
        raise ValueError("A2 #1062 configuration must be a mapping")
    if configuration.get("schema") != "kokuno-a2-current-i1-leading-oscillatory-identity-v1":
        raise ValueError("A2 #1062 configuration schema drifted")
    if configuration.get("task") != "K2-OSC-095":
        raise ValueError("A2 #1062 task identity drifted")

    parent = configuration.get("parent_agent2")
    if not isinstance(parent, Mapping):
        raise ValueError("A2 #1062 parent identity is missing")
    expected_parent = {
    ...
    for key, expected in expected_parent.items():
        if parent.get(key) != expected:
            raise ValueError(f"A2 #1062 parent identity drifted at {key}")

    leading = configuration.get("agent1_leading")
    if not isinstance(leading, Mapping):
        raise ValueError("A2 #1062 Agent-1 identity is missing")
    expected_leading = {
    ...
    for key, expected in expected_leading.items():
        if leading.get(key) != expected:
            raise ValueError(f"A2 #1062 Agent-1 identity drifted at {key}")
    if not _valid_sha256(leading.get("semantic_sha256")):
        raise ValueError("A2 #1062 Agent-1 semantic identity is malformed")
    if not _valid_sha256(leading.get("configuration_sha256")):
        raise ValueError("A2 #1062 Agent-1 configuration identity is malformed")

    oscillatory = configuration.get("oscillatory_runtime")
    if not isinstance(oscillatory, Mapping):
        raise ValueError("A2 #1062 oscillatory runtime identity is missing")
    payload_sha = oscillatory.get("payload_sha256")
    if not _valid_sha256(payload_sha):
        raise ValueError("A2 #1062 oscillatory runtime SHA is malformed")
    if payload_sha != runtime_oscillatory_sha256:
        raise ValueError("A2 #1062 oscillatory runtime SHA disagrees with runtime")

    if not _valid_sha256(runtime_semantic_sha256):
        raise ValueError("A2 #1062 composite semantic identity is malformed")
    if configuration.get("source_provenance") != _expected_source_provenance():
        raise ValueError("A2 #1062 corrected-source provenance drifted")
    if configuration.get("composition") != "u_current_I1 = u_lead_A1_1051 + u_osc_frozen_complete_curl":
        raise ValueError("A2 #1062 composition contract drifted")

    truth = configuration.get("truth_boundary")
    if not isinstance(truth, Mapping):
        raise ValueError("A2 #1062 truth boundary is missing")
    required_true = (
        "current_modulated_leading_through_I1_consumed",
        "current_I1_leading_plus_frozen_complete_curl_oscillation_materialized",
        "full_concrete_oscillatory_runtime_digest_bound",
        "identity_preserving_current_I1_composite_save_load_available",
    ...
    for key in required_true:
        if truth.get(key) is not True:
            raise ValueError(f"A2 #1062 truth boundary drifted at {key}")
...
class ExactCurrentI1NonlinearBackend:
    """Checksum-bound A2 #1062/#960 bridge into the existing A3 mean projector."""
    ...
    def bind(
        ...
        _validate_current_i1_configuration(
...
def materialize_current_i1_nonlinear_mean_attribution(
    backend: ExactCurrentI1NonlinearBackend,
    radius: Any,
    z: Any,
    t: Any,
) -> OscillatoryNonlinearMeanAttributionWitness:
    """Project exact current-I1 nonlinear pieces; no surrogate defect is accepted."""
    if not isinstance(backend, ExactCurrentI1NonlinearBackend):
        raise TypeError("backend must be ExactCurrentI1NonlinearBackend")
    if backend.composite_source_blob != AGENT2_COMPOSITE_SOURCE_BLOB:
        raise ValueError("A2 #1062 composite provenance drifted")
    ...
    return _materialize_from_provider(
        ...
        t,
        backend=backend,
        backend_kind="exact-current-a2-pr1062-through-i1-nonlinear-attribution",
    )


def truth_boundary() -> dict[str, object]:
    signature = inspect.signature(materialize_current_i1_nonlinear_mean_attribution)
    forbidden = {
    ...
    return {
        ...
        "current_I1_leading_plus_oscillatory_identity_consumed": True,
        "current_I1_mixed_nonlinear_mean_materialized": True,
        "current_I1_quadratic_oscillatory_mean_materialized": True,
        "current_I1_aggregate_nonlinear_mean_materialized": True,
        "agent2_oscillatory_curl_or_jacobian_reimplemented": False,
        "current_I2_overlay_consumed": False,
