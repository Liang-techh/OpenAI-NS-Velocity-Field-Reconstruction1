"""Independent divergence audit for exact A1 #1148 pre-relative-swirl prefix.

The scientific path consumes only the save/reloaded public Cartesian
``velocity(x,y,z,t)`` candidate and reconstructs spatial derivatives with an
A4-owned centered FD2 operator at the preregistered absolute steps.  The A1
analytic/source-coordinate derivative surfaces and the separately exposed
full-hold *unedited* low-dimensional helper are deliberately excluded.
...
TASK = "K4-VAL-119"
SCHEMA = "kokuno-a4-postpulse-hold-prefix-leading-divergence-audit-v1"
UPSTREAM_PR = 1148
UPSTREAM_HEAD = "5bafa4199ef127bf50bce30a58cc69ded99e1aa9"
PARENT_PR = 1140
PARENT_HEAD = "c5442c11165d7f0976889b826bf58dce38a3d3dd"
SEED = 9173911
SPATIAL_STEPS = (0.02, 0.01, 0.005)
TIMES = (0.31, 0.47, 0.63, 0.71)
PREFIX_RHO_BANDS = ((0.08, 0.24), (0.38, 0.58), (0.72, 0.88))
LATE_RHO = (0.92, 0.96, 0.985)
...
_TRUTH_BOUNDARY = {
    "exact_a1_1148_pre_relative_swirl_prefix_consumed": True,
    "candidate_save_reload_required": True,
    "public_cartesian_velocity_only_for_scientific_derivatives": True,
    "full_hold_unedited_helper_excluded_from_scientific_derivatives": True,
    ...
    "matching_prefix_oscillatory_composite_materialized": False,
    "current_lineage_correction_velocity_materialized": False,
    "relative_swirl_global_leading_completion_materialized": False,
...
def default_field() -> KokunoPA16CurrentCartesianPostPulseEtaIndependentHoldPrefix:
...
def _region(
    ...
    ur, ue, ut = _lhs(rng, n), _lhs(rng, n), _lhs(rng, n)
    rho = rho_lo + (rho_hi - rho_lo) * ur
    log_X = field.log_X_flatten_end + field.prefix_length * rho
    ...
    dlogX = field.prefix_length * (rho_hi - rho_lo)
...
def make_heldout_probes(field: Any) -> HeldoutProbeSet:
    ...
    for tid, time in enumerate(TIMES):
        for zid, (lo, hi) in enumerate(PREFIX_RHO_BANDS):
            blocks.append(
                _region(
                    field,
                    rng,
                    time,
                    tid,
                    f"pre_relative_swirl_prefix_zone_{zid}",
                    lo,
                    hi,
                )
            )
    ...
    for time in (0.47, 0.63):
        for eta0 in (-0.22, 0.22):
            offsets = np.asarray((-0.08, -0.02, 0.02, 0.08), dtype=float)
            log_X = field.log_X_flatten_end + offsets
            eta = np.full(4, eta0)
    ...
    late_rho = np.asarray(LATE_RHO, dtype=float)
    late_log_X = field.log_X_flatten_end + field.prefix_length * late_rho
...
def fd_representability(points: np.ndarray, step: float) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    for axis in range(3):
        ...
        ratio = np.divide(
            spacing, h, out=np.zeros_like(spacing), where=np.isfinite(spacing)
        )
        if len(ratio):
        ...
        per_axis.append(
            {"axis": axis, "nonrepresentable_count": int(np.count_nonzero(~changed))}
        )
    return ok, {
        "step": h,
        "probe_count": len(p),
...
def _stable_weighted_metrics(values: np.ndarray, log_weights: np.ndarray) -> dict[str, Any]:
    ...
    sumsq = float(np.sum(weights * values * values))
    ...
    return {
        ...
        "volume_l2_estimate": 0.0
        if sumsq == 0.0
        else (math.exp(log_l2) if log_l2 < _LOG_MAX else None),
        "log_volume_l2_estimate": log_l2,
        "weighted_rms": rms,
    }


def _subset_divergence(
    field: Any, points: np.ndarray, times: np.ndarray, step: float
) -> tuple[np.ndarray, dict[str, Any]]:
    mask, rep = fd_representability(points, step)
    if not np.any(mask):
        return np.empty(0), rep
    jac = independent_fd2_jacobian(
        field, np.asarray(points)[mask], np.asarray(times)[mask], step
    )
    return np.trace(jac, axis1=1, axis2=2), rep
...
def _metrics(field: Any, probes: HeldoutProbeSet, step: float) -> dict[str, Any]:
    ...
    valid_indices = np.flatnonzero(representable)
    if len(valid_indices):
        valid_points = probes.points[valid_indices]
        valid_times = probes.times[valid_indices]
        jac = independent_fd2_jacobian(field, valid_points, valid_times, step)
        div = np.trace(jac, axis1=1, axis2=2)
        frob = np.linalg.norm(jac, axis=(1, 2))
        normed = np.abs(div) / np.maximum(1.0, frob)
        weighted = _stable_weighted_metrics(div, probes.log_weights[valid_indices])
        norm_weighted = _stable_weighted_metrics(
            normed, probes.log_weights[valid_indices]
        )
        ...
        worst = {
            ...
            "region": str(probes.region[valid_indices[iw]]),
    ...
    for label in sorted(set(map(str, probes.region.tolist()))):
        chosen = [
            j for j, idx in enumerate(valid_indices) if str(probes.region[idx]) == label
        ]
        item = {
            "probe_count": int(np.count_nonzero(probes.region == label)),
            "representable_count": len(chosen),
        }
        if chosen:
            choice = np.asarray(chosen, int)
            vals = div[choice]
            idxs = valid_indices[choice]
            item["sampled_max"] = float(np.max(np.abs(vals)))
            item.update(_stable_weighted_metrics(vals, probes.log_weights[idxs]))
        else:
            item.update(
                sampled_max=None,
                weighted_rms=None,
    ...
    for tid, time in enumerate(TIMES):
        chosen = [
            j for j, idx in enumerate(valid_indices) if int(probes.time_index[idx]) == tid
        ]
        item = {
            "time": time,
            "probe_count": int(np.count_nonzero(probes.time_index == tid)),
            "representable_count": len(chosen),
        }
        if chosen:
            choice = np.asarray(chosen, int)
    ...
    seam_div, seam_rep = _subset_divergence(
        field, probes.seam_points, probes.seam_times, step
    )
    axis_div, axis_rep = _subset_divergence(
        field, probes.axis_points, probes.axis_times, step
    )
    late_div, late_rep = _subset_divergence(
        field, probes.late_points, probes.late_times, step
    )
    return {
        ...
        "prefix_entry_seam_fd_representability": seam_rep,
        "axis_fd_representability": axis_rep,
        "late_prefix_fd_representability": late_rep,
        "prefix_entry_seam_sampled_max": float(np.max(np.abs(seam_div)))
        if seam_div.size
        else None,
        "axis_axis_near_sampled_max": float(np.max(np.abs(axis_div)))
        if axis_div.size
        else None,
        "late_prefix_sampled_max": float(np.max(np.abs(late_div)))
        if late_div.size
        else None,
...
class _ManufacturedSolenoidalField:
    def velocity(self, x, y, z, t):
        ...
        return np.stack(
            ((1.0 + t) * y, -(1.0 + t) * x, np.zeros_like(z)), axis=-1
        )


def _manufactured_calibration() -> dict[str, Any]:
    points = np.asarray(
        ((0.2, -0.3, 0.1), (-0.4, 0.1, -0.2), (0.6, 0.2, 0.3)), dtype=float
    )
    times = np.asarray((0.31, 0.47, 0.71), dtype=float)
    maxima = []
    for h in SPATIAL_STEPS:
        jac = independent_fd2_jacobian(
            _ManufacturedSolenoidalField(), points, times, h
        )
        maxima.append(float(np.max(np.abs(np.trace(jac, axis1=1, axis2=2)))))
    return {
        "max_abs_divergence_by_step": maxima,
        "passed": bool(max(maxima) <= 1.0e-12),
    }


def _mutation_detection() -> dict[str, Any]:
    mutated = _XLinearMutation(_ManufacturedSolenoidalField())
    points = np.asarray(
        ((0.2, -0.3, 0.1), (-0.4, 0.1, -0.2), (0.6, 0.2, 0.3)), dtype=float
    )
    times = np.asarray((0.31, 0.47, 0.71), dtype=float)
    ...
    return {
        "minimum_abs_detected_divergence": signal,
        "passed": bool(signal >= MUTATION_DETECTION_FLOOR),
    }


def _configuration_mutation_rejected(saved: Mapping[str, Any]) -> bool:
    raw = copy.deepcopy(dict(saved))
    raw["bound_scope"]["pre_bump_guard_role"] = "residual_tuned"
    try:
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHoldPrefix.from_configuration(raw)
    except (ValueError, RuntimeError):
        return True
    return False


def _prefix_length_mutation_rejected(saved: Mapping[str, Any]) -> bool:
    raw = copy.deepcopy(dict(saved))
    raw["bound_scope"]["candidate_prefix_length"] = (
        float(raw["bound_scope"]["candidate_prefix_length"]) * 1.001
    )
    try:
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHoldPrefix.from_configuration(raw)
    except (ValueError, RuntimeError):
...
def _post_stage_fails_closed(field: Any) -> bool:
    try:
        field.similarity_profile_values_logX(field.log_X_prefix_end + 0.02, 0.0)
    except (ValueError, RuntimeError):
        return True
    return False
...
def _all_fd_representable(metrics: Mapping[str, Any]) -> bool:
    return all(
        ...
        for key in (
            ...
            "prefix_entry_seam_fd_representability",
            "axis_fd_representability",
            "late_prefix_fd_representability",
        )
    )
...
def _order_invariance(field: Any, probes: HeldoutProbeSet) -> bool:
    ...
    d0 = np.trace(
        independent_fd2_jacobian(field, p, tt, h), axis1=1, axis2=2
    )
    d1 = np.trace(
        independent_fd2_jacobian(field, p[::-1], tt[::-1], h), axis1=1, axis2=2
    )[::-1]
...
def materialize_receipt() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        field = KokunoPA16CurrentCartesianPostPulseEtaIndependentHoldPrefix.load_configuration(
            path
        )
        ...
        replay_error = float(
            np.max(
                np.abs(
                    _velocity(original, replay_points, replay_times)
                    - _velocity(field, replay_points, replay_times)
                )
            )
        )
        guard_mutation_rejected = _configuration_mutation_rejected(saved)
        length_mutation_rejected = _prefix_length_mutation_rejected(saved)
    ...
    gates = {
        ...
        "fine_weighted_rms_pass": _numeric_leq(
            fine["pooled_weighted_rms"], DIVERGENCE_GATE
        ),
        "prefix_entry_seam_pass": _numeric_leq(
            fine["prefix_entry_seam_sampled_max"], DIVERGENCE_GATE
        ),
        "axis_near_pass": _numeric_leq(
            fine["axis_axis_near_sampled_max"], DIVERGENCE_GATE
        ),
        "late_prefix_pass": _numeric_leq(
            fine["late_prefix_sampled_max"], DIVERGENCE_GATE
        ),
        "nontrivial_speed_pass": fine["speed_rms"] is not None
        and float(fine["speed_rms"]) >= NONTRIVIAL_SPEED_RMS,
        "medium_to_fine_sampled_max_stable": _stability_ok(
            medium["sampled_max"], fine["sampled_max"]
        ),
        "medium_to_fine_weighted_rms_stable": _stability_ok(
            medium["pooled_weighted_rms"], fine["pooled_weighted_rms"]
        ),
        "all_canonical_fd_perturbations_representable": all(
            _all_fd_representable(levels[f"h={h:g}"]) for h in SPATIAL_STEPS
        ),
        ...
        "pre_bump_guard_mutation_rejected": guard_mutation_rejected,
        "prefix_length_mutation_rejected": length_mutation_rejected,
    ...
    return {
        ...
        "times": list(TIMES),
        "prefix_rho_bands": [list(v) for v in PREFIX_RHO_BANDS],
        "late_rho": list(LATE_RHO),
        "hold_length_public_formula": field.hold_length,
        "candidate_prefix_length": field.prefix_length,
        ...
        "probe_counts": {
            ...
            "late_prefix": len(probes.late_points),
        },
        ...
        "limitations": [
            ...
            "the full-hold unedited helper is intentionally excluded because it is not the complete source field in the later relative-swirl neighborhoods",
            "A2 has no matching self-contained composite on exact A1 #1148",
            "A3 has no same-identity Cartesian correction velocity on this prefix candidate",
            "relative-swirl/global leading, matched pressure, preregistered restricted forcing, and whole-domain admission remain absent",
...
def enforce_preregistered_gates(report: Mapping[str, Any]) -> None:
    ...
    required = (
        ...
        "pre_bump_guard_mutation_rejected",
        "prefix_length_mutation_rejected",
    ...
    if (
        float(gates.get("final_project_momentum_gate_unchanged", math.nan))
        != FINAL_PROJECT_MOMENTUM_GATE
    ):
        failed.append("momentum_gate_mutated")
    if (
        float(gates.get("final_project_divergence_gate_unchanged", math.nan))
