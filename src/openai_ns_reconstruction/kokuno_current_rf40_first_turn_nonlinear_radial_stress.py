"""Route current RF40-first-turn nonlinear m=0 means through compact radial stress.
...
for the identity-bound Agent-2 PR #992 composite through the first RF40 turn
``X_1 = e X_R``.  It then reuses the already-reviewed Agent-3 compact
...
TASK = "KOKUNO-A3-CURRENT-RF40-FIRST-TURN-NONLINEAR-RADIAL-STRESS-107"
SCHEMA = "kokuno-a3-current-rf40-first-turn-nonlinear-radial-stress-v1"

PARENT_AGENT3_PR = 994
PARENT_AGENT3_HEAD = "6088e3d3055e2792ba20fbd703fcc238ce1b27b5"
PARENT_AGENT3_SOURCE_BLOB = "0f0e1f5c31cf33e28fb6f2851c2df7250bc582d1"
...
def _authenticate_parent_source() -> str:
    ...
    if source is None:
        raise RuntimeError("RF40-first-turn nonlinear mean source is unavailable")
    blob = _git_blob_sha1(Path(source))
    if blob != PARENT_AGENT3_SOURCE_BLOB:
        raise RuntimeError("A3 #994 RF40-first-turn nonlinear mean source blob drifted")
    return blob


@dataclass(frozen=True)
class CurrentRF40FirstTurnNonlinearRadialStressWitness:
    """Current through-X1 mean attribution plus inherited compact radial stresses."""
    ...
    def to_receipt(self) -> dict[str, object]:
        ...
        return {
            ...
            "task": TASK,
            "backend_kind": (
                "exact-current-a3-pr994-mean-to-inherited-compact-radial-stress"
            ),
            "geometry": base["geometry"],
            "current_rf40_first_turn_mean_witness": base["mean_witness"],
            ...
            "parent_agent3_994_source_blob": self.parent_source_blob,
            "provenance": {
...
def _materialize_current_mean_witness(
    ...
    *,
    ...
    """Apply only the inherited compact radial operator to an exact #994 mean witness."""
    ...
    if parent_source_blob != PARENT_AGENT3_SOURCE_BLOB:
        raise ValueError("parent source blob does not match exact A3 #994")

    inherited = _materialize_from_mean_witness(
        witness,
        geometry,
        backend_kind=(
            "current-a3-pr994-mean-reusing-a3-pr920-a3-pr875-radial-operator"
        ),
    )
    if inherited.mean_witness is not witness:
        raise RuntimeError("inherited radial routing detached the current #994 mean witness")
    return CurrentRF40FirstTurnNonlinearRadialStressWitness(
        inherited=inherited,
        parent_source_blob=parent_source_blob,
    )


def materialize_current_rf40_first_turn_nonlinear_radial_stress(
    backend: ExactCurrentRF40FirstTurnNonlinearBackend,
    geometry: StrictInnerTransportRadialGeometry,
) -> CurrentRF40FirstTurnNonlinearRadialStressWitness:
    """Recompute #994 means and route them through the compact radial inverse."""
    ...
    witness = materialize_current_rf40_first_turn_nonlinear_mean_attribution(
        backend, radii, z, t
    )
    if witness.backend_kind != (
        "exact-current-a2-pr992-through-rf40-first-turn-nonlinear-attribution"
    ):
        raise RuntimeError("RF40-first-turn nonlinear mean backend identity drifted")
...
def truth_boundary() -> dict[str, object]:
    ...
    return {
        ...
        "current_RF40_first_turn_mixed_nonlinear_mean_consumed": True,
        "current_RF40_first_turn_quadratic_nonlinear_mean_consumed": True,
        "current_RF40_first_turn_aggregate_nonlinear_mean_consumed": True,
        "current_RF40_first_turn_nonlinear_radial_inverse_performed": True,
        "current_RF40_first_turn_quadratic_radial_stress_materialized": True,
        "current_RF40_first_turn_mixed_radial_stress_materialized": True,
        "current_RF40_first_turn_aggregate_radial_stress_materialized": True,
        ...
        "scoped_current_RF40_first_turn_radial_stress_authorized_as_correction_target": False,
