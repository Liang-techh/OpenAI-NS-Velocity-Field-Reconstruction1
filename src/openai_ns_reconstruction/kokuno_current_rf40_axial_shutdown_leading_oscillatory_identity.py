"""Identity-bound current Kokuno leading + complete-curl oscillation through RF40 axial shutdown.

Agent 1 PR #993 advances the current Cartesian leading field from the first RF40
turn endpoint ``X_1=e X_R`` through the public axial-shutdown endpoint ``X_2``.
Agent 2 PR #992 already composes the exact preceding leading field through
``X_1`` with the frozen complete-curl oscillation and binds the full concrete
oscillatory runtime to deterministic save/load.

This module makes exactly one further composition step::

    u_current_to_X2(x,t) = u_lead,current-through-X2(x,t)
                           + u_osc,frozen-complete-curl(x,t).

Neither summand is changed.  The exact Agent-1 #993 class/blob is consumed via a
dynamic runtime dependency; the oscillatory field remains the existing A2
axis-safe strict-support runtime.  This adapter additionally binds the public
corrected-reader provenance block into *its own* semantic identity.  That does
not repair or relabel the separately reported upstream #993 internal provenance
semantic-binding limitation.

The corrected 2026-09-09 Kokuno reconstruction is structural provenance for the
localized-wave/complete-curl organization and RF40 axial-shutdown schedule.  The
...
from . import kokuno_current_rf40_first_turn_leading_oscillatory_identity as _first_turn_parent
...
TASK = "K2-OSC-085"
SCHEMA = "kokuno-a2-current-rf40-axial-shutdown-leading-oscillatory-identity-v1"
SERIALIZED_SCHEMA = "kokuno-a2-current-rf40-axial-shutdown-leading-oscillatory-saved-candidate-v1"
RECEIPT_SCHEMA = "kokuno-a2-current-rf40-axial-shutdown-leading-oscillatory-receipt-v1"

PARENT_AGENT2_PR = 992
PARENT_AGENT2_HEAD = "2a2fad307a674ec1eb5a2bc426c0d01c3a0e7377"
PARENT_AGENT2_MODULE = (
    "openai_ns_reconstruction.kokuno_current_rf40_first_turn_leading_oscillatory_identity"
)
PARENT_AGENT2_SOURCE_BLOB_SHA1 = "c126dace161e5dae1745185c79152736817994b8"

AGENT1_PR = 993
AGENT1_HEAD = "2ac6460b483efb1c07f2fa65e7fed781a32f2718"
AGENT1_MODULE = "openai_ns_reconstruction.kokuno_pa16_current_cartesian_rf40_axial_shutdown"
AGENT1_CLASS = "KokunoPA16CurrentCartesianRF40AxialShutdown"
AGENT1_SOURCE_BLOB_SHA1 = "057a0514c8a941c3e59922b8158f480434b4441e"
AGENT1_TEST_BLOB_SHA1 = "b1703748f567d7d7d505bf7f2e07aa287a8c0559"
AGENT1_WORKFLOW_BLOB_SHA1 = "a5f485f89456c7171bf3789d11f677c4d800658a"
...
_AXIAL_STAGE_FRACTIONS = np.asarray((0.18, 0.57, 1.0), dtype=float)
...
def _validate_parent_a2_source() -> str:
    cls = _first_turn_parent.CurrentRF40FirstTurnLeadingOscillatoryField
    if cls.__module__ != PARENT_AGENT2_MODULE:
        raise RuntimeError("Agent-2 #992 parent module identity drifted")
    source_path = inspect.getsourcefile(cls)
    if source_path is None:
        raise RuntimeError("cannot locate Agent-2 #992 parent implementation source")
    blob = _composition._git_blob_sha1(source_path)
    if blob != PARENT_AGENT2_SOURCE_BLOB_SHA1:
        raise RuntimeError("Agent-2 #992 parent implementation source blob drifted")
    return blob
...
def _validate_exact_agent1_backend(backend: Any) -> dict[str, Any]:
    ...
    if cls.__module__ != AGENT1_MODULE or cls.__name__ != AGENT1_CLASS:
        raise RuntimeError("loaded leading backend is not the pinned Agent-1 #993 class")
    source_path = inspect.getsourcefile(cls)
    if source_path is None:
        raise RuntimeError("cannot locate Agent-1 #993 leading implementation source")
    blob = _composition._git_blob_sha1(source_path)
    if blob != AGENT1_SOURCE_BLOB_SHA1:
        raise RuntimeError("Agent-1 #993 leading implementation source blob drifted")

    truth = getattr(backend, "truth_boundary", None)
    if not isinstance(truth, Mapping):
        raise RuntimeError("Agent-1 #993 truth boundary is missing")
    for key, expected in _required_agent1_truth().items():
        if truth.get(key) is not expected:
            raise RuntimeError(f"Agent-1 #993 truth boundary drifted at {key}")

    semantic = str(getattr(backend, "semantic_sha256", ""))
    _require_sha256(semantic, "Agent-1 #993 semantic SHA")
    configuration = backend.configuration()
    if not isinstance(configuration, Mapping):
        raise RuntimeError("Agent-1 #993 configuration is missing")
    ...
    T_d = float(getattr(backend, "T_d", np.nan))
    D = float(getattr(backend, "D", np.nan))
    if not all(np.isfinite(v) for v in (X_R, X_1, X_2, T_d, D)):
        raise RuntimeError("Agent-1 #993 X_R/X_1/X_2/T_d/D geometry is invalid")
    if not (0.0 < X_R < X_1 < X_2 and T_d > 0.0):
        raise RuntimeError("Agent-1 #993 staged radial geometry is invalid")
    if not math.isclose(X_1 / X_R, math.e, rel_tol=2.0e-14, abs_tol=0.0):
        raise RuntimeError("Agent-1 #993 first-turn endpoint identity drifted")
    if not math.isclose(math.log(X_2 / X_1), T_d, rel_tol=2.0e-13, abs_tol=2.0e-13):
        raise RuntimeError("Agent-1 #993 axial-shutdown endpoint identity drifted")
...
def _load_exact_agent1_backend() -> Any:
    try:
        ...
        raise RuntimeError(
            "exact Agent-1 #993 runtime is unavailable; cannot materialize the RF40 axial-shutdown composite"
        ) from exc
    cls = getattr(module, AGENT1_CLASS, None)
    if cls is None:
        raise RuntimeError("exact Agent-1 #993 class is unavailable")
    backend = cls()
...
def _load_exact_agent1_from_configuration(configuration: Mapping[str, Any]) -> Any:
    try:
        ...
        raise RuntimeError(
            "exact Agent-1 #993 runtime is unavailable; cannot reload the RF40 axial-shutdown composite"
        ) from exc
    cls = getattr(module, AGENT1_CLASS, None)
    if cls is None or not callable(getattr(cls, "from_configuration", None)):
        raise RuntimeError("exact Agent-1 #993 class/from_configuration is unavailable")
...
def _truth_boundary() -> dict[str, bool]:
    return {
        ...
        "RF40_axial_shutdown_leading_plus_oscillatory_materialized": True,
...
def _cartesian_probe(
    field: "CurrentRF40AxialShutdownLeadingOscillatoryField",
    X: Any,
    eta: Any,
    t: Any,
    theta: Any,
...
class CurrentRF40AxialShutdownLeadingOscillatoryField:
    """Exact #993 current leading through ``X_2`` plus frozen A2 oscillation."""
    ...
    @property
    def T_d(self) -> float:
        return float(self._leading_identity["T_d"])
    ...
    def configuration(self) -> dict[str, Any]:
        ...
        if not isinstance(leading_configuration, Mapping):
            raise RuntimeError("Agent-1 #993 leading configuration is missing")
        return {
            "schema": SCHEMA,
            "task": TASK,
            "parent_agent2": {
            ...
            "composition": (
                "u_current_to_X2 = u_lead_current_through_RF40_axial_shutdown "
                "+ u_osc_frozen_complete_curl"
            ),
            "source_provenance": _source_provenance(),
            "upstream_identity_note": (
                "A2 pins exact A1 #993 head/source/runtime semantic identity; this does not repair "
                "the separately reported A1 internal external-source semantic-binding limitation"
            ),
    ...
    def from_configuration(
    ...
    ) -> "CurrentRF40AxialShutdownLeadingOscillatoryField":
        if not isinstance(payload, Mapping) or payload.get("schema") != SCHEMA:
            raise ValueError("unexpected RF40-axial-shutdown composite configuration schema")
        if payload.get("task") != TASK:
            raise ValueError("unexpected RF40-axial-shutdown composite task identity")
        if payload.get("source_provenance") != _source_provenance():
            raise ValueError("RF40-axial-shutdown corrected-source provenance drifted")
        if payload.get("truth_boundary") != _truth_boundary():
            raise ValueError("RF40-axial-shutdown truth boundary drifted")

        parent = payload.get("parent_agent2")
        if not isinstance(parent, Mapping):
            raise ValueError("serialized Agent-2 #992 parent identity is missing")
        if (
            ...
            raise ValueError("RF40-axial-shutdown composite parent identity drifted")
        if _validate_parent_a2_source() != PARENT_AGENT2_SOURCE_BLOB_SHA1:
            raise ValueError("current Agent-2 #992 parent source identity drifted")

        leading = payload.get("agent1_leading")
        if not isinstance(leading, Mapping):
            raise ValueError("serialized Agent-1 #993 identity is missing")
        if (
            ...
            raise ValueError("serialized Agent-1 #993 provenance drifted")
        leading_configuration = leading.get("configuration")
        if not isinstance(leading_configuration, Mapping):
            raise ValueError("serialized Agent-1 #993 configuration is missing")
        stored_config_sha = _require_sha256(
            leading.get("configuration_sha256"), "Agent-1 #993 configuration SHA"
        )
        if _identity._sha256(leading_configuration) != stored_config_sha:
            raise ValueError("serialized Agent-1 #993 configuration digest mismatch")
        stored_semantic = _require_sha256(
            leading.get("semantic_sha256"), "Agent-1 #993 semantic SHA"
        )
        ...
        if identity["configuration_sha256"] != stored_config_sha:
            raise ValueError("reloaded Agent-1 #993 configuration identity drifted")
        if identity["semantic_sha256"] != stored_semantic:
            raise ValueError("reloaded Agent-1 #993 semantic identity drifted")
        for key in ("X_R", "X_1", "X_2", "T_d"):
            if not math.isclose(
                float(leading.get(key, np.nan)), float(identity[key]), rel_tol=0.0, abs_tol=0.0
            ):
                raise ValueError(f"reloaded Agent-1 #993 {key} identity drifted")
    ...
    @classmethod
    def load_candidate(cls, path: str | Path) -> "CurrentRF40AxialShutdownLeadingOscillatoryField":
        raw = json.loads(Path(path).read_text(encoding="utf-8"))
        if not isinstance(raw, Mapping) or raw.get("schema") != SERIALIZED_SCHEMA:
            raise ValueError("unexpected saved RF40-axial-shutdown composite candidate schema")
        configuration = raw.get("configuration")
        if not isinstance(configuration, Mapping):
            raise ValueError("saved RF40-axial-shutdown composite configuration is missing")
        stored_semantic = _require_sha256(
            raw.get("semantic_sha256"), "saved RF40-axial-shutdown composite semantic SHA"
        )
        if _identity._sha256(configuration) != stored_semantic:
            raise ValueError("saved RF40-axial-shutdown composite semantic digest mismatch")
        obj = cls.from_configuration(configuration)
        if obj.semantic_sha256 != stored_semantic:
            raise ValueError("reloaded RF40-axial-shutdown composite semantic identity drifted")
        return obj


@lru_cache(maxsize=1)
def default_field() -> CurrentRF40AxialShutdownLeadingOscillatoryField:
    return CurrentRF40AxialShutdownLeadingOscillatoryField(_load_exact_agent1_backend())


def velocity(x: Any, y: Any, z: Any, t: Any) -> np.ndarray:
    """Return current leading through RF40 axial shutdown plus frozen A2 oscillation."""
...
def public_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "leading_materialized_through_RF40_axial_shutdown": True,
        "full_oscillatory_runtime_payload_bound": True,
        "identity_preserving_save_load": True,
        "after_RF40_axial_shutdown_fail_closed": True,
...
def _axial_shutdown_probes(
    field: CurrentRF40AxialShutdownLeadingOscillatoryField,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    X = field.X_1 * np.exp(field.T_d * _AXIAL_STAGE_FRACTIONS)
...
def materialize_rf40_axial_shutdown_composite_receipt() -> dict[str, Any]:
    field = default_field()
    ...
    X_axial, x_axial, y_axial, z_axial, t_axial = _axial_shutdown_probes(field)
    axial = _composition._evaluate_with_backend(
        field.leading_backend, x_axial, y_axial, z_axial, t_axial
    )

    inner_closure = float(np.max(np.abs(inner.velocity - (inner.leading + inner.oscillatory))))
    axial_closure = float(np.max(np.abs(axial.velocity - (axial.leading + axial.oscillatory))))
    inner_osc_rms = float(
        np.sqrt(np.mean(np.sum(inner.oscillatory * inner.oscillatory, axis=-1)))
    )
    axial_osc_max = float(np.max(np.abs(axial.oscillatory)))

    with __import__("tempfile").TemporaryDirectory() as tmp:
        path = Path(tmp) / "candidate.json"
        saved = field.save_candidate(path)
        loaded = CurrentRF40AxialShutdownLeadingOscillatoryField.load_candidate(path)
        replay_points = np.concatenate(
            (
                _INNER_POINTS,
                np.stack((x_axial, y_axial, z_axial), axis=-1),
            ),
            axis=0,
        )
        replay_times = np.concatenate((_INNER_TIMES, t_axial), axis=0)
    ...
    xb, yb, zb, tb = _cartesian_probe(field, field.X_2 * 1.001, 0.2, 0.5, 0.3)
    ...
    return {
        ...
        "diagnostic": {
            ...
            "axial_shutdown_probe_count": int(X_axial.size),
            "axial_shutdown_stage_fraction": [float(v) for v in _AXIAL_STAGE_FRACTIONS],
            "axial_shutdown_X_over_X1": [float(v / field.X_1) for v in X_axial],
            "axial_shutdown_X_over_X2": [float(v / field.X_2) for v in X_axial],
            "inner_composition_closure_max_abs": inner_closure,
            "axial_shutdown_composition_closure_max_abs": axial_closure,
            "inner_oscillatory_vector_rms": inner_osc_rms,
            "axial_shutdown_oscillatory_abs_max": axial_osc_max,
            "velocity_save_load_replay_max_abs": replay_max,
            "beyond_current_X2_fail_closed": beyond_fail_closed,
            "X_R": field.X_R,
            "X_1": field.X_1,
            "X_2": field.X_2,
            "T_d": field.T_d,
        ...
        "frozen_mechanical_gates": {
            ...
            "axial_shutdown_composition_closure_max_abs": COMPOSITION_ATOL,
            "inner_oscillatory_vector_rms_min": OSCILLATORY_SIGNAL_FLOOR,
            "axial_shutdown_oscillatory_abs_max": 0.0,
            "velocity_save_load_replay_max_abs": VELOCITY_REPLAY_ATOL,
            "beyond_current_X2_fail_closed": True,
        },
    }
...
def enforce_receipt(receipt: Mapping[str, Any]) -> None:
    if not isinstance(receipt, Mapping) or receipt.get("schema") != RECEIPT_SCHEMA:
        raise AssertionError("unexpected RF40-axial-shutdown composite receipt schema")
    ...
    if not isinstance(diagnostic, Mapping) or not isinstance(gates, Mapping):
        raise AssertionError("RF40-axial-shutdown composite receipt is incomplete")
    ...
    if float(diagnostic.get("axial_shutdown_composition_closure_max_abs", np.inf)) > COMPOSITION_ATOL:
        raise AssertionError("axial-shutdown leading+oscillatory composition closure failed")
    ...
    if float(diagnostic.get("axial_shutdown_oscillatory_abs_max", np.inf)) != 0.0:
        raise AssertionError("frozen compact oscillation leaked into RF40 axial-shutdown probes")
    if float(diagnostic.get("velocity_save_load_replay_max_abs", np.inf)) > VELOCITY_REPLAY_ATOL:
        raise AssertionError("RF40-axial-shutdown candidate save/load velocity replay failed")
    if diagnostic.get("beyond_current_X2_fail_closed") is not True:
        raise AssertionError("RF40-axial-shutdown composite no longer fails closed after X_2")
    ...
    if semantic != saved_semantic:
        raise AssertionError("RF40-axial-shutdown semantic identity did not survive save/load")
    ...
    if receipt.get("parent_agent2_source_blob_sha1") != PARENT_AGENT2_SOURCE_BLOB_SHA1:
        raise AssertionError("Agent-2 #992 parent source identity drifted")
    ...
    if gates != {
        ...
        raise AssertionError("frozen RF40-axial-shutdown mechanical gates drifted")

    truth = receipt.get("truth_boundary")
    if truth != _truth_boundary():
        raise AssertionError("RF40-axial-shutdown truth boundary drifted or was illegally promoted")
    contract = receipt.get("public_contract")
    if contract != public_contract():
        raise AssertionError("RF40-axial-shutdown public contract drifted")
