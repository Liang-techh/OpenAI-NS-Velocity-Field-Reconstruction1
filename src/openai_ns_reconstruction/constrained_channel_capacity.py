...
def _cutoff(values):
    ...
    flat = np.fromiter(
        (standard_cutoff(float(v)) for v in values.ravel()),
        dtype=float,
        count=values.size,
    )
...
def _cutoff_derivative(values):
    values = np.asarray(values, dtype=float)
    flat = np.fromiter(
        (standard_cutoff_derivative(float(v)) for v in values.ravel()),
        dtype=float,
        count=values.size,
    )
    return flat.reshape(values.shape)
...
def poloidal_curl_probe(points, time):
    ...
    ``q`` is axisymmetric and localized near the current final-time axial peak.
    The returned field has radial/axial components and is exactly a curl at the
    analytic level, so it supplies the missing channel without free forcing.
    """
    value = _points(points)
    x, y, z = np.moveaxis(value, -1, 0)
    r2 = x * x + y * y
