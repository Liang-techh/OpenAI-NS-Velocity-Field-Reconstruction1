...
class CompactCandidate:
    ...
    def __post_init__(self):
        bounds = {
            ...
            'axial_shape': (-0.2, 0.2), 'amplitude': (1e-4, 100),
    ...
    def save(self, path):
        Path(path).write_text(json.dumps({
            ...
            'family': 'compact_axisymmetric_window_v1', 'parameters': asdict(self)
        },indent=2)+'\n',encoding='utf-8')

    @classmethod
    def load(cls, path):
        data=json.loads(Path(path).read_text(encoding='utf-8'))
        if data.get('schema_version') != 1 or data.get('family') != 'compact_axisymmetric_window_v1':
