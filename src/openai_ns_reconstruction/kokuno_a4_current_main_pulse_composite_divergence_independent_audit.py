"""Independent divergence audit for exact Kokuno current-main-pulse composite #1108.
...
from . import kokuno_current_main_pulse_leading_oscillatory_identity as _candidate

CurrentMainPulseLeadingOscillatoryField = _candidate.CurrentMainPulseLeadingOscillatoryField
default_field = _candidate.default_field

SCHEMA = "kokuno-a4-current-main-pulse-composite-divergence-audit-v1"
UPSTREAM_PR = 1108
UPSTREAM_HEAD = "9f221dd57ef4b5e2d2e80e24c8cf531e991a5387"
AGENT1_PR = 1100
AGENT1_HEAD = "2a6c58ff6bf6d1ba6b875bca0e6abf9d1118e156"
...
PULSE_ZONE_FRACTIONS = ((0.12, 0.28), (0.40, 0.56), (0.70, 0.84))
PRE_PULSE_FRACTIONS = (-0.20, -0.05)
...
_TRUTH_BOUNDARY = {
    "current_main_pulse_finite_x_leading_plus_oscillatory_velocity_consumed": True,
    ...
    "newer_agent1_1107_logx_xi11_leading_consumed": False,
...
def _lhs(rng: np.random.Generator, n: int) -> np.ndarray:
    value = (np.arange(n, dtype=float) + rng.random(n)) / n
    rng.shuffle(value)
    return value
...
def make_heldout_probes(field: Any) -> HeldoutProbeSet:
    ...
    Xp = float(field.X_p)
    Xe = float(field.X_materializable_end)
    if not (math.isfinite(Xp) and math.isfinite(Xe) and 0.0 < Xp < Xe):
        raise RuntimeError("invalid main-pulse finite-X interval")
    s0, s1 = math.log(Xp), math.log(Xe)
    span = s1 - s0
    if span <= 0.0:
        raise RuntimeError("invalid main-pulse log-X span")
    ...
    for tid, time in enumerate(TIMES):
        blocks.append(
            _region(
                ...
                tid,
                "pre_pulse_control",
                s0 + PRE_PULSE_FRACTIONS[0] * span,
                s0 + PRE_PULSE_FRACTIONS[1] * span,
            )
        )
        for zid, (a, b) in enumerate(PULSE_ZONE_FRACTIONS):
            blocks.append(
                _region(
                    field,
                    rng,
    ...
    return HeldoutProbeSet(
        ...
        regions,
        tids,
        np.concatenate(seam_points),
        np.concatenate(seam_times),
        _source_to_cartesian(field, axis_X, axis_eta, axis_theta, axis_t),
        axis_t,
    )
...
def _metrics(field: Any, probes: HeldoutProbeSet, h: float) -> dict[str, Any]:
    ...
    return {
        ...
        "normalized_weighted_rms": float(
            np.sqrt(np.sum(probes.weights * normed**2) / volume)
        ),
        "pulse_entry_seam_sampled_max": float(np.max(np.abs(seam))),
        "axis_axis_near_sampled_max": float(np.max(np.abs(axis))),
        "speed_rms": float(np.sqrt(np.mean(speed**2))),
        "per_region": by_region,
        "per_time": by_time,
        "worst_witness": {
            "point": probes.points[iw].tolist(),
            "time": float(probes.times[iw]),
            "region": str(probes.region[iw]),
...
def _fd_perturbation_report(points: np.ndarray, steps: tuple[float, ...] = SPATIAL_STEPS) -> dict[str, Any]:
    p = np.asarray(points, float)
    if p.ndim != 2 or p.shape[1] != 3 or np.any(~np.isfinite(p)):
        raise ValueError("invalid Cartesian points for FD representability audit")
    by_step = []
    all_ok = True
    for step in steps:
        h = float(step)
        bad = 0
        total = p.shape[0] * 3
        min_effective = math.inf
        for axis in range(3):
            center = p[:, axis]
            plus = center + h
            minus = center - h
            changed = (plus != center) & (minus != center) & (plus != minus)
            bad += int(np.count_nonzero(~changed))
            effective = np.minimum(np.abs(plus - center), np.abs(center - minus))
            if effective.size:
                min_effective = min(min_effective, float(np.min(effective)))
        ok = bad == 0
        all_ok = all_ok and ok
        by_step.append({
            "step": h,
            "coordinate_axis_pairs": total,
            "nonrepresentable_coordinate_axis_pairs": bad,
            "min_effective_one_sided_increment": (
                None if not math.isfinite(min_effective) else min_effective
            ),
            "all_coordinate_axis_pairs_representable": ok,
        })
    return {"all_steps_representable": all_ok, "by_step": by_step}
...
def _leading_mutation_rejected(saved: Mapping[str, Any], tmp: Path) -> bool:
    ...
    _save_json(path, raw)
    try:
        CurrentMainPulseLeadingOscillatoryField.load_candidate(path)
...
def _post_materializable_fails_closed(field: Any) -> bool:
    X = 1.0000000001 * float(field.X_materializable_end)
    point = _source_to_cartesian(field, [X], [0.0], [0.71], [0.47])
    try:
...
def _component_order_invariant(a: Mapping[str, Any], b: Mapping[str, Any]) -> bool:
    return all(
        abs(float(a[key]) - float(b[key])) <= ORDER_INVARIANCE_ATOL
...
def _derive_audit_pass(receipt: Mapping[str, Any]) -> bool:
    ...
    return bool(
        ...
        and checks["fd_perturbations_representable_all_axes"]
        ...
        and checks["post_materializable_fails_closed"]
...
def _manufactured_calibration() -> bool:
    ...
    div, _ = _divergence(
        _ManufacturedSolenoidalField(), points, times, SPATIAL_STEPS[-1]
    )
...
def materialize_receipt() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="k4-main-pulse-") as directory:
        tmp = Path(directory)
        path = tmp / "candidate.json"
        saved = original.save_candidate(path)
        field = CurrentMainPulseLeadingOscillatoryField.load_candidate(path)
    ...
    reversed_probes = HeldoutProbeSet(
        probes.points[reverse],
        probes.times[reverse],
        probes.weights[reverse],
        probes.region[reverse],
        probes.time_index[reverse],
        probes.seam_points,
        probes.seam_times,
        probes.axis_points,
        probes.axis_times,
    ...
    order_ok = all(
        _component_order_invariant(resolutions[-1][name], reverse_fine[name])
        for name in ("leading", "leading_plus_oscillatory", "oscillatory_increment")
    )

    base_div, _ = _divergence(
        field, probes.points, probes.times, SPATIAL_STEPS[-1]
    )
    mut_div, _ = _divergence(
        _XLinearMutation(field), probes.points, probes.times, SPATIAL_STEPS[-1]
    )
    mutation_signal = float(np.max(np.abs(mut_div - base_div)))
    representability = _fd_perturbation_report(
        np.concatenate((probes.points, probes.seam_points, probes.axis_points), axis=0)
    )
    ...
    receipt = {
        ...
        "domain": {
            "X_p": float(field.X_p),
            "X_materializable_end": float(field.X_materializable_end),
            "xi_materializable_max": float(field.xi_materializable_max),
            "full_source_xi_11_current_cartesian_materialized": False,
        },
        "protocol": {
            "seed": SEED,
            ...
            "times": list(TIMES),
            "pre_pulse_fractions_of_finite_log_span": list(PRE_PULSE_FRACTIONS),
            "main_pulse_zone_fractions_of_finite_log_span": [
                list(value) for value in PULSE_ZONE_FRACTIONS
            ],
            ...
            "physical_volume_weighting": (
                "independent fixed-time chart Jacobian with dX=X*dlogX"
            ),
        },
        "gates": {
        ...
        "checks": {
            ...
            "velocity_mutation_detected": (
                mutation_signal >= MUTATION_DETECTION_FLOOR
            ),
            "fd_perturbation_representability": representability,
            "fd_perturbations_representable_all_axes": bool(
                representability["all_steps_representable"]
            ),
            "rehash_consistent_leading_mutation_rejected": lead_mut,
            "rehash_consistent_oscillatory_mutation_rejected": osc_mut,
            "post_materializable_fails_closed": _post_materializable_fails_closed(
                field
            ),
        ...
        "limitations": [
            "scoped current-main-pulse finite-X divergence evidence, not complete NS momentum residual",
            "exact consumed composite is A2 #1108 on A1 #1100; newer A1 #1107 xi=11 leading-only sibling is not transferred",
            "source-exact pulse amplitude and pulse-end M/J compensators are not materialized by the consumed candidate",
            "canonical absolute Cartesian FD steps must be representable at every sampled coordinate; otherwise the audit fails closed instead of accepting a zero-by-roundoff derivative",
...
def enforce_preregistered_gates(receipt: Mapping[str, Any]) -> None:
    ...
    if not _derive_audit_pass(receipt):
        ...
        raise AssertionError(
            "preregistered current-main-pulse divergence audit failed: "
            f"max={fine['sampled_max']:.6e}, "
            f"rms={fine['pooled_weighted_rms']:.6e}, "
            f"seam={fine['pulse_entry_seam_sampled_max']:.6e}, "
