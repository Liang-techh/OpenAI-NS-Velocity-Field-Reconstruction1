"""Agent-5 admission refresh for one independently audited ordinary-R1 slot.

This module performs one narrow integration-state promotion: Agent 4 PR #710
completed an exact-head independent audit of Agent 1 PR #707's
``d u zeta_* Phi`` ordinary R1 post-J2 slot.  Only that slot is admitted here.
The other R1 slots, full post-J2 R1, full post-J1 R2, M/K, global leading
fields, correction readiness, export, and PDE validation remain fail-closed.
...
from .kokuno_a5_source_leading_admission_firewall_v2 import (
    FROZEN_EVIDENCE as V2_FROZEN_EVIDENCE,
    derive_state as _derive_state_v2,
)

SCHEMA = "kokuno-a5-source-leading-admission-firewall-v3"
TASK = "KOKUNO-A5-ADMIT-INDEPENDENT-R1-ZETA-SLOT-062"
PARENT_A5_PR = 719
PARENT_A5_HEAD = "e4b6703907efc34984b3e77a77b6235fe226e05b"

A4_ZETA_R1_RECEIPT = {
    "pr": 710,
    "head": "a6616ec2cdd21948b1e45a1cccc27e5cf3206a46",
    "workflow_id": 35471381036,
    "workflow_name": "kokuno-agent4-pa10-source-r1-zeta-independent-audit",
    "workflow_event": "push",
    "workflow_conclusion": "success",
    "artifact_name": "kokuno-agent4-pa10-source-r1-zeta-independent-audit-v1",
    "artifact_id": 10594044299,
    "artifact_digest": "sha256:8ebbc26b1a476fb5d16c024652b33ccbcb167c546ca26f90087b5647ab23d18b",
    "receipt_sha256": "a80b6a6dba517a740f2ee35796196658f707497b05c3038aceab7d8074a2f40e",
}

AGENT1_R1_PROGRESS = {
    "latest_pr": 724,
    "latest_head": "d62461a873936500fcc5b125179f593433295e17",
    "ordinary_r1_source_bound_slots": 8,
    "ordinary_r1_total_slots": 11,
    "ordinary_r1_remaining_slots": 3,
...
ST006_BASELINE = {
    "candidate": "ST006",
    "same_protocol_momentum_sampled_max": 0.1082289305112118,
    "same_protocol_momentum_volume_L2": 0.10758432876230622,
    "normalized_momentum_gate": 1.0e-3,
    "pde_validated": False,
}

FROZEN_EVIDENCE: dict[str, AdmissionEvidence] = dict(V2_FROZEN_EVIDENCE)
FROZEN_EVIDENCE["r1_zeta_ordinary"] = AdmissionEvidence(
    key="r1_zeta_ordinary",
    producer="Agent 4",
    pr=710,
    head=A4_ZETA_R1_RECEIPT["head"],
    workflow_id=A4_ZETA_R1_RECEIPT["workflow_id"],
    workflow_conclusion_at_freeze="success",
    independently_admitted=True,
    scope="Agent-1 PR #707 d u zeta_* Phi ordinary R1 post-J2 slot",
    note=(
        "Exact-head Agent-4 push workflow PASS only for this zeta ordinary-R1 "
        "slot; it does not retroactively admit other R1 slots or full R1."
    ),
)
...
def derive_state(
    ...
    """Extend the v2 fail-closed state with one narrow R1 admission."""

    state = dict(_derive_state_v2(evidence))
    zeta = evidence.get("r1_zeta_ordinary")
    zeta_admitted = bool(zeta is not None and zeta.independently_admitted)
    state.update(
        {
            "source_r1_zeta_ordinary_slot_independently_admitted": zeta_admitted,
            "source_r1_averaged_eta_ordinary_slot_independently_admitted": False,
            "source_r1_wstar_logradial_ordinary_slot_independently_admitted": False,
...
def deterministic_receipt(*, exact_head: str | None = None) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        ...
        "new_independent_receipt": dict(A4_ZETA_R1_RECEIPT),
        "evidence": {
        ...
        "truth_boundary": {
            "single_integration_increment": "admit independently audited R1 zeta ordinary slot",
        ...
        "shortest_legal_next_chain": [
            "resolve Agent-4 #718 averaged-eta and #727 Wstar-logradial exact-head audits",
            "bind Agent-1's remaining 3/11 ordinary R1 slots and independently audit them",
            "obtain independent admission for the earlier ordinary-R1 slots not covered by #710/#718/#727",
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if independent != A4_ZETA_R1_RECEIPT:
        raise RuntimeError("Agent-4 R1-zeta receipt changed")
    if independent["workflow_event"] != "push":
        raise RuntimeError("R1-zeta evidence is not an exact-head push workflow")
    if independent["head"] != "a6616ec2cdd21948b1e45a1cccc27e5cf3206a46":
        raise RuntimeError("unexpected Agent-4 R1-zeta head")
    if independent["workflow_conclusion"] != "success":
        raise RuntimeError("Agent-4 R1-zeta workflow is not successful")

    evidence = receipt["evidence"]["r1_zeta_ordinary"]
    if evidence["producer"] != "Agent 4" or evidence["pr"] != 710:
        raise RuntimeError("R1-zeta evidence is not the frozen Agent-4 audit")
    if evidence["workflow_id"] != 35471381036:
        raise RuntimeError("unexpected R1-zeta workflow id")
    if evidence["workflow_conclusion_at_freeze"] != "success":
        raise RuntimeError("R1-zeta audit was not frozen as success")
    if evidence["independently_admitted"] is not True:
        raise RuntimeError("green R1-zeta audit lost admission")
    ...
    if state["source_r1_zeta_ordinary_slot_independently_admitted"] is not True:
        raise RuntimeError("R1-zeta admission missing")
