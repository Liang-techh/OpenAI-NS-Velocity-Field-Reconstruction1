...
smooth callable parent. PR #499 applies the same map through a trilinear
interpolant of a frozen 33^3 grid, which is a distinct numerical
representation. This module changes no velocity, candidate, pressure, forcing,
route, threshold, or upstream implementation.
"""
...
_PR499_ARTIFACT_DIGEST = "sha256:ec99470af7028e0004877def1065d11e4ee66c7b234c95283b20e923f2235f6d"
...
_UPSTREAM_PIOLA_IDENTITY = "div(u_beta)=h'(z)*(div u)(x,y,h(z),t) for the smooth parent"
_LOCAL_PIOLA_IDENTITY = "div(P_beta[u])=h_z*(div u)(x,y,h_beta(z),t)"
...
def _audit_cr001(contract: Mapping[str, Any], constraints: Mapping[str, Any]) -> None:
    ...
    _equal(frozen["reference_energy_abs_tolerance"], nontrivial["reference_energy_abs_tolerance"], "reference energy tolerance")
...
def audit_sampled_grid_piola_governance(
    ...
    """Audit sampled/interpolated Piola evidence against live delivery contracts."""
    ...
    axial = deepcopy(dict(load_axial_warp_governance() if axial_warp_governance is None else axial_warp_governance))
    grid = deepcopy(dict(load_grid_identity_contract() if grid_identity_contract is None else grid_identity_contract))
    render = deepcopy(dict(load_render_scope_contract() if render_scope_contract is None else render_scope_contract))
    ...
    _equal(continuum["divergence_identity"], _LOCAL_PIOLA_IDENTITY, "local Piola identity")
    ...
    _equal(
        set(sampled_child["identity_depends_on"]),
        {"source_grid_identity", "grid_resolution_and_nodes", "sample_times", "interpolation_kernel_and_boundary_behavior", "beta", "sampled_energy_normalization_rule"},
        "sampled child identity inputs",
    )
    ...
    # Existing contracts establish the smooth-parent identity and separately
    # require a new numerical contract for sampled interpolation. Keep both true.
    _equal(axial["representation"]["divergence_identity"], _UPSTREAM_PIOLA_IDENTITY, "upstream Piola identity")
    _false(axial["representation"]["navier_stokes_invariant_under_warp"], "warp NS invariance")
    _false(grid["identity_layers"]["sampled_grid"]["sampled_grid_is_continuum_velocity_api"], "grid/continuum API distinction")
    _true(grid["identity_layers"]["sampled_grid"]["off_grid_or_unsampled_time_interpolation_requires_explicit_new_numerical_contract"], "interpolation contract requirement")
    ...
    _require(abs(evidence["fine_common_reference_energy_scale"] - evidence["coarse_common_reference_energy_scale"]) > 1e-3, "resolution-specific energy-scale distinction disappeared")
