"""Replay the frozen ST052-M localized taper on Agent-9 material paths.

CR-A9-060 consumes Agent 7 PR #544's exact localized radial-Piola taper on
ST052-M + the frozen kappa=.05 inner/mid swirl redistribution, then compares
that field against the exact redistributed control under Agent 9's unchanged
48-path / 24-pair material-trajectory contract.

This module performs no taper scan, pressure/forcing refit, PDE promotion,
...
TASK_ID = "CR-A9-060"
SCHEMA = "st052m_localized_taper_material_path_v1"
BASE_MAIN_SHA = "f0193d66c9d92948b4820ebcb70263673995b324"

AGENT7_PR = 544
AGENT7_HEAD = "093c7171cd61c6bd439afa30b2da69598a02d182"
AGENT7_TASK_ID = "CR003-ST052M-LOCALIZED-TAPER-080"
...
TAPER_TAU = 0.05
AXIAL_WINDOW = (0.50, 0.82)
EXPECTED_TAPER_SCALE = 0.9929528816
TAPER_SCALE_IDENTITY_TOL = 5.0e-7
...
UPSTREAM_AGENT7_RECEIPT = {
    ...
    "clean_localized_taper_capacity": True,
    "taper_energy_scale": EXPECTED_TAPER_SCALE,
    "tip_radial_rms_relative": [-0.0137325, -0.0149126, -0.0136258],
    "central_radial_rms_relative": [0.0, 0.0, 0.0],
    "global_axial_rms_relative": [0.0097628, 0.0120790, 0.0140494],
    "divergence_fd_max": 2.613e-9,
    "response_rank": 3,
    "response_condition_number": 1.109075,
    "response_max_pairwise_abs_cosine": 0.084803,
    "pde_validated": False,
...
def load_agent7_source(root: str | Path):
    ...
    if _git_head(root) != AGENT7_HEAD:
        raise ValueError("Agent-7 checkout is not pinned PR #544 exact head")
    path = root / "experiments" / "root_st052" / "agent7_st052m_localized_taper.py"
    spec = importlib.util.spec_from_file_location("agent7_st052m_localized_taper", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load frozen Agent-7 localized taper source")
...
def build_report(agent7_root: str | Path, agent9_root: str | Path) -> dict:
    ...
    redistribution_scale, redistribution_parent_energy, redistribution_raw_energy = source.base.child_scale(family, raw)
    taper_scale, control_energy, taper_raw_energy = source.taper_energy_scale(
        family, raw, redistribution_scale
    )
    if not np.isfinite(taper_scale) or taper_scale <= 0.0:
        raise ValueError("localized taper normalization is not finite and positive")
    if abs(taper_scale - EXPECTED_TAPER_SCALE) > TAPER_SCALE_IDENTITY_TOL:
        raise ValueError("Agent-7 localized taper normalization drift")

    def control_velocity(points: np.ndarray, time: float) -> np.ndarray:
        return source.control_velocity(
            family,
            raw,
    ...
    def child_velocity(points: np.ndarray, time: float) -> np.ndarray:
        return source.taper_velocity(
            family,
            raw,
            np.asarray(points, dtype=float),
            float(time),
            redistribution_scale=redistribution_scale,
            tau=source.TAPER_TAU,
    ...
    report = {
        ...
        "frozen_transform": {
            ...
            "redistribution_parent_reference_energy": redistribution_parent_energy,
            "redistribution_unscaled_reference_energy": redistribution_raw_energy,
            "taper_tau": TAPER_TAU,
            "taper_axial_window_abs_z_over_2": list(AXIAL_WINDOW),
            "taper_normalization": taper_scale,
            "control_reference_energy": control_energy,
            "unscaled_taper_reference_energy": taper_raw_energy,
        },
        ...
        "localized_taper_child_material_paths": child,
        "redistributed_control_by_seed_radius": control_by_radius,
        "localized_taper_child_by_seed_radius": child_by_radius,
        ...
        "internal_method": {
            ...
            "migrated_scope": "exact PR #544 localized taper plus exact PR #435 trajectory measurement contract",
            "difference": "Agent 7 #544 assessed Eulerian vorticity morphology, locality and response capacity and explicitly did not integrate material paths; this increment measures the same frozen child on Agent 9's cross-candidate trajectory protocol",
        },
        "truth_boundary": dict(TRUTH_BOUNDARY),
        "direct_contribution_to_final_velocity": (
            "tests whether the localized tip-thinning geometry degree is compatible with real-time inward-spiral trajectories before any pressure/restricted-force rebuild or production materialization"
        ),
    }
