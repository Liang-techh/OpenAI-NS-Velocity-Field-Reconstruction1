"""Independent A4 audit of current RF40 power-law nonlinear radial stress.

Consumes serialized/public receipts from Agent-3 PR #1037 only. The numerical
reference does not call the inherited Agent-3 #920/#875 radial inverse. Instead
it reconstructs compact moment-complement stresses from public cylindrical
...
TASK = "K4-VAL-106"
SCHEMA = "kokuno-a4-current-rf40-power-law-radial-stress-independent-audit-v1"
...
PARENT_SCHEMA = "kokuno-a3-current-rf40-power-law-nonlinear-radial-stress-v1"
PARENT_AGENT3_1028_HEAD = "4b7a4400dcf1ee0be6bb07f23c2f8af6f893de88"
PARENT_AGENT3_1028_SOURCE_BLOB = "4ec98a6adbb7afd16d6595cc24072fe18e62fa87"
AGENT2_1010_HEAD = "e36d9da4b7f037e998f5b1658f8c0ea291a76b80"
AGENT2_1010_SOURCE_BLOB = "4f1dc0566c041b9de20f18b4ae9c49d9fd93c58d"
AGENT2_960_HEAD = "6d2fb1f701a34f783dca15a267ae2ce0734ba741"
AGENT2_960_SOURCE_BLOB = "12df3bf6c949baeebf609b366f2973e7f515a157"
AGENT1_1005_HEAD = "2c76ebdc41d6c566f43a1305034ba2ff9dce410b"
AGENT1_1005_SOURCE_BLOB = "36a0183352a2005bd0b5f477594e05a2e4fe3868"
...
# Preserved unchanged from A4 #1017/#977. These are scoped stress gates.
...
def _extract_sources(receipt: Mapping[str, Any]) -> dict[str, np.ndarray]:
    mean = receipt.get("current_rf40_power_law_mean_witness")
    if not isinstance(mean, Mapping):
        raise ValueError("receipt is missing current_rf40_power_law_mean_witness")
    names = {
...
def _audit_one_receipt(
    ...
    *,
    ...
    if receipt.get("schema") != PARENT_SCHEMA:
        raise ValueError("unexpected A3 #1037 receipt schema")
    if receipt.get("parent_agent3_1028_source_blob") != PARENT_AGENT3_1028_SOURCE_BLOB:
        raise ValueError("A3 #1028 provenance drifted")
    ...
    for key, expected in expected_provenance.items():
        if provenance.get(key) != expected:
            raise ValueError(f"A3 #1037 provenance drifted at {key}")
    ...
    for key in false_keys:
        if boundary.get(key) is not False:
            raise ValueError(f"A3 #1037 scientific boundary drifted at {key}")
...
def enforce_scientific_gates(report: Mapping[str, Any]) -> None:
    ...
    if report.get("parent_agent3_head") != PARENT_AGENT3_HEAD:
        raise RuntimeError("A3 #1037 identity drifted")
    if report.get("parent_agent3_source_blob") != PARENT_AGENT3_SOURCE_BLOB:
        raise RuntimeError("A3 #1037 source identity drifted")
    if report.get("audit_pass") is not True:
        raise RuntimeError(
            f"independent RF40 power-law stress audit failed: {report.get('failures')}"
        )
