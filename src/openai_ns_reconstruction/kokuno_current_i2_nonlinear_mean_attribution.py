"""Current-I2 nonlinear mean attribution for the exact A1/A2 candidate identity.

This Kokuno Agent-3 increment consumes Agent-2 PR #1071

    u_current_I2 = u_lead,A1#1061 + u_osc,frozen-complete-curl
...
PARENT_AGENT3_PR = 1063
PARENT_AGENT3_HEAD = "e6ac8bed0ef775232e18e81e87f6a7572dcba8fb"

AGENT2_COMPOSITE_PR = 1071
AGENT2_COMPOSITE_HEAD = "48d69e37e78e7f7f0e4e9936f28ff7974719288d"
AGENT2_COMPOSITE_MODULE = "openai_ns_reconstruction.kokuno_current_i2_leading_oscillatory_identity"
AGENT2_COMPOSITE_CLASS = "CurrentI2LeadingOscillatoryField"
AGENT2_COMPOSITE_SOURCE_BLOB = "be68248aca97173a15474324f63efff2c9ffce56"

AGENT2_COMPOSITE_PARENT_PR = 1062
AGENT2_COMPOSITE_PARENT_HEAD = "109527f520abb29bbe10372b0517eda44bcad0b6"
AGENT2_COMPOSITE_PARENT_MODULE = "openai_ns_reconstruction.kokuno_current_i1_leading_oscillatory_identity"
AGENT2_COMPOSITE_PARENT_SOURCE_BLOB = "3a8db3e12f57050fd6579b36b9e20ce0e6b95af7"
...
AGENT1_LEADING_MODULE = "openai_ns_reconstruction.kokuno_pa16_current_cartesian_i2_heat_repair"
AGENT1_LEADING_CLASS = "KokunoPA16CurrentCartesianI2HeatRepair"
...
def _validate_current_i2_configuration(
    configuration: Mapping[str, Any],
    *,
    runtime_semantic_sha256: str,
    runtime_oscillatory_sha256: str,
) -> None:
    """Fail closed unless the runtime is exact A2 #1071 / A1 #1061."""
    if not isinstance(configuration, Mapping):
        raise ValueError("A2 #1071 configuration must be a mapping")
    if configuration.get("schema") != "kokuno-a2-current-i2-leading-oscillatory-identity-v1":
        raise ValueError("A2 #1071 configuration schema drifted")
    if configuration.get("task") != "K2-OSC-096":
        raise ValueError("A2 #1071 task identity drifted")

    parent = configuration.get("parent_agent2")
    if not isinstance(parent, Mapping):
        raise ValueError("A2 #1071 parent identity is missing")
    expected_parent = {
    ...
    for key, expected in expected_parent.items():
        if parent.get(key) != expected:
            raise ValueError(f"A2 #1071 parent identity drifted at {key}")

    leading = configuration.get("agent1_leading")
    if not isinstance(leading, Mapping):
        raise ValueError("A2 #1071 Agent-1 identity is missing")
    expected_leading = {
    ...
    for key, expected in expected_leading.items():
        if leading.get(key) != expected:
            raise ValueError(f"A2 #1071 Agent-1 identity drifted at {key}")
    if not _valid_sha256(leading.get("semantic_sha256")):
        raise ValueError("A2 #1071 Agent-1 semantic identity is malformed")
    if not _valid_sha256(leading.get("configuration_sha256")):
        raise ValueError("A2 #1071 Agent-1 configuration identity is malformed")

    oscillatory = configuration.get("oscillatory_runtime")
    if not isinstance(oscillatory, Mapping):
        raise ValueError("A2 #1071 oscillatory runtime identity is missing")
    payload_sha = oscillatory.get("payload_sha256")
    if not _valid_sha256(payload_sha):
        raise ValueError("A2 #1071 oscillatory runtime SHA is malformed")
    if payload_sha != runtime_oscillatory_sha256:
        raise ValueError("A2 #1071 oscillatory runtime SHA disagrees with runtime")

    if not _valid_sha256(runtime_semantic_sha256):
        raise ValueError("A2 #1071 composite semantic identity is malformed")
    if configuration.get("source_provenance") != _expected_source_provenance():
        raise ValueError("A2 #1071 corrected-source provenance drifted")
    if configuration.get("composition") != "u_current_I2 = u_lead_A1_1061 + u_osc_frozen_complete_curl":
        raise ValueError("A2 #1071 composition contract drifted")

    truth = configuration.get("truth_boundary")
    if not isinstance(truth, Mapping):
        raise ValueError("A2 #1071 truth boundary is missing")
    required_true = (
        "current_modulated_leading_through_I2_consumed",
        "current_I2_leading_plus_frozen_complete_curl_oscillation_materialized",
        "full_concrete_oscillatory_runtime_digest_bound",
        "identity_preserving_current_I2_composite_save_load_available",
    ...
    required_false = (
        ...
        "high_precision_I2_nonzero_implies_public_float64_velocity_delta",
        "public_float64_I2_delta_verified",
    ...
    for key in required_true:
        if truth.get(key) is not True:
            raise ValueError(f"A2 #1071 truth boundary drifted at {key}")
...
class ExactCurrentI2NonlinearBackend:
    """Checksum-bound A2 #1071/#960 bridge into the existing A3 mean projector."""
    ...
    def bind(
        ...
        if field_type.__module__ != AGENT2_COMPOSITE_MODULE:
            raise ValueError("unexpected A2 #1071 composite module identity")
        if field_type.__name__ != AGENT2_COMPOSITE_CLASS:
            raise ValueError("unexpected A2 #1071 composite class identity")
        if not callable(getattr(composite_field, "velocity", None)):
            raise TypeError("A2 #1071 composite must expose velocity(x,y,z,t)")
        if not callable(getattr(composite_field, "configuration", None)):
            raise TypeError("A2 #1071 composite must expose configuration()")

        leading = getattr(composite_field, "leading_backend", None)
        if leading is None or not callable(getattr(leading, "velocity", None)):
            raise TypeError("A2 #1071 composite must retain its authenticated leading backend")

        field_source = inspect.getsourcefile(field_type)
        if field_source is None:
            raise ValueError("A2 #1071 composite source is unavailable")
        field_blob = _git_blob_sha1(field_source)
        if field_blob != AGENT2_COMPOSITE_SOURCE_BLOB:
            raise ValueError("A2 #1071 composite source blob drifted")
        ...
        _validate_current_i2_configuration(
...
def materialize_current_i2_nonlinear_mean_attribution(
    backend: ExactCurrentI2NonlinearBackend,
    radius: Any,
    z: Any,
    t: Any,
) -> OscillatoryNonlinearMeanAttributionWitness:
    """Project exact current-I2 nonlinear pieces; no surrogate defect is accepted."""
    ...
    if backend.composite_source_blob != AGENT2_COMPOSITE_SOURCE_BLOB:
        raise ValueError("A2 #1071 composite provenance drifted")
    ...
    return _materialize_from_provider(
        ...
        t,
        backend=backend,
        backend_kind="exact-current-a2-pr1071-through-i2-nonlinear-attribution",
    )


def truth_boundary() -> dict[str, object]:
    signature = inspect.signature(materialize_current_i2_nonlinear_mean_attribution)
    forbidden = {
    ...
    return {
        ...
        "current_I2_leading_plus_oscillatory_identity_consumed": True,
        "current_I2_mixed_nonlinear_mean_materialized": True,
        "current_I2_quadratic_oscillatory_mean_materialized": True,
        "current_I2_aggregate_nonlinear_mean_materialized": True,
