"""Identity-bound current Kokuno I2 leading + frozen complete-curl oscillation.

This Kokuno Agent-2 increment consumes the exact current Agent-1 #1061
Cartesian leading candidate through reserved I2 and composes it with the same
frozen, bounded, axis-safe complete-curl oscillatory runtime used by the
current-I1 composite::

    u_current_I2(x,y,z,t) = u_lead_A1_1061(x,y,z,t) + u_osc_frozen(x,y,z,t).

Neither summand is changed.  Agent-1 remains the sole owner of the leading
profile and I2 heat-repair construction; Agent-2 remains the owner of the
oscillatory complete-curl realization.  The corrected 2026-09-09 Kokuno
reconstruction is structural provenance only.  In particular, the
high-precision I2 repair can be nonzero before the public float64 boundary,
so this adapter does not claim that I2 creates a nonzero public velocity delta.
It is not paper-exact, globally complete, or Navier--Stokes validation.
"""
...
from . import kokuno_current_i1_leading_oscillatory_identity as _parent_a2

TASK = "K2-OSC-096"
SCHEMA = "kokuno-a2-current-i2-leading-oscillatory-identity-v1"
SERIALIZED_SCHEMA = "kokuno-a2-current-i2-leading-oscillatory-saved-candidate-v1"
RECEIPT_SCHEMA = "kokuno-a2-current-i2-leading-oscillatory-receipt-v1"

PARENT_AGENT2_PR = 1062
PARENT_AGENT2_HEAD = "109527f520abb29bbe10372b0517eda44bcad0b6"
PARENT_AGENT2_MODULE = "openai_ns_reconstruction.kokuno_current_i1_leading_oscillatory_identity"
PARENT_AGENT2_SOURCE_BLOB_SHA1 = "3a8db3e12f57050fd6579b36b9e20ce0e6b95af7"

AGENT1_PR = 1061
AGENT1_HEAD = "9c2abbc69ba1ef8c8a9e8d1699a466a75d0293c3"
AGENT1_MODULE = "openai_ns_reconstruction.kokuno_pa16_current_cartesian_i2_heat_repair"
AGENT1_CLASS = "KokunoPA16CurrentCartesianI2HeatRepair"
AGENT1_SOURCE_BLOB_SHA1 = "bfeff163d304c4cf1fb84eb8ff1cdfcf430870b4"
AGENT1_TEST_BLOB_SHA1 = "23d315ac7df6dde287735dd3e1e9e6dbcf5363c6"
AGENT1_WORKFLOW_BLOB_SHA1 = "db55c7198e8a4071edbaf3b41f471ba16633d9e9"
...
_I2_STAGE_FRACTIONS = np.asarray((0.18, 0.57, 0.92), dtype=float)
_I2_ETA = np.asarray((-0.31, 0.08, 0.37), dtype=float)
_I2_TIMES = np.asarray((0.43, 0.55, 0.67), dtype=float)
_I2_THETA = np.asarray((0.23, 0.91, 1.47), dtype=float)
...
def _source_provenance() -> dict[str, Any]:
    return {
        ...
        "scope": "localized waves / complete curls and phase/support organization",
        "paper_exact_claim": False,
    }


def _validate_parent_a2_source() -> str:
    source_path = inspect.getsourcefile(_parent_a2.CurrentI1LeadingOscillatoryField)
    if source_path is None:
        raise RuntimeError("cannot locate exact Agent-2 #1062 parent source")
    blob = _composition._git_blob_sha1(source_path)
    if blob != PARENT_AGENT2_SOURCE_BLOB_SHA1:
        raise RuntimeError("Agent-2 #1062 parent source blob drifted")
    return blob
...
def _validate_exact_agent1_backend(backend: Any) -> dict[str, Any]:
    ...
    if cls.__module__ != AGENT1_MODULE or cls.__name__ != AGENT1_CLASS:
        raise RuntimeError("loaded leading backend is not exact Agent-1 #1061")
    source_path = inspect.getsourcefile(cls)
    if source_path is None:
        raise RuntimeError("cannot locate Agent-1 #1061 source")
    source_blob = _composition._git_blob_sha1(source_path)
    if source_blob != AGENT1_SOURCE_BLOB_SHA1:
        raise RuntimeError("Agent-1 #1061 source blob drifted")

    truth = getattr(backend, "truth_boundary", None)
    if not isinstance(truth, Mapping):
        raise RuntimeError("Agent-1 #1061 truth boundary is missing")
    for key, expected in _required_agent1_truth().items():
        if truth.get(key) is not expected:
            raise RuntimeError(f"Agent-1 #1061 truth boundary drifted at {key}")

    semantic = _require_sha256(str(getattr(backend, "semantic_sha256", "")), "Agent-1 semantic SHA")
    configuration = backend.configuration()
    if not isinstance(configuration, Mapping):
        raise RuntimeError("Agent-1 #1061 configuration is missing")
    configuration_sha = _identity._sha256(configuration)

    d = float(getattr(backend, "D", np.nan))
    x_start = float(getattr(backend, "X_I2_start", np.nan))
    x_end = float(getattr(backend, "X_I2_end", np.nan))
    if not all(np.isfinite(value) for value in (d, x_start, x_end)):
        raise RuntimeError("Agent-1 #1061 I2 geometry is non-finite")
    if not (d > 0.0 and 0.0 < x_start < x_end):
        raise RuntimeError("Agent-1 #1061 I2 geometry is invalid")
...
def _load_exact_agent1_backend() -> Any:
    try:
        ...
        raise RuntimeError(
            "exact Agent-1 #1061 runtime is unavailable; current-I2 composition cannot materialize"
        ) from exc
    cls = getattr(module, AGENT1_CLASS, None)
    if cls is None:
        raise RuntimeError("exact Agent-1 #1061 class is unavailable")
    backend = cls()
...
def _load_exact_agent1_from_configuration(configuration: Mapping[str, Any]) -> Any:
    try:
        ...
        raise RuntimeError(
            "exact Agent-1 #1061 runtime is unavailable; current-I2 candidate cannot reload"
        ) from exc
    cls = getattr(module, AGENT1_CLASS, None)
    if cls is None or not callable(getattr(cls, "from_configuration", None)):
        raise RuntimeError("exact Agent-1 #1061 from_configuration is unavailable")
...
def _truth_boundary() -> dict[str, bool]:
    return {
        "current_modulated_leading_through_I2_consumed": True,
        "current_I2_leading_plus_frozen_complete_curl_oscillation_materialized": True,
        "full_concrete_oscillatory_runtime_digest_bound": True,
        "identity_preserving_current_I2_composite_save_load_available": True,
        ...
        "high_precision_I2_nonzero_implies_public_float64_velocity_delta": False,
...
def _cartesian_i2_probe(
    field: "CurrentI2LeadingOscillatoryField",
    fractions: Any,
    eta: Any,
    t: Any,
    theta: Any,
    ...
    if not all(np.all(np.isfinite(v)) for v in (fractions, eta, t, theta)):
        raise ValueError("I2 probe inputs must be finite")
    if np.any((fractions <= 0.0) | (fractions >= 1.0)):
        raise ValueError("I2 stage fractions must lie strictly between zero and one")
    if np.any(np.abs(eta) >= 1.0):
        raise ValueError("I2 probe eta must satisfy |eta|<1")
    if np.any((t <= 0.0) | (t >= 1.0)):
        raise ValueError("I2 probe time must lie strictly between zero and one")

    log_start = np.log(field.X_I2_start)
    log_end = np.log(field.X_I2_end)
...
class CurrentI2LeadingOscillatoryField:
    """Exact A1 #1061 leading through I2 plus frozen A2 complete-curl oscillation."""
    ...
    def X_I2_start(self) -> float:
        return float(self._leading_identity["X_I2_start"])

    @property
    def X_I2_end(self) -> float:
        return float(self._leading_identity["X_I2_end"])
    ...
    @classmethod
    def from_configuration(cls, payload: Mapping[str, Any]) -> "CurrentI2LeadingOscillatoryField":
        if not isinstance(payload, Mapping) or payload.get("schema") != SCHEMA:
            raise ValueError("unexpected current-I2 composite configuration schema")
        ...
        if not isinstance(parent, Mapping) or (
            ...
            raise ValueError("Agent-2 #1062 parent identity drifted")

        if payload.get("source_provenance") != _source_provenance():
            raise ValueError("corrected-source provenance drifted")
        if payload.get("composition") != "u_current_I2 = u_lead_A1_1061 + u_osc_frozen_complete_curl":
            raise ValueError("current-I2 composition contract drifted")
        if payload.get("truth_boundary") != _truth_boundary():
            raise ValueError("current-I2 truth boundary drifted")
        ...
        if not isinstance(leading, Mapping) or (
            ...
            raise ValueError("Agent-1 #1061 provenance drifted")
        leading_config = leading.get("configuration")
        if not isinstance(leading_config, Mapping):
            raise ValueError("serialized Agent-1 #1061 configuration is missing")
        ...
        if obj.configuration() != payload:
            raise ValueError("reconstructed current-I2 composite configuration drifted")
        return obj

    @classmethod
    def load_candidate(cls, path: str | Path) -> "CurrentI2LeadingOscillatoryField":
        raw = json.loads(Path(path).read_text(encoding="utf-8"))
        if not isinstance(raw, Mapping) or raw.get("schema") != SERIALIZED_SCHEMA:
            raise ValueError("unexpected saved current-I2 candidate schema")
        configuration = raw.get("configuration")
        if not isinstance(configuration, Mapping):
            raise ValueError("saved current-I2 candidate configuration is missing")
        stored_semantic = _require_sha256(raw.get("semantic_sha256"), "saved current-I2 semantic SHA")
        if _identity._sha256(configuration) != stored_semantic:
            raise ValueError("saved current-I2 semantic digest mismatch")
        obj = cls.from_configuration(configuration)
        if obj.semantic_sha256 != stored_semantic:
            raise ValueError("reloaded current-I2 semantic identity drifted")
        return obj


@lru_cache(maxsize=1)
def default_field() -> CurrentI2LeadingOscillatoryField:
    return CurrentI2LeadingOscillatoryField(_load_exact_agent1_backend())


def velocity(x: Any, y: Any, z: Any, t: Any) -> np.ndarray:
    """Return exact current-I2 leading plus the frozen A2 complete-curl oscillation."""
...
def public_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "current_modulated_leading_through_I2_materialized": True,
        "current_I2_leading_plus_oscillatory_materialized": True,
...
def materialize_current_i2_composite_receipt() -> dict[str, Any]:
    field = default_field()
    ...
    i2_x, i2_y, i2_z, i2_t, i2_X = _cartesian_i2_probe(
        field, _I2_STAGE_FRACTIONS, _I2_ETA, _I2_TIMES, _I2_THETA
    )
    i2 = _composition._evaluate_with_backend(field.leading_backend, i2_x, i2_y, i2_z, i2_t)
    ...
    i2_closure = float(np.max(np.abs(i2.velocity - (i2.leading + i2.oscillatory))))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        candidate_path = Path(tmp) / "current_i2_candidate.json"
        saved = field.save_candidate(candidate_path)
        loaded = CurrentI2LeadingOscillatoryField.load_candidate(candidate_path)
        replay_inner = loaded.velocity(
            _INNER_POINTS[:, 0], _INNER_POINTS[:, 1], _INNER_POINTS[:, 2], _INNER_TIMES
        )
        replay_i2 = loaded.velocity(i2_x, i2_y, i2_z, i2_t)
        replay_max = float(
            max(
                np.max(np.abs(replay_inner - inner.velocity)),
                np.max(np.abs(replay_i2 - i2.velocity)),
            )
        )
    ...
    return {
        ...
        "diagnostic": {
            ...
            "i2_probe_count": int(_I2_STAGE_FRACTIONS.shape[0]),
            "i2_stage_fraction": [float(v) for v in _I2_STAGE_FRACTIONS],
            "i2_similarity_X": [float(v) for v in i2_X],
            "X_I2_start": field.X_I2_start,
            "X_I2_end": field.X_I2_end,
            "inner_composition_closure_max_abs": inner_closure,
            "i2_composition_closure_max_abs": i2_closure,
        ...
        "frozen_mechanical_gates": {
            ...
            "i2_composition_closure_max_abs": COMPOSITION_ATOL,
...
def enforce_receipt(receipt: Mapping[str, Any]) -> None:
    if not isinstance(receipt, Mapping) or receipt.get("schema") != RECEIPT_SCHEMA:
        raise AssertionError("unexpected current-I2 composite receipt schema")
    ...
    if receipt.get("saved_semantic_sha256") != receipt.get("semantic_sha256"):
        raise AssertionError("saved/reloaded current-I2 semantic identity drifted")
    ...
    if receipt.get("parent_agent2_source_blob_sha1") != PARENT_AGENT2_SOURCE_BLOB_SHA1:
        raise AssertionError("Agent-2 #1062 parent source identity drifted")
    ...
    if receipt.get("public_contract") != public_contract():
        raise AssertionError("public current-I2 velocity contract drifted")
    if receipt.get("truth_boundary") != _truth_boundary():
        raise AssertionError("current-I2 truth boundary drifted")

    diagnostic = receipt.get("diagnostic")
    if not isinstance(diagnostic, Mapping):
        raise AssertionError("current-I2 diagnostic is missing")
    if int(diagnostic.get("inner_probe_count", -1)) != int(_INNER_POINTS.shape[0]):
        raise AssertionError("inner probe count drifted")
    if int(diagnostic.get("i2_probe_count", -1)) != int(_I2_STAGE_FRACTIONS.shape[0]):
        raise AssertionError("I2 probe count drifted")
    if diagnostic.get("i2_stage_fraction") != [float(v) for v in _I2_STAGE_FRACTIONS]:
        raise AssertionError("I2 probe fractions drifted")
    x_start = float(diagnostic.get("X_I2_start", np.nan))
    x_end = float(diagnostic.get("X_I2_end", np.nan))
    xs = np.asarray(diagnostic.get("i2_similarity_X", ()), dtype=float)
    if not (np.isfinite(x_start) and np.isfinite(x_end) and 0.0 < x_start < x_end):
        raise AssertionError("I2 interval is invalid")
    if xs.shape != _I2_STAGE_FRACTIONS.shape or not np.all((xs > x_start) & (xs < x_end)):
        raise AssertionError("I2 probes left the authenticated I2 interval")
    ...
    if float(diagnostic.get("i2_composition_closure_max_abs", np.inf)) > COMPOSITION_ATOL:
        raise AssertionError("I2 leading+oscillatory addition failed")
    ...
    if float(diagnostic.get("velocity_save_load_replay_max_abs", np.inf)) > VELOCITY_REPLAY_ATOL:
        raise AssertionError("current-I2 save/load velocity replay drifted")
    ...
    if gates != expected_gates:
        raise AssertionError("current-I2 mechanical gates drifted")
...
def _main() -> None:
    ...
    receipt = materialize_current_i2_composite_receipt()
