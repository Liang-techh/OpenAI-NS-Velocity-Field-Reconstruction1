...
from openai_ns_reconstruction.kokuno_current_rf40_first_turn_nonlinear_radial_force import (
...
def test_centered_dz_sigma1_mechanics_remain_exact_for_affine_stress():
    radii = np.asarray(_geometry().radii)
    step = 0.006
    z0 = -0.11
    slope = 0.25 - 0.4 * radii + 0.6 * radii**2
    intercept = np.cos(0.8 * radii)
    plus = intercept + (z0 + step) * slope
    minus = intercept + (z0 - step) * slope
    derivative = centered_axial_stress_derivative(plus, minus, step)
    assert np.allclose(derivative, slope, rtol=0.0, atol=3.0e-14)


def test_first_turn_piece_attribution_uses_fixed_derivative_ladder():
    geometry = _geometry()
    radii = np.asarray(geometry.radii)
    quadratic_slope = 0.2 * radii + 0.1 * radii**2
    mixed_slope = -0.08 + 0.35 * radii
    aggregate_slope = quadratic_slope + mixed_slope

    def sampler(z_value: float):
        base = np.sin(0.9 * radii)
        quadratic = 0.15 * base + z_value * quadratic_slope
        mixed = -0.05 * base + z_value * mixed_slope
        return {
            "quadratic": quadratic,
            "mixed": mixed,
    ...
    for level in result["levels"]:
        assert (
            level["piece_closure_relative_max"]
            <= RADIAL_FORCE_PIECE_CLOSURE_RELATIVE_GATE
        )
        assert np.allclose(level["quadratic"], quadratic_slope, atol=3.0e-13, rtol=0.0)
        assert np.allclose(level["mixed"], mixed_slope, atol=3.0e-13, rtol=0.0)
        assert np.allclose(level["aggregate"], aggregate_slope, atol=4.0e-13, rtol=0.0)


def test_piece_attribution_mutation_still_fails_closed():
    geometry = _geometry()
    radii = np.asarray(geometry.radii)
...
def test_truth_boundary_advances_only_first_turn_radial_force():
    boundary = truth_boundary()
    assert SOURCE_RADIAL_FORCE_FORMULA == "(div T)_r = partial_z sigma_1"
    assert boundary["parent_agent3_pr"] == 1000
    assert boundary["current_RF40_first_turn_axial_e1_stress_consumed"] is True
    assert (
        boundary["current_RF40_first_turn_radial_force_from_dz_sigma1_materialized"]
        is True
    )
    ...
    assert boundary["z_derivative_step_ladder"] == [0.02, 0.01, 0.005]
    ...
    for key in (
        ...
        "scoped_current_RF40_first_turn_stress_authorized_as_correction_target",
