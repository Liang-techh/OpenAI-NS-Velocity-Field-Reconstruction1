...
from openai_ns_reconstruction import constrained_st051b_frozen_redist_material_path as m
...
def test_source_identity_and_truth_boundary_are_fail_closed():
    ...
    assert m.AGENT7_SOURCE_HEAD == "95f94188270f02511c8bdf660822f77530c9983a"
...
def test_truth_audit_rejects_promotion_and_rebalance():
    report = {
        "task_id": m.TASK_ID,
        ...
        "redistribution": {"transform_rebalanced_on_st051b": False},
    ...
    report["redistribution"]["transform_rebalanced_on_st051b"] = True
    with pytest.raises(ValueError, match="transform identity drift"):
        m._audit_truth(report)
