...
from openai_ns_reconstruction.kokuno_current_i4_nonlinear_radial_stress import (
...
def _mechanics_fixture():
    ...
    witness = _materialize_from_provider(
        ...
        backend=None,
        backend_kind="analytic-current-i4-radial-stress-routing-mechanics-only",
    )
...
def test_current_i4_mean_routes_through_inherited_compact_radial_operator():
...
def test_receipt_records_exact_pr1101_current_i4_lineage():
    receipt = _mechanics_fixture().to_receipt()
    boundary = receipt["truth_boundary"]

    assert receipt["backend_kind"] == (
        "exact-current-a3-pr1101-mean-to-inherited-compact-radial-stress"
    )
    assert receipt["parent_agent3_1101_source_blob"] == PARENT_AGENT3_SOURCE_BLOB
    assert boundary["parent_agent3_pr"] == 1101
    assert boundary["parent_agent3_head"] == "6c9a4e7b0dec4b3669fb1dc60297b18f10163e0b"
    assert boundary["agent2_composite_pr"] == 1080
    assert boundary["agent2_composite_head"] == "c40d8ddecd2971544a6e07dab093436b423cf326"
    assert boundary["agent1_leading_pr"] == 1079
    assert boundary["agent1_leading_head"] == "b06742ca6e189499192ede3cce40f62cdc1e35ca"
    assert boundary["source_I4_reserved_mean_correction_interval_recorded"] is True
    assert boundary["current_I4_nonlinear_radial_inverse_performed"] is True
    assert boundary["current_I4_quadratic_radial_stress_materialized"] is True
    assert boundary["source_I4_five_row_mean_correction_materialized"] is False
...
def test_parent_source_blob_is_fail_closed():
    routed = _mechanics_fixture()
    with pytest.raises(ValueError, match="exact A3 #1101"):
        _materialize_current_mean_witness(
            routed.mean_witness,
...
def test_public_contract_has_no_scientific_tuning_knobs():
    params = inspect.signature(materialize_current_i4_nonlinear_radial_stress).parameters
...
def test_materializer_rejects_untyped_backend_before_any_residual_work():
    ...
    with pytest.raises(TypeError, match="ExactCurrentI4NonlinearBackend"):
        materialize_current_i4_nonlinear_radial_stress(object(), geometry)


def test_truth_boundary_keeps_force_full_ns_and_cycle_claims_closed():
    boundary = truth_boundary()
    assert boundary["current_I4_leading_plus_oscillatory_identity_consumed"] is True
    assert boundary["current_I4_mixed_nonlinear_mean_consumed"] is True
    assert boundary["current_I4_quadratic_oscillatory_mean_consumed"] is True
    assert boundary["current_I4_aggregate_nonlinear_mean_consumed"] is True
    assert boundary["current_I4_quadratic_radial_stress_materialized"] is True
    assert boundary["current_I4_mixed_radial_stress_materialized"] is True
    assert boundary["current_I4_aggregate_radial_stress_materialized"] is True
    ...
    for key in (
        ...
        "complete_ns_defect",
        "scoped_current_I4_radial_stress_authorized_as_complete_ns_defect",
        "scoped_current_I4_radial_stress_authorized_as_correction_target",
