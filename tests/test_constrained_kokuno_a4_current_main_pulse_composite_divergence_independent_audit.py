...
from openai_ns_reconstruction import (
    kokuno_a4_current_main_pulse_composite_divergence_independent_audit as audit,
)
...
class _ZeroField:
    ...
    def velocity(self, x, y, z, t):
        ...
        return np.stack(
            (np.zeros_like(x), np.zeros_like(y), np.zeros_like(z)), axis=-1
        )
...
def _metric(value: float = 1.0e-6, speed: float = 1.0) -> dict[str, float]:
    return {
        ...
        "pulse_entry_seam_sampled_max": value,
...
def _passing_receipt() -> dict:
    checks = {
        ...
        "fd_perturbations_representable_all_axes": True,
        ...
        "post_materializable_fails_closed": True,
...
def test_protocol_is_frozen_to_exact_current_main_pulse_heads_and_gates():
    assert audit.UPSTREAM_PR == 1108
    assert audit.UPSTREAM_HEAD == "9f221dd57ef4b5e2d2e80e24c8cf531e991a5387"
    assert audit.AGENT1_PR == 1100
    assert audit.AGENT1_HEAD == "2a6c58ff6bf6d1ba6b875bca0e6abf9d1118e156"
    assert audit.NEWER_AGENT1_PR == 1107
    assert audit.NEWER_AGENT1_HEAD == "45da043dd2b4cd067f005a72c7e21fd0f2bcf309"
    assert audit.SEED == 9173861
    assert audit.SPATIAL_STEPS == (0.02, 0.01, 0.005)
    assert audit.DIVERGENCE_GATE == 1.0e-5


def test_truth_boundary_cannot_promote_newer_leading_full_pde_or_missing_closure():
    truth = audit._TRUTH_BOUNDARY
    assert truth["current_main_pulse_finite_x_leading_plus_oscillatory_velocity_consumed"] is True
    assert truth["newer_agent1_1107_logx_xi11_leading_consumed"] is False
    assert truth["source_exact_main_pulse_amplitude_materialized"] is False
...
def test_xlinear_mutation_adds_expected_divergence_signal():
    ...
    mutated_div, _ = audit._divergence(
        audit._XLinearMutation(_ZeroField()), points, times, 0.005
    )
    signal = np.max(np.abs(mutated_div - base_div))
    assert signal == pytest.approx(
        audit.MUTATION_EPSILON, rel=1.0e-12, abs=1.0e-14
    )
    assert signal >= audit.MUTATION_DETECTION_FLOOR


def test_fd_perturbation_representability_fails_closed_at_huge_coordinates():
    ordinary = np.asarray(((0.2, -0.3, 0.1), (1.0, -1.0, 0.5)), dtype=float)
    report = audit._fd_perturbation_report(ordinary)
    assert report["all_steps_representable"] is True

    huge = np.asarray(((1.0e200, 0.0, 0.0),), dtype=float)
    report = audit._fd_perturbation_report(huge)
    assert report["all_steps_representable"] is False
    assert any(
        item["nonrepresentable_coordinate_axis_pairs"] > 0
...
def test_each_scientific_gate_can_fail_without_threshold_rewrite():
    ...
    bad = copy.deepcopy(base)
    bad["checks"]["post_materializable_fails_closed"] = False
    assert audit._derive_audit_pass(bad) is False

    bad = copy.deepcopy(base)
    bad["checks"]["fd_perturbations_representable_all_axes"] = False
    assert audit._derive_audit_pass(bad) is False
...
def test_public_audit_surface_has_no_scientific_tuning_inputs():
    assert list(inspect.signature(audit.materialize_receipt).parameters) == []
    forbidden = {
        "threshold",
        "residual",
        "forcing",
    ...
    assert not (set(inspect.signature(audit.materialize_receipt).parameters) & forbidden)
