...
from openai_ns_reconstruction.kokuno_current_exterior_xr_nonlinear_mean_attribution import (
    AGENT1_LEADING_HEAD,
    AGENT1_LEADING_PR,
...
def test_through_xr_configuration_accepts_only_pinned_truth_and_runtime():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("truth_boundary", "post_XR_velocity_materialized"), True, "truth boundary"),
        (
...
def test_through_xr_configuration_fails_closed_on_identity_or_truth_drift(
    path, value, message
):
    config = _configuration()
    cursor = config
...
def test_exact_binder_rejects_unpinned_backend_before_scientific_use():
    ...
    with pytest.raises(ValueError, match="module identity"):
        ExactCurrentExteriorXRNonlinearBackend.bind(FakeField(), lambda p, t: None)
...
def test_truth_boundary_advances_only_through_xr_mean_scope():
    boundary = truth_boundary()
    assert boundary["current_leading_plus_oscillation_through_XR_consumed"] is True
    assert boundary["velocity_beyond_Xh_through_XR_materialized"] is True
    assert boundary["current_through_XR_mixed_nonlinear_mean_materialized"] is True
    assert boundary["current_through_XR_quadratic_nonlinear_mean_materialized"] is True
    assert boundary["current_through_XR_aggregate_nonlinear_mean_materialized"] is True
