...
from openai_ns_reconstruction import (
    kokuno_current_rf40_axial_shutdown_leading_oscillatory_identity as mod,
)
...
def _synthetic_receipt() -> dict:
    ...
    return {
        ...
        "diagnostic": {
            "inner_probe_count": 6,
            "axial_shutdown_probe_count": 3,
            "axial_shutdown_stage_fraction": [0.18, 0.57, 1.0],
            "axial_shutdown_X_over_X1": [math.exp(0.18), math.exp(0.57), math.e],
            "axial_shutdown_X_over_X2": [math.exp(-0.82), math.exp(-0.43), 1.0],
            "inner_composition_closure_max_abs": 0.0,
            "axial_shutdown_composition_closure_max_abs": 0.0,
        ...
        "frozen_mechanical_gates": {
            ...
            "axial_shutdown_composition_closure_max_abs": mod.COMPOSITION_ATOL,
...
def test_parent_agent2_source_is_exactly_the_stacked_pr992_blob() -> None:
    assert mod.PARENT_AGENT2_PR == 992
    assert mod.PARENT_AGENT2_HEAD == "2a2fad307a674ec1eb5a2bc426c0d01c3a0e7377"
    assert mod._validate_parent_a2_source() == mod.PARENT_AGENT2_SOURCE_BLOB_SHA1


def test_agent1_target_is_exact_rf40_axial_shutdown_sibling() -> None:
    assert mod.AGENT1_PR == 993
    assert mod.AGENT1_HEAD == "2ac6460b483efb1c07f2fa65e7fed781a32f2718"
    assert mod.AGENT1_CLASS == "KokunoPA16CurrentCartesianRF40AxialShutdown"
    truth = mod._required_agent1_truth()
    assert truth["current_lineage_RF40_axial_shutdown_materialized"] is True
...
def test_public_velocity_contract_has_no_tuning_escape_hatch() -> None:
    ...
    assert contract["leading_materialized_through_RF40_axial_shutdown"] is True
    assert contract["full_oscillatory_runtime_payload_bound"] is True
    assert contract["identity_preserving_save_load"] is True
    assert contract["after_RF40_axial_shutdown_fail_closed"] is True
...
def test_truth_boundary_advances_only_axial_shutdown_composition() -> None:
    truth = mod._truth_boundary()
    assert truth["current_leading_plus_oscillatory_velocity_through_RF40_axial_shutdown_materialized"] is True
    assert truth["RF40_axial_shutdown_leading_plus_oscillatory_materialized"] is True
    assert truth["full_concrete_oscillatory_runtime_digest_bound"] is True
    assert truth["identity_preserving_RF40_axial_shutdown_composite_save_load_available"] is True
...
def test_synthetic_receipt_gate_accepts_only_frozen_scope() -> None:
    ...
    leaked["diagnostic"]["axial_shutdown_oscillatory_abs_max"] = 1.0e-12
