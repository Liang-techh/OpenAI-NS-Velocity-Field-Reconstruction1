...
from openai_ns_reconstruction.kokuno_current_partial_nonlinear_radial_stress import (
...
def _mechanics_fixture():
    ...
    witness = _materialize_from_provider(
        ...
        backend=None,
        backend_kind="analytic-current-routing-mechanics-only",
    )
...
def test_current_mean_routes_through_inherited_compact_radial_operator():
...
def test_receipt_replaces_historical_strict_inner_truth_boundary():
    receipt = _mechanics_fixture().to_receipt()
    boundary = receipt["truth_boundary"]

    assert receipt["backend_kind"] == (
        "exact-current-a3-pr971-mean-to-inherited-compact-radial-stress"
    )
    assert boundary["current_partial_nonlinear_radial_inverse_performed"] is True
    assert boundary["current_partial_quadratic_radial_stress_materialized"] is True
    assert boundary["partial_domain_through_current_Xh_only"] is True
    assert boundary["strict_inner_only"] is False
    assert boundary["complete_ns_defect"] is False
    assert boundary["scoped_current_partial_radial_stress_authorized_as_correction_target"] is False
...
def test_parent_source_blob_is_fail_closed():
    ...
    with pytest.raises(ValueError, match="exact A3 #971"):
        _materialize_current_mean_witness(
            witness,
            geometry,
...
def test_public_contract_has_no_scientific_tuning_knobs():
    params = inspect.signature(materialize_current_partial_nonlinear_radial_stress).parameters
...
def test_truth_boundary_keeps_full_ns_and_correction_claims_closed():
    ...
    assert boundary["current_partial_leading_plus_oscillation_consumed"] is True
