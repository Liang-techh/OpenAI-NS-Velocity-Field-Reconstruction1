...
from openai_ns_reconstruction import kokuno_current_i1_leading_oscillatory_identity as mod
...
def _synthetic_receipt() -> dict:
    ...
    x_start = 10.0
    x_end = 80.0
    fractions = [float(v) for v in mod._I1_STAGE_FRACTIONS]
    xs = [math.exp(math.log(x_start) + f * (math.log(x_end) - math.log(x_start))) for f in fractions]
    return {
        ...
        "diagnostic": {
            "inner_probe_count": 6,
            "i1_probe_count": 3,
            "i1_stage_fraction": fractions,
            "i1_similarity_X": xs,
            "X_I1_start": x_start,
            "X_I1_end": x_end,
            "frozen_i1_closure_tolerance": mod.I1_CLOSURE_TOLERANCE,
            "inner_composition_closure_max_abs": 0.0,
            "i1_composition_closure_max_abs": 0.0,
        ...
        "frozen_mechanical_gates": {
            ...
            "i1_composition_closure_max_abs": mod.COMPOSITION_ATOL,
...
def test_parent_a2_is_exact_1052_source_blob() -> None:
    assert mod.PARENT_AGENT2_PR == 1052
    assert mod.PARENT_AGENT2_HEAD == "d58cee2bf38bdca8da13a796215a079a569ad7d7"
    assert mod._validate_parent_a2_source() == mod.PARENT_AGENT2_SOURCE_BLOB_SHA1


def test_target_is_exact_current_i1_frozen_gate_sibling() -> None:
    assert mod.AGENT1_PR == 1051
    assert mod.AGENT1_HEAD == "ea59dc305b4265fcb0bb0f948c2a45f4477d1ae5"
    assert mod.AGENT1_CLASS == "KokunoPA16CurrentCartesianI1FrozenGate"
    assert mod.AGENT1_SOURCE_BLOB_SHA1 == "fe14dab8dccdbca828d722bdfe2f75f287aa9ea8"
    assert mod.I1_CLOSURE_TOLERANCE == 5.0e-7
    truth = mod._required_agent1_truth()
    assert truth["current_I1_closure_gate_frozen"] is True
    assert truth["current_I2_overlay_applied"] is False
...
def test_public_velocity_contract_has_no_tuning_or_correction_escape_hatch() -> None:
    ...
    assert contract["current_modulated_leading_through_I1_materialized"] is True
    assert contract["current_I1_leading_plus_oscillatory_materialized"] is True
    assert contract["full_oscillatory_runtime_payload_bound"] is True
    assert contract["identity_preserving_save_load"] is True
    assert contract["frozen_agent1_i1_closure_tolerance"] == 5.0e-7
...
def test_truth_boundary_advances_only_current_i1_composition() -> None:
    truth = mod._truth_boundary()
    assert truth["current_modulated_leading_through_I1_consumed"] is True
    assert truth["current_I1_leading_plus_frozen_complete_curl_oscillation_materialized"] is True
    assert truth["full_concrete_oscillatory_runtime_digest_bound"] is True
    assert truth["identity_preserving_current_I1_composite_save_load_available"] is True
...
def test_i1_probe_fractions_are_fixed_strictly_interior_and_replay_similarity_x() -> None:
    np.testing.assert_array_equal(mod._I1_STAGE_FRACTIONS, np.asarray((0.18, 0.57, 0.92)))
...
def test_synthetic_receipt_gate_accepts_only_frozen_scope() -> None:
    ...
    loosened_gate = copy.deepcopy(receipt)
    loosened_gate["diagnostic"]["frozen_i1_closure_tolerance"] = 1.0e-5
    with pytest.raises(AssertionError, match="closure tolerance"):
        mod.enforce_receipt(loosened_gate)
