...
def test_cubic_schedule_preserves_early_endpoint_and_three_static_returns():
    base = _base()
    candidate = Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=base)
    quadratic = Eq45SupportedPhi10EarlyLocalizedTemporalCandidate(base=base)
    ...
    np.testing.assert_array_equal(
        ...
        quadratic.at_points(points, 0.25),
    )
    ...
    # Off-keyframe motion remains nontrivial but is much more localized than the
    # quadratic schedule screened earlier.
    assert candidate.coefficient_at(0.375) == pytest.approx(-0.65)
    assert candidate.coefficient_at(0.6875) == pytest.approx(-0.321875)
    assert not np.array_equal(
        candidate.at_points(points, 0.375),
        base.at_points(points, 0.375),
    )


def test_candidate_changes_only_existing_phi10_and_supports_mixed_time_grid():
    base = _base()
    candidate = Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=base)

    for time in (0.25, 0.375, 0.50, 0.625, 0.6875, 0.75):
    ...
    points = _points()
    times = np.array([0.25, 0.375, 0.50, 0.625, 0.6875, 0.75])
...
def test_candidate_roundtrips_with_distinct_identity_and_truth_boundary(tmp_path):
    ...
    path = tmp_path / "phi10_cubic_localized.json"
...
def test_candidate_fails_closed_on_zero_nonfinite_bounds_time_and_metadata():
    ...
    with pytest.raises(ValueError, match="nonzero"):
        Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=base, early_delta=0.0)
    with pytest.raises(ValueError, match="finite"):
        Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=base, early_delta=np.nan)
    with pytest.raises(ValueError, match="coefficient bound"):
        Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=base, early_delta=-3.8)
    with pytest.raises(TypeError, match="base"):
        Eq45SupportedPhi10CubicLocalizedTemporalCandidate(base=object())
    ...
    with pytest.raises(ValueError, match="truth-boundary"):
        Eq45SupportedPhi10CubicLocalizedTemporalCandidate.from_dict(payload)
