...
import openai_ns_reconstruction.kokuno_a4_current_exterior_xr_leading_oscillatory_divergence_independent_audit as a4
...
def synthetic_receipt():
    ...
    receipt = {
        ...
        "protocol": {
            ...
            "exterior_zones": [list(v) for v in a4.EXTERIOR_ZONES],
            "exterior_points_per_zone_time": a4.EXTERIOR_POINTS_PER_ZONE_TIME,
            "seam_offsets": list(a4.SEAM_OFFSETS),
...
def test_sampler_is_deterministic_and_contains_all_strata():
    f = SumField()
    p1, p2 = a4.make_probes(f), a4.make_probes(f)
    np.testing.assert_array_equal(p1.inner.points, p2.inner.points)
    np.testing.assert_array_equal(p1.exterior.points, p2.exterior.points)
    np.testing.assert_array_equal(p1.inner.weights, p2.inner.weights)
    np.testing.assert_array_equal(p1.exterior.weights, p2.exterior.weights)
    assert len(p1.inner.points) == len(a4.TIMES) * a4.INNER_POINTS_PER_TIME
    assert len(p1.exterior.points) == len(a4.TIMES) * len(a4.EXTERIOR_ZONES) * a4.EXTERIOR_POINTS_PER_ZONE_TIME
    assert len(p1.seam_points) == len(a4.TIMES) * len(a4.SEAM_OFFSETS)
    assert len(p1.axis_points) == 5
    assert np.all(p1.inner.weights > 0.0)
    assert np.all(p1.exterior.weights > 0.0)
...
def test_staged_metrics_and_mutation_detection_on_manufactured_fields():
    ...
    assert max(mutated["inner"]["total"]["sampled_max"], mutated["exterior"]["total"]["sampled_max"]) >= 9.9e-4
...
def test_frozen_gate_accepts_good_receipt_and_rejects_laundering():
    ...
    bad["resolutions"][-1]["inner"]["total"]["sampled_max"] = 2.0e-5
    bad["audit_pass"] = True
    _seal(bad)
