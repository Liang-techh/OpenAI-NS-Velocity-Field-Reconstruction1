...
from openai_ns_reconstruction import (
    kokuno_a4_postpulse_eta_independent_hold_leading_divergence_independent_audit as m,
)
...
def test_preregistered_identity_and_thresholds_are_frozen():
    ...
    assert m.UPSTREAM_HEAD == "76aba89041e8103ac464fda9ed75f7a18ef596b9"
...
def test_heldout_hold_probe_geometry_is_deterministic_and_finite():
    ...
    expected = field.log_X_flatten_end + field.hold_length * a.late_rho
    assert np.allclose(recovered, expected, rtol=0.0, atol=2e-12 * max(1.0, abs(field.log_X_hold_end)))


def test_save_reload_is_identity_bound_and_hold_mutations_fail_closed(tmp_path):
    field = m.default_field()
    path = tmp_path / "candidate.json"
    payload = field.save_configuration(path)
    loaded = KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.load_configuration(path)
    assert loaded.semantic_sha256 == field.semantic_sha256

    mutated = copy.deepcopy(payload)
    mutated["bound_scope"]["hold_length_role"] = "residual_tuned"
    with pytest.raises(ValueError):
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.from_configuration(mutated)

    mutated = copy.deepcopy(payload)
    mutated["bound_scope"]["hold_length"] *= 1.001
    with pytest.raises(ValueError):
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHold.from_configuration(mutated)


def test_post_hold_stage_fails_closed():
    field = m.default_field()
    with pytest.raises(ValueError):
