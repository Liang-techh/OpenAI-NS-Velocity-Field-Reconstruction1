...
from openai_ns_reconstruction import (
    kokuno_current_rf40_power_law_leading_oscillatory_identity as mod,
)
...
def _synthetic_receipt() -> dict:
    ...
    T_w = 2.0
    X4 = X3 * math.exp(T_w)
    fractions = [float(v) for v in mod._POWER_STAGE_FRACTIONS]
    X = [X3 * math.exp(v * T_w) for v in fractions]
    return {
        ...
        "diagnostic": {
            "inner_probe_count": 6,
            "power_law_probe_count": 3,
            "power_law_stage_fraction": fractions,
            "power_law_log_X_over_X3": [math.log(v / X3) for v in X],
            "power_law_X_over_X4": [v / X4 for v in X],
            "inner_composition_closure_max_abs": 0.0,
            "power_law_composition_closure_max_abs": 0.0,
        ...
        "frozen_mechanical_gates": {
            ...
            "power_law_composition_closure_max_abs": mod.COMPOSITION_ATOL,
...
def test_parent_agent2_source_is_exactly_the_stacked_pr1004_blob() -> None:
    assert mod.PARENT_AGENT2_PR == 1004
    assert mod.PARENT_AGENT2_HEAD == "e9697cdfe2249e1b8bc2f9241bd16d456c60418f"
    assert mod._validate_parent_a2_source() == mod.PARENT_AGENT2_SOURCE_BLOB_SHA1


def test_agent1_target_is_exact_rf40_power_law_sibling() -> None:
    assert mod.AGENT1_PR == 1005
    assert mod.AGENT1_HEAD == "2c76ebdc41d6c566f43a1305034ba2ff9dce410b"
    assert mod.AGENT1_CLASS == "KokunoPA16CurrentCartesianRF40PowerLaw"
...
def test_source_provenance_is_explicit_and_non_paper_exact() -> None:
    ...
    assert "power-law" in source["scope"]
...
def test_public_velocity_contract_has_no_tuning_escape_hatch() -> None:
    ...
    assert contract["leading_materialized_through_RF40_power_law"] is True
    assert contract["full_oscillatory_runtime_payload_bound"] is True
    assert contract["identity_preserving_save_load"] is True
    assert contract["after_RF40_power_law_fail_closed"] is True
...
def test_truth_boundary_advances_only_power_law_composition() -> None:
    ...
    assert truth["RF40_power_law_leading_plus_oscillatory_materialized"] is True
    assert truth["full_concrete_oscillatory_runtime_digest_bound"] is True
    assert truth["identity_preserving_RF40_power_law_composite_save_load_available"] is True
...
def test_power_stage_probe_fractions_are_fixed_and_strictly_interior() -> None:
    np.testing.assert_array_equal(mod._POWER_STAGE_FRACTIONS, np.asarray((0.08, 0.37, 0.72)))
    assert np.all(mod._POWER_STAGE_FRACTIONS > 0.0)
    assert np.all(mod._POWER_STAGE_FRACTIONS < 1.0)
...
def test_synthetic_receipt_gate_accepts_only_frozen_scope() -> None:
    ...
    leaked["diagnostic"]["power_law_oscillatory_abs_max"] = 1.0e-12
