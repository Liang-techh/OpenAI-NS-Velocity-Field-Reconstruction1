...
from openai_ns_reconstruction.kokuno_current_rf40_axial_shutdown_nonlinear_mean_attribution import (
    AGENT1_LEADING_HEAD,
    AGENT1_LEADING_PR,
...
def test_axial_shutdown_configuration_accepts_only_pinned_truth_runtime_and_source():
    _validate_axial_shutdown_configuration(
        _configuration(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ("truth_boundary", "velocity_after_RF40_axial_shutdown_materialized"),
            True,
            "truth boundary",
        ),
        (
...
def test_axial_shutdown_configuration_fails_closed_on_identity_source_or_truth_drift(
    path, value, message
):
...
def test_exact_binder_rejects_unpinned_backend_before_scientific_use():
    ...
    with pytest.raises(ValueError, match="module identity"):
        ExactCurrentRF40AxialShutdownNonlinearBackend.bind(
...
def test_truth_boundary_advances_only_axial_shutdown_mean_scope():
    boundary = truth_boundary()
    assert boundary["current_leading_plus_oscillation_through_RF40_axial_shutdown_consumed"] is True
    assert boundary["RF40_axial_shutdown_velocity_materialized"] is True
    assert boundary["current_RF40_axial_shutdown_mixed_nonlinear_mean_materialized"] is True
    assert boundary["current_RF40_axial_shutdown_quadratic_nonlinear_mean_materialized"] is True
    assert boundary["current_RF40_axial_shutdown_aggregate_nonlinear_mean_materialized"] is True
