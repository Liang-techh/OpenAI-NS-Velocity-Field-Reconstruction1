...
from openai_ns_reconstruction import (
    kokuno_a4_postpulse_hold_prefix_leading_divergence_independent_audit as m,
)
...
def test_preregistered_identity_and_thresholds_are_frozen():
    assert m.TASK == "K4-VAL-119"
    assert m.UPSTREAM_HEAD == "5bafa4199ef127bf50bce30a58cc69ded99e1aa9"
    assert m.SEED == 9173911
...
def test_heldout_prefix_probe_geometry_is_deterministic_and_finite():
    ...
    assert np.allclose(a.late_rho, (0.92, 0.96, 0.985))

    coords = field.similarity_coordinates_logX(
        a.late_points[:, 0], a.late_points[:, 1], a.late_points[:, 2], a.late_times
    )
    recovered = np.asarray(coords["log_X"], float)
    expected = field.log_X_flatten_end + field.prefix_length * a.late_rho
    assert np.allclose(
        recovered,
        expected,
        rtol=0.0,
        atol=2e-12 * max(1.0, abs(field.log_X_prefix_end)),
    )
    assert np.max(expected) < field.log_X_prefix_end
    assert field.log_X_prefix_end < field.log_X_hold_end


def test_save_reload_is_identity_bound_and_prefix_mutations_fail_closed(tmp_path):
    field = m.default_field()
    ...
    mutated["bound_scope"]["pre_bump_guard_role"] = "residual_tuned"
    ...
    mutated = copy.deepcopy(payload)
    mutated["bound_scope"]["candidate_prefix_length"] *= 1.001
    with pytest.raises(ValueError):
        KokunoPA16CurrentCartesianPostPulseEtaIndependentHoldPrefix.from_configuration(
            mutated
        )
...
def test_post_prefix_stage_fails_closed_while_unedited_helper_is_separate():
    field = m.default_field()
    with pytest.raises(ValueError):
        field.similarity_profile_values_logX(field.log_X_prefix_end + 0.02, 0.0)
    helper = field.unedited_hold_profile_logX(field.log_X_prefix_end + 0.02, 0.0)
    assert np.isfinite(np.asarray(helper["E_unedited"], float)).all()
    assert field.truth_boundary["source_relative_swirl_bumps_materialized"] is False
...
def test_gate_enforcer_accepts_only_frozen_contract():
    ...
    bad = copy.deepcopy(report)
    bad["gates"]["all_canonical_fd_perturbations_representable"] = False
    with pytest.raises(AssertionError):
        m.enforce_preregistered_gates(bad)
