...
import openai_ns_reconstruction.kokuno_current_i2_nonlinear_radial_force as mod
...
def test_public_signature_and_truth_boundary_are_fail_closed() -> None:
    signature = inspect.signature(mod.materialize_current_i2_nonlinear_radial_force)
    assert tuple(signature.parameters) == ("backend", "geometry")
    boundary = mod.truth_boundary()
    assert boundary["parent_agent3_pr"] == 1081
    assert boundary["parent_agent3_head"] == (
        "cb9ee25d0c466d42ec7ca30ee89cc10517d4b5c4"
    )
    assert boundary["parent_agent3_source_blob"] == (
        "876cdd2cbe4060b9d249cfc4960c26da0d577ca9"
    )
    assert boundary["source_radial_force_formula"] == "(div T)_r = partial_z sigma_1"
    assert boundary["current_I2_axial_e1_stress_consumed"] is True
    assert boundary["current_I2_radial_force_from_dz_sigma1_materialized"] is True
    ...
    assert boundary["current_I3_overlay_consumed"] is False
    assert boundary["current_I4_overlay_consumed"] is False
    assert boundary["newer_current_I4_velocity_identity_consumed"] is False
    assert boundary["outer_global_velocity_consumed"] is False
    assert boundary["complete_ns_defect"] is False
    assert boundary["scoped_current_I2_radial_force_authorized_as_correction_target"] is False
...
def test_fixed_z_ladder_recovers_known_derivative_and_piece_closure(
    ...
    monkeypatch.setattr(mod, "ExactCurrentI2NonlinearBackend", FakeBackend)
    monkeypatch.setattr(
        mod,
    ...
    witness = mod.materialize_current_i2_nonlinear_radial_force(
...
def test_wrong_backend_fails_before_parent_evaluation() -> None:
    with pytest.raises(TypeError, match="backend must be"):
        mod.materialize_current_i2_nonlinear_radial_force(object(), _geometry())
