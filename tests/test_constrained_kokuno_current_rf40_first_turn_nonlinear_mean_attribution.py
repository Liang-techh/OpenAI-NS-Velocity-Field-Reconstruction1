...
from openai_ns_reconstruction.kokuno_current_rf40_first_turn_nonlinear_mean_attribution import (
    AGENT1_LEADING_HEAD,
    AGENT1_LEADING_PR,
...
def test_first_turn_configuration_accepts_only_pinned_truth_and_runtime():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ("truth_boundary", "velocity_after_RF40_first_turn_materialized"),
            True,
            "truth boundary",
        ),
        (
...
def test_first_turn_configuration_fails_closed_on_identity_or_truth_drift(
    path, value, message
):
    config = _configuration()
    cursor = config
...
def test_exact_binder_rejects_unpinned_backend_before_scientific_use():
    ...
    with pytest.raises(ValueError, match="module identity"):
        ExactCurrentRF40FirstTurnNonlinearBackend.bind(
...
def test_truth_boundary_advances_only_rf40_first_turn_mean_scope():
    boundary = truth_boundary()
    assert (
        boundary[
            "current_leading_plus_oscillation_through_RF40_first_turn_consumed"
        ]
        is True
    )
    assert boundary["post_XR_RF40_first_turn_velocity_materialized"] is True
    assert boundary["current_RF40_first_turn_mixed_nonlinear_mean_materialized"] is True
    assert (
        boundary["current_RF40_first_turn_quadratic_nonlinear_mean_materialized"]
        is True
    )
    assert (
        boundary["current_RF40_first_turn_aggregate_nonlinear_mean_materialized"]
        is True
    )
