...
import openai_ns_reconstruction.kokuno_current_rf40_axial_shutdown_nonlinear_radial_force as mod
...
def test_public_signature_and_truth_boundary_are_fail_closed() -> None:
    signature = inspect.signature(
        mod.materialize_current_rf40_axial_shutdown_nonlinear_radial_force
    )
    assert tuple(signature.parameters) == ("backend", "geometry")
    boundary = mod.truth_boundary()
    assert boundary["parent_agent3_pr"] == 1015
    assert boundary["parent_agent3_head"] == (
        "2e7683f06bea810881afbe39d8f0cf64815ddd56"
    )
    assert boundary["parent_agent3_source_blob"] == (
        "9c6082e6dc5edacf7b6a64e87549cbec142b3d4a"
    )
    assert boundary["source_radial_force_formula"] == "(div T)_r = partial_z sigma_1"
    assert boundary["current_RF40_axial_shutdown_axial_e1_stress_consumed"] is True
    assert (
        boundary[
            "current_RF40_axial_shutdown_radial_force_from_dz_sigma1_materialized"
        ]
        is True
    )
    ...
    assert boundary["partial_domain_through_RF40_axial_shutdown_only"] is True
    assert boundary["RF40_lambda_turn_current_lineage_consumed"] is False
    assert boundary["RF40_power_law_current_lineage_consumed"] is False
    assert boundary["complete_ns_defect"] is False
    assert (
        boundary[
            "scoped_current_RF40_axial_shutdown_radial_force_authorized_as_correction_target"
        ]
        is False
    )
...
def test_fixed_z_ladder_recovers_known_derivative_and_piece_closure(
    ...
    monkeypatch.setattr(
        mod, "ExactCurrentRF40AxialShutdownNonlinearBackend", FakeBackend
    )
    monkeypatch.setattr(
        mod,
    ...
    witness = mod.materialize_current_rf40_axial_shutdown_nonlinear_radial_force(
