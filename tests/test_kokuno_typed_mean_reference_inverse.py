...
def test_deterministic_receipt_solves_only_uncertified_regression_operator() -> None:
    ...
    assert receipt["provenance"]["upstream_agent3_reference_inverse_pr"] == 703
    assert (
        receipt["provenance"]["upstream_agent3_reference_inverse_head"]
    ...
    rhs = np.asarray(inverse["rhs"], dtype=float)
    delta_y = np.asarray(inverse["delta_y"], dtype=float)
    assert rhs.shape == (49, 2)
    assert delta_y.shape == rhs.shape
...
def test_truth_boundary_remains_fail_closed_for_real_candidate() -> None:
    ...
    assert boundary["upstream_agent3_reference_inverse_pr"] == 703
    assert (
        boundary["upstream_agent3_reference_inverse_head"]
        == "56957cd51c7e6638608f28f8fa21df590194dc84"
    )
