...
from openai_ns_reconstruction.kokuno_current_rf40_axial_shutdown_nonlinear_radial_stress import (
...
def _mechanics_fixture():
    ...
    witness = _materialize_from_provider(
        ...
        backend=None,
        backend_kind="analytic-rf40-axial-shutdown-routing-mechanics-only",
    )
...
def test_rf40_axial_shutdown_mean_routes_through_inherited_compact_radial_operator():
...
def test_receipt_records_exact_pr1011_lineage_and_axial_shutdown_scope():
    ...
    assert receipt["parent_agent3_1011_source_blob"] == PARENT_AGENT3_SOURCE_BLOB
    assert boundary["current_RF40_axial_shutdown_nonlinear_radial_inverse_performed"] is True
    assert boundary["current_RF40_axial_shutdown_quadratic_radial_stress_materialized"] is True
...
def test_parent_source_blob_is_fail_closed():
    ...
    with pytest.raises(ValueError, match="exact A3 #1011"):
        _materialize_current_mean_witness(
            witness,
            geometry,
...
def test_truth_boundary_keeps_full_ns_and_correction_claims_closed():
    ...
    assert (
        boundary["current_leading_plus_oscillation_through_RF40_axial_shutdown_consumed"]
        is True
    )
