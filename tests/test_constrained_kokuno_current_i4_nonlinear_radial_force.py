...
import openai_ns_reconstruction.kokuno_current_i4_nonlinear_radial_force as mod
...
def _geometry() -> StrictInnerTransportRadialGeometry:
    return StrictInnerTransportRadialGeometry(
        time=0.39,
        axial_z=-0.08,
        radii=tuple(float(v) for v in np.linspace(0.02, 0.44, 11)),
        bump_center=0.30,
        bump_halfwidth=0.10,
    )
...
def test_public_signature_and_truth_boundary_are_fail_closed() -> None:
    signature = inspect.signature(mod.materialize_current_i4_nonlinear_radial_force)
    assert tuple(signature.parameters) == ("backend", "geometry")
    boundary = mod.truth_boundary()
    assert boundary["parent_agent3_pr"] == 1109
    assert boundary["parent_agent3_head"] == (
        "49590ff311fef1dec4bd850b3013fd989485c87a"
    )
    assert boundary["parent_agent3_source_blob"] == (
        "b8d9928934d4df74b52e8cd0d2c27c468257cdc9"
    )
    ...
    assert boundary["current_I4_axial_e1_stress_consumed"] is True
    assert boundary["current_I4_radial_force_from_dz_sigma1_materialized"] is True
    ...
    assert boundary["post_I4_or_pulse_leading_identity_consumed"] is False
    assert boundary["outer_global_velocity_consumed"] is False
    assert boundary["complete_ns_defect"] is False
    assert boundary["scoped_current_I4_radial_force_authorized_as_correction_target"] is False
...
def test_fixed_z_ladder_recovers_known_derivative_and_piece_closure(
    ...
    monkeypatch.setattr(mod, "ExactCurrentI4NonlinearBackend", FakeBackend)
    monkeypatch.setattr(
        mod,
    ...
    witness = mod.materialize_current_i4_nonlinear_radial_force(
...
def test_wrong_backend_fails_before_parent_evaluation() -> None:
    with pytest.raises(TypeError, match="backend must be"):
        mod.materialize_current_i4_nonlinear_radial_force(object(), _geometry())
