...
from openai_ns_reconstruction.constrained_eq45_supported_phi10_early_localized_seed_generalization import (
    DEFAULT_SEEDS,
    REGIONS,
    audit_early_localized_phi10_seed_generalization,
    held_out_probe_set,
)


def test_early_localized_probe_draw_is_deterministic_and_inside_contract():
    points, times, labels = held_out_probe_set(DEFAULT_SEEDS[0], 2)
    again_points, again_times, again_labels = held_out_probe_set(DEFAULT_SEEDS[0], 2)
...
def test_early_localized_tradeoff_uses_frozen_force_on_fresh_seeds():
    report = audit_early_localized_phi10_seed_generalization(
        seeds=DEFAULT_SEEDS,
        points_per_region=1,
    )
    assert report["schema"] == "eq45_supported_phi10_early_localized_seed_generalization_v1"
    ...
    assert report["static_supported_sha256"] != report["early_localized_trial_sha256"]
    ...
    for row in report["finest_rows"]:
        assert isinstance(row["zero_sign_consistent_across_levels"], bool)
        assert isinstance(row["forced_sign_consistent_across_levels"], bool)
    ...
    assert isinstance(summary["zero_all_seed_level_signs_consistent"], bool)
    assert isinstance(summary["forced_all_seed_level_signs_consistent"], bool)
...
def test_early_localized_generalization_contract_fails_closed():
    with pytest.raises(ValueError, match="at least three"):
        audit_early_localized_phi10_seed_generalization(seeds=(1, 2), points_per_region=1)
    with pytest.raises(ValueError, match="unique"):
        audit_early_localized_phi10_seed_generalization(seeds=(1, 1, 2), points_per_region=1)
