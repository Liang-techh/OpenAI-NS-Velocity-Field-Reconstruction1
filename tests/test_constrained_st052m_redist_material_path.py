...
from openai_ns_reconstruction import constrained_st052m_redist_material_path as m
...
def test_frozen_source_and_truth_contract() -> None:
    assert m.TASK_ID == "CR-A9-059"
    assert m.BASE_MAIN_SHA == "f0193d66c9d92948b4820ebcb70263673995b324"
    assert m.AGENT7_PR == 528
    assert m.AGENT7_HEAD == "779ffca71066e2864496d37de55a7aafc45d6f57"
    ...
    assert m.EXPECTED_AGENT7_NORMALIZATION == pytest.approx(1.0032534663681094)
    ...
    assert m.UPSTREAM_AGENT7_RECEIPT["clean_frozen_profile_transfer"] is True
