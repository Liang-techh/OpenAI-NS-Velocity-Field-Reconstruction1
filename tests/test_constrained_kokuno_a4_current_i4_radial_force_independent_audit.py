...
import openai_ns_reconstruction.kokuno_a4_current_i4_radial_force_independent_audit as mod
...
def _stress_receipt(z: float, r: np.ndarray) -> dict[str, object]:
    ...
    return {
        ...
        "parent_agent3_1101_source_blob": mod.STRESS_PARENT_AGENT3_1101_SOURCE_BLOB,
        "provenance": {
            "parent_agent3_head": mod.STRESS_PARENT_AGENT3_1101_HEAD,
            "agent2_composite_head": mod.AGENT2_1080_HEAD,
            "agent1_leading_head": mod.AGENT1_1079_HEAD,
        },
        "axial_e1": {
...
def _force_receipt(center: float, r: np.ndarray) -> dict[str, object]:
    out: dict[str, object] = {
        ...
        "parent_agent3_1109_source_blob": mod.STRESS_AGENT3_SOURCE_BLOB,
...
def _case(center: float) -> dict[str, object]:
    ...
    return {
        ...
        "stress_receipts": [
            _stress_receipt(center + off, r) for off in mod.required_z_offsets()
        ],
    }
...
def test_frozen_protocol_is_deterministic_offgrid_and_axis_near() -> None:
...
def test_fd4_recovers_cubic_stress_derivative() -> None:
    ...
    indexed = {
        off: _stress_values(center + off, r) for off in mod.required_z_offsets()
    }
...
def test_case_and_shift_order_do_not_change_metrics() -> None:
    cases = _cases()
...
def test_force_sign_mutation_is_rejected_by_scoped_gates() -> None:
    cases = _cases()
    for level in cases[0]["force_receipt"]["aggregate"]["levels"]:
...
def test_shifted_stress_mutation_breaks_piece_attribution() -> None:
    cases = _cases()
...
def test_force_provenance_drift_fails_closed() -> None:
    cases = _cases()
    cases[0]["force_receipt"]["provenance"]["agent2_composite_head"] = "0" * 40
    with pytest.raises(ValueError, match="force provenance"):
        mod.audit_serialized_cases(cases)


def test_stress_provenance_drift_fails_closed() -> None:
    cases = _cases()
    cases[0]["stress_receipts"][0]["provenance"]["parent_agent3_head"] = "f" * 40
    with pytest.raises(ValueError, match="stress provenance"):
        mod.audit_serialized_cases(cases)
...
def test_public_audit_signature_has_no_scientific_tuning_knobs() -> None:
    ...
    assert boundary["current_i4_identity_consumed"] is True
