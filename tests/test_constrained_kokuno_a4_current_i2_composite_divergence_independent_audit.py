...
from openai_ns_reconstruction import kokuno_a4_current_i2_composite_divergence_independent_audit as audit
...
def _metric(value: float = 1.0e-6, speed: float = 1.0) -> dict[str, float]:
    return {
        ...
        "i2_entry_seam_sampled_max": value,
...
def test_protocol_is_frozen_to_current_i2_heads_and_gates():
    assert audit.UPSTREAM_PR == 1071
    assert audit.UPSTREAM_HEAD == "48d69e37e78e7f7f0e4e9936f28ff7974719288d"
    assert audit.AGENT1_PR == 1061
    assert audit.AGENT1_HEAD == "9c2abbc69ba1ef8c8a9e8d1699a466a75d0293c3"
    assert audit.SEED == 9173821
    assert audit.SPATIAL_STEPS == (0.02, 0.01, 0.005)
    assert audit.DIVERGENCE_GATE == 1.0e-5


def test_truth_boundary_cannot_promote_full_pde_state():
    truth = audit._TRUTH_BOUNDARY
    assert truth["current_i2_leading_plus_oscillatory_velocity_consumed"] is True
    ...
    assert truth["public_float64_i2_delta_verified_by_this_audit"] is False
...
def test_synthetic_passing_receipt_passes_frozen_gate_logic():
    ...
    assert audit._derive_audit_pass(receipt) is True
...
def test_component_order_invariance_checks_all_aggregate_metrics():
    ...
    assert audit._component_order_invariant(a, copy.deepcopy(a))
    b = copy.deepcopy(a)
    b["pooled_weighted_rms"] += 2.0 * audit.ORDER_INVARIANCE_ATOL
    assert not audit._component_order_invariant(a, b)
