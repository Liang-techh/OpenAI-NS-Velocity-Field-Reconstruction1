"""Screen one endpoint-zero relative temporal phase on the existing ST052-M shoulder channel.

Preregistered in issue #633 before evaluation. The frozen parent is Agent-7 PR #587's
linear-ramp child. The tip Piola taper keeps the accepted linear schedule

    g(t) = 2*(t-.25),

while only the already-existing shoulder swirl-energy compensation receives

    g_beta(t) = g(t) + lambda*16*(t-.25)*(.75-t),  lambda in {-0.02,+0.02}.

No spatial basis, window, coefficient, pressure, forcing, normalization, or visual target
is fitted. The question is whether relative timing of the two already-admitted channels
adds a target-free Pareto-improving spacetime direction.
"""
...
TASK_ID = "CR003-ST052M-SHOULDER-TEMPORAL-PHASE-089"
PREREG_ISSUE = 633
...
SOURCE_COMMON_CURVATURE_PR = 604
SOURCE_PURE_SWIRL_PR = 621
...
TRUTH = {
    ...
    "spatial_basis_changed": False,
    "screened_relative_temporal_degree": True,
...
def curvature_basis(time: float) -> float:
    time = float(time)
    if not (base.TIME_INTERVAL[0] - 1e-12 <= time <= base.TIME_INTERVAL[1] + 1e-12):
        raise ValueError("time outside frozen interval")
    return float(16.0 * (time - 0.25) * (0.75 - time))


def shoulder_activation(time: float, lam: float) -> float:
    if abs(float(lam)) > EPSILON + 1e-15:
...
def phased_velocity(field, raw, points, time, redistribution_scale, beta, lam):
    g_tau = base.activation(time)
    g_beta = shoulder_activation(time, lam)
    u = base.morph.prior.taper.taper_velocity(
        ...
        tau=base.TAPER_TAU * g_tau,
        taper_scale=1.0,
    )
    return base.morph.prior.comp.apply_swirl_compensation(points, u, beta * g_beta)
...
def response_diagnostic(control_fn, ramp_fn, plus_fn, minus_fn) -> dict:
    ...
    for time in ACTIVE_TIMES:
        ...
        ramp_blocks.append((np.asarray(ramp_fn(pts, time), float) - u0).ravel())
        phase_blocks.append(
            ((np.asarray(plus_fn(pts, time), float) - np.asarray(minus_fn(pts, time), float))
             / (2.0 * EPSILON)).ravel()
        )
    ramp = np.concatenate(ramp_blocks)
    phase = np.concatenate(phase_blocks)
    n_ramp = float(np.linalg.norm(ramp))
    n_phase = float(np.linalg.norm(phase))
    if n_ramp <= np.finfo(float).tiny or n_phase <= np.finfo(float).tiny:
        raise RuntimeError("degenerate response column")
    matrix = np.column_stack((ramp / n_ramp, phase / n_phase))
    ...
    return {
        ...
        "ramp_response_norm": n_ramp,
        "phase_response_norm_per_lambda": n_phase,
    }
...
def shoulder_activation_range() -> dict:
    times = np.linspace(0.25, 0.75, 401)
    out = {}
    for label, lam in (("plus", EPSILON), ("minus", -EPSILON)):
        values = np.asarray([shoulder_activation(t, lam) for t in times], float)
...
def run(out: Path) -> dict:
    ...
    if abs(beta - base.EXPECTED_BETA) > 2e-10:
    ...
    plus_fn = lambda p, t: phased_velocity(field, raw, p, t, redistribution_scale, beta, EPSILON)
    minus_fn = lambda p, t: phased_velocity(field, raw, p, t, redistribution_scale, beta, -EPSILON)
    ...
    activation_bounds = shoulder_activation_range()
    structure = {
    ...
    e_plus = base.axisymmetric_energy_at_time(plus_fn, MID_TIME)
    e_minus = base.axisymmetric_energy_at_time(minus_fn, MID_TIME)
    energy = {
        "linear": e_linear,
        "plus": e_plus,
        "minus": e_minus,
        "plus_relative_to_linear": float(e_plus / e_linear - 1.0),
        "minus_relative_to_linear": float(e_minus / e_linear - 1.0),
    }
    ...
    energy_max = max(abs(energy["plus_relative_to_linear"]), abs(energy["minus_relative_to_linear"]))
    structural_preflight_pass = bool(
        ...
        and activation_bounds["plus"]["min"] >= -1e-12
        and activation_bounds["plus"]["max"] <= 1.0 + 1e-12
        and activation_bounds["minus"]["min"] >= -1e-12
        and activation_bounds["minus"]["max"] <= 1.0 + 1e-12
        and energy_max <= ENERGY_DRIFT_MAX
    )
    ...
    report = {
        ...
        "source_common_curvature_pr": SOURCE_COMMON_CURVATURE_PR,
        "source_pure_swirl_pr": SOURCE_PURE_SWIRL_PR,
        "source_render_pr": SOURCE_RENDER_PR,
        "frozen_screen": {
            "epsilon": EPSILON,
            "lambda_values": [-EPSILON, EPSILON],
            "taper_activation": "2*(t-0.25)",
            "shoulder_activation": "2*(t-0.25)+lambda*16*(t-0.25)*(0.75-t)",
            ...
            "midpoint_energy_relative_drift_max": ENERGY_DRIFT_MAX,
            "parameter_scan_performed": False,
        },
        "shoulder_activation_bounds": activation_bounds,
        "endpoint_identity": endpoints,
        "morphology_relative_to_control": morphology,
        "path_relative_to_control": path_relative,
        "midpoint_energy": energy,
        ...
        "shoulder_relative_temporal_growth_justified": growth_justified,
        **TRUTH,
    }
...
def main() -> None:
    ...
    r = run(args.out)
    print("shoulder_relative_temporal_growth_justified=", r["shoulder_relative_temporal_growth_justified"])
    ...
    print("midpoint_energy=", r["midpoint_energy"])
