...
ST071 adapter extends the original recurrence to arbitrary analytic axis data.
Do not identify new arrays as the old ST068 field.
"""
...
def build(order=18,eta_degree=64,h=.005,swirl=1.,slope=4.,bias=.02,
          pressure=1.,pressure_shape=.5, checkpoint_dir=None, axis_F=None, axis_U=None, axis_P=None):
    ...
    """
    if not(0<h<.01 and 0<=order<=64 and eta_degree>=8):raise ValueError('Invalid truncation/h')
    ...
    if axis_F is not None: F[0,:len(axis_F)]=axis_F
    if axis_U is not None: U[0]=0.; U[0,:len(axis_U)]=axis_U
    if axis_P is not None: P[0]=0.; P[0,:len(axis_P)]=axis_P
    axis_pressure=P[0].copy()
    ...
    P=integrate_x(product(F,F));P[0,:len(axis_pressure)]=axis_pressure
    meta=dict(schema='local_axis_series_v1',id='ST071-TRIAL',order=order,eta_degree=eta_degree,h=h,
              axis=dict(F=F[0].tolist(),U=U[0].tolist(),P=axis_pressure.tolist()),
