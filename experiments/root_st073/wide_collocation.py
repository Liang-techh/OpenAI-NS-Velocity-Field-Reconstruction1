...
def run():
 ...
 for k,e in [(k,e) for k in (2.5,5.5) for e in (-.2,.2)]:
  ...
  for label,f in [('baseline',zero),('joint',corrected)]:
 ...
 scope='Width ratio6;24 fixed-scale coefficients, unforced axisymmetric compact corrections. Frozen core and interface jets unchanged. Does not construct global finite energy, axial closure or oscillatory stress realization.',pde_validated=False,global_field_ready=False)
 out=ROOT/'wide_collocation';out.mkdir(exist_ok=True);(out/'report.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
