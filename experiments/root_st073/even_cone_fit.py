...
from wide_modes import CachedWidth,sample
from even_modes import EvenJointModes,expand_old
...
def run():
 base=CachedWidth(6.);prior=expand_old(json.loads((ROOT/'wide_collocation/report.json').read_text())['amplitudes']);training=[];nodes,weights=leggauss(16);Q=prefix_matrix(nodes);interior=np.where(((nodes+1)/2>=.12)&((nodes+1)/2<=.88))[0]
 for k in (1.,4.,5.5):
  tau=.5*2**(-k)
  for eta in (-.3,0,.3):
   ...
   args=(pts,tau,.0005*np.sqrt(base.nu*tau),.00025*tau);f0=EvenJointModes(base,np.zeros(30));jb=jets(f0,*args);modes=[]
   for j in range(30):
    a=np.zeros(30);a[j]=1;ju=jets(EvenJointModes(base,a),*args);modes.append(tuple(x-y for x,y in zip(ju,jb)))
 ...
 fit=least_squares(objective,prior,bounds=(-8,8),max_nfev=80,ftol=1e-8,xtol=1e-8,gtol=1e-8,verbose=0)
 f=EvenJointModes(base,fit.x);old=EvenJointModes(base,prior);rows=[]
 for k in (2.5,5.5):
  for eta in (-.2,.2):
 ...
 objective='Full momentum and radial theta/axial moments, plus soft penalty for interior approximate direction and growth rate exceeding nu/rÃ‚Â² for azimuthal mode1; no hard cone guarantee. Approximate physical frozen tangential formula, not direct normalized paper cone.',
 limitations='Adds eta-squared poloidal and pressure modes; exploratory bounded fit. Fitting cone diagnostics can worsen full momentum and physical L2. Training k1,4,5.5 eta=-.3,0,.3; holdout k2.5,5.5 eta=+/-.2. Finite grids only; no supported wave, global closure or proof.',pde_validated=False,global_field_ready=False)
 out=ROOT/'even_cone_fit';out.mkdir(exist_ok=True);(out/'report.json').write_bytes((json.dumps(report,indent=2)+'\n').encode());print('cone',cone,flush=True)
