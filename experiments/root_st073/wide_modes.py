...
class CachedWidth(WidthField):
 def __init__(self,ratio=6.):super().__init__(ratio);self.cache={};self.join_X=3/64
...
class WidePoloidalModes:
 def __init__(self,base,amplitudes):
  self.base=base;self.inner=base.inner;self.nu=base.nu;self.ratio=base.ratio;self.join_X=getattr(base,'join_X',3/64);self.a=np.asarray(amplitudes).reshape(2,3,2)
 def fields(self,points,tau):
  ...
  ri=np.sqrt(2*self.nu*q*self.join_X);width=(self.ratio-1)*ri;raw_y=(r-ri)/width;active=(raw_y>0)&(raw_y<1);y=np.clip(raw_y,0,1);yz=-(1+(self.ratio-1)*y)*qz/(2*q*(self.ratio-1))
  psi_scale=self.nu**1.5*(2*self.join_X)*q**(1-self.inner.A)
...
class WideAngularModes:
 def __init__(self,base,amplitudes):
  self.base=base;self.inner=base.inner;self.nu=base.nu;self.ratio=base.ratio;self.join_X=getattr(base,'join_X',3/64);self.amplitudes=np.asarray(amplitudes).reshape(4,3)
 def fields(self,points,tau):
  pts=np.asarray(points,float);u,p=self.base.fields(pts,tau)
  r=np.hypot(pts[:,0],pts[:,1]);sn=np.sqrt(self.nu)
  co=coordinates(r/sn,pts[:,2]/sn,np.broadcast_to(tau,r.shape),self.inner.h)
  q=np.asarray(co['q']);eta=np.asarray(co['eta']);ri=np.sqrt(2*self.nu*q*self.join_X)
...
class WideJointModes:
 def __init__(self,base,amplitudes):
  self.base=base;self.inner=base.inner;self.nu=base.nu;self.ratio=base.ratio;self.join_X=getattr(base,'join_X',3/64);self.a=np.asarray(amplitudes)
...
def sample(f,k,eta,order,step=.0005):
 tau=.5*2**(-k);g,w=leggauss(order);ip=f.inner.from_similarity([f.join_X],[eta],tau)[0];ri=ip[0];width=(f.ratio-1)*ri;r=ri+width*(g+1)/2
