...
class LoopModes:
 def __init__(self,base,amplitudes):
  self.base=base;self.inner=base.inner;self.nu=base.nu;self.ratio=base.ratio
  self.field=SwirlLoop(WideJointModes(base,amplitudes),2,-.3)
 def fields(self,points,tau):return self.field.fields(points,tau)
JointModes=LoopModes

def run():
 base=CachedWidth(6.);zero=JointModes(base,np.zeros(24));train=[(k,e) for k in (1.,4.) for e in (-.3,0,.3)];data=[]
 for k,e in train:
  tau=.5*2**(-k);g,w=leggauss(12);ip=base.inner.from_similarity([3/64],[e],tau)[0];r=ip[0]*(1+5*(g+1)/2)
 ...
 start=np.array(json.loads((ROOT/'wide_collocation/report.json').read_text())['amplitudes'])
 fit=least_squares(objective,start,jac=jacobian,bounds=(-8,8),max_nfev=150,ftol=1e-10,xtol=1e-10,gtol=1e-10)
 ...
 for k,e in [(k,e) for k in (2.5,5.5) for e in (-.2,.2)]:
  ...
  for label,f in [('prior',WideJointModes(base,start)),('raw_loop',previous),('repaired_loop',corrected)]:
   r,d,mw=sample(f,k,e,18,.0005);row[label]=metrics(r,d,mw)
   radial_y=(leggauss(18)[0]+1)/2
   row[label]['terminal_rz']=float(-np.sum(mw*6/(1+5*radial_y)*r[:,2]))
 ...
 for k in (2.5,5.5):
  ...
  ze=np.sqrt(base.nu)*q**base.inner.D*(1+2*base.inner.D*eta**2/(1-eta**2));weights=np.repeat(.4*we,12)*np.tile(wy/2,8)*2*np.pi*r*5*ri*ze
 ...
 report=dict(amplitudes=fit.x.tolist(),coefficient_order='12 WidePoloidalModes followed by12 WideAngularModes',bounds=[-8,8],fit_nfev=fit.nfev,fit_success=bool(fit.success),fit_cost=float(fit.cost),initial_cost=float(.5*np.sum(objective(start)**2)),holdouts=rows,volume_audit=volume,surrogate_full_fd_max_difference=float(np.max(np.abs(rb-predicted))),optimizer_jacobian_directional_error=jac_error,
 ...
 volume_domain='eta in[-.4,.4], ri<r<6ri, all azimuths; 12x8 Gauss quadrature with physical Jacobian. No continuum maximum/integral certificate.',
 scope='Width ratio6 with a fixed two-cycle swirl loop of amplitude -.3 and 24 refitted coefficients; unforced axisymmetric compact corrections. Frozen core and interface jets unchanged. Does not construct global finite energy, axial closure or oscillatory stress realization.',pde_validated=False,global_field_ready=False)
 out=ROOT/'loop_repair_fit';out.mkdir(exist_ok=True);(out/'report.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
