...
class FirstIntervalField:
    ...
    def coefficients(self, t):
        ...
        harmonic = [interval*s[:27] for s in self.harmonic_slopes]
        mean = interval*self.mean_slope[:18]
...
class HermiteIntervalField(FirstIntervalField):
    ...
    def coefficients(self, t):
        ...
        harmonic = [a*old[:27]+b*new[:27]
                    for old, new in zip(self.harmonic_slopes,
                                        self.next_harmonic_slopes)]
        mean = a*self.mean_slope[:18]+b*self.next_mean_slope[:18]
...
def run(evolution_name='curl_wave_patch_evolution.json',
        output_name='curl_wave_patch_trajectory.json'):
    ...
    report = {'tau': tau, 'time_step': evolution['time_step'],
              ...
              'evolved_first_interval': residual_metrics(field, points, tau,
                                                          evolution['time_step']),
              'hermite_first_interval': residual_metrics(hermite, points, tau,
                                                         evolution['time_step']),
              'scope': 'Direct finite-difference full momentum at the midpoint of the first linear and Hermite potential-coefficient intervals. The temporal derivative stencil stays within the interval. This is a spatially held-out sample, not an inter-node/global bound or a matched pulse.',
              'accepted': False}
...
if __name__ == '__main__':
    ...
    run(args.evolution_name, args.output_name)
