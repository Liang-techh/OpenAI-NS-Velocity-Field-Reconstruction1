"""Target-free 3-D vorticity morphology transfer audit for ST047-E.

Constrained Agent 7 representation/visualization evidence only.  This compares
frozen ST046-A and frozen ST047-E under the same contract previously used for
ST045-H -> ST046-A.  It does not fit a basis, alter pressure/forcing, choose an
OpenAI image target, recompute held-out PDE residuals, or promote a candidate.
"""
...
TASK_ID = "CR003-ST047-MORPHOLOGY-TRANSFER-SCREEN-059"
...
def _load_frozen_fields() -> tuple[_FrozenField, _FrozenField]:
    """Load exact mathematical parent/child recipes without fitting."""
    child_family, child_raw = replay_st047.reconstruct("ST047-E")

    root = Path(__file__).resolve().parents[2]
    st046 = root / "experiments" / "root_st046"
    if str(st046) not in sys.path:
        sys.path.insert(0, str(st046))
    import replay_st046  # imported after dependency paths are installed by replay_st047

    parent_family, parent_raw = replay_st046.reconstruct("ST046-A")
    return (
        _FrozenField(parent_family, parent_raw, "ST046-A"),
        _FrozenField(child_family, child_raw, "ST047-E"),
    )
...
def _structure_checks(field: _FrozenField, times: Iterable[float]) -> dict[str, object]:
    ...
    for time in times:
        for radius in (0.05, 0.10, 0.20):
            for z in (-0.10, 0.10):
                ux, uy, uz = field.at_points(np.array([[radius, 0.0, z]], dtype=float), float(time))[0]
...
def audit_morphology_transfer(
    *, times: Iterable[float] = DEFAULT_TIMES, grid_sizes: Iterable[int] = DEFAULT_GRID_SIZES
) -> dict[str, object]:
    ...
    parent, child = _load_frozen_fields()
    by_grid = {}
    ...
    for ident in ("parent", "child"):
        ...
        refinement[ident] = {
            key: max(
                abs(coarse_row[ident][key] - fine_row[ident][key]) / max(abs(fine_row[ident][key]), 1.0e-12)
    ...
    # Reuse the exact Agent-7 clean-transfer rule preregistered in PR #378.
    ...
    if clean_transfer:
        routing = (
            "ST047-E preserves a clean axial morphology gain over ST046-A under the frozen Agent-7 contract. "
            "Use ST047-E as the next visualization backbone; keep the single Piola axial-warp degree from PR #367 "
            "in reserve rather than resuming basis growth."
        )
    else:
        routing = (
            "ST047-E's lower full-momentum residual does not satisfy the frozen clean axial-morphology transfer rule. "
            "Keep ST047-E as the residual backbone, but do not assume it is the visualization backbone; inspect the "
            "reported defect channel and, if axial reach is the issue, test only the already-identified single Piola "
            "warp degree from PR #367 rather than adding another basis family."
        )

    recipes = json.loads((Path(__file__).resolve().parent / "recipes.json").read_text())
    child_recipe = recipes["ST047-E"]
    return {
        "task_id": TASK_ID,
        "issue": 387,
        "parent_id": "ST046-A",
        "child_id": "ST047-E",
        "parent_original_raw_sha256": child_recipe.get("parent_original_raw_sha256"),
        "child_original_raw_sha256": child_recipe.get("original_candidate_sha256"),
        "contract": {
            "times": list(times),
            ...
            "clean_transfer_rule_reused_from_pr_378": True,
        },
...
def main() -> None:
    ...
    parser.add_argument("--output", default="artifacts/st047_agent7_morphology_transfer/report.json")
